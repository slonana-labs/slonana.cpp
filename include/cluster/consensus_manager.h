...
public:
  ...
  virtual bool
  send_append_entries_response(const AppendEntriesResponse &response) = 0;
};
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<IClusterCommunication>>
      peers_;
  ...
  get_peer_communication(const std::string &peer_id);
...
public:
  ...
  void set_state_machine_callback(
...
std::vector<uint8_t>
serialize_append_entries(const AppendEntriesRequest &request);
AppendEntriesRequest
deserialize_append_entries(const std::vector<uint8_t> &data);
} // namespace consensus_utils
