...
public:
    ...
    virtual bool redirect_traffic(const std::string& from_node, const std::string& to_node) = 0;
...
private:
    ...
    bool should_trigger_failover(const std::string& node_id, FailoverTrigger& trigger);
...
public:
    ...
    void set_action_handler(std::shared_ptr<IFailoverActionHandler> handler) { action_handler_ = handler; }
    ...
    // Failover operations
    bool trigger_manual_failover(const std::string& node_id, const std::string& reason = "");
    ...
    // Callbacks
    void set_failover_callback(std::function<void(const FailoverEvent&)> callback);
