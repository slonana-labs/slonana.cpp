...
struct MasterNode {
    ...
    MasterNode() : role(MasterRole::NONE), shard_id(0), load_score(0), is_healthy(true) {}
};
...
struct ServiceAffinityRule {
    ...
    std::string load_balancing_strategy;  // "round_robin", "least_loaded", "regional"
};
...
public:
    ...
    std::vector<std::string> get_masters_for_service(const std::string& service_type) const;
    
    // Load balancing
    bool update_master_load(const std::string& master_id, uint64_t load_score);
    std::string select_least_loaded_master(MasterRole role = MasterRole::NONE) const;
    std::string select_master_by_region(const std::string& region, MasterRole role = MasterRole::NONE) const;
    ...
    bool broadcast_to_masters(const std::string& message_type, const std::vector<uint8_t>& payload);
    ...
    // Network topology
    bool add_master_to_region(const std::string& master_id, const std::string& region);
    std::vector<std::string> get_regional_masters(const std::string& region) const;
    bool setup_cross_region_replication(const std::string& source_region, const std::string& target_region);
    ...
    // Event callbacks
    using MasterEventCallback = std::function<void(const std::string& event, const MasterNode& master)>;
    void set_master_event_callback(MasterEventCallback callback) { master_event_callback_ = callback; }
...
private:
    ...
    std::unordered_map<MasterRole, std::unordered_set<std::string>> masters_by_role_;
    std::unordered_map<std::string, std::unordered_set<std::string>> masters_by_region_;
    ...
    uint64_t calculate_master_priority(const MasterNode& master, MasterRole role) const;
