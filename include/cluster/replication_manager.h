...
public:
    ...
    virtual bool send_batch(const std::string& target_id, const ReplicationBatch& batch) = 0;
    virtual bool send_heartbeat(const std::string& target_id) = 0;
    virtual bool request_sync(const std::string& target_id, uint64_t from_index) = 0;
};
...
private:
    ...
    std::function<std::vector<ReplicationEntry>(uint64_t, uint64_t)> data_provider_;
...
public:
    ...
    void set_transport(std::shared_ptr<IReplicationTransport> transport) { transport_ = transport; }
    
    // Target management
    bool add_target(const std::string& node_id, const std::string& address, uint16_t port);
    ...
    bool replicate_entry(const std::vector<uint8_t>& data, uint64_t index, uint64_t term);
    ...
    // Callbacks
    void set_replication_callback(std::function<void(const std::string&, bool)> callback);
    void set_target_failed_callback(std::function<void(const std::string&)> callback);
    void set_data_provider(std::function<std::vector<ReplicationEntry>(uint64_t, uint64_t)> provider);
