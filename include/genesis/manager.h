...
public:
   ...
   */
  static common::Result<GenesisConfig>
  load_genesis_config(const std::string &filepath);

  /**
   * Save genesis configuration to JSON file
   */
  static common::Result<bool> save_genesis_config(const GenesisConfig &config,
                                                  const std::string &filepath);

  /**
   * Create the actual genesis block from configuration
   */
  static common::Result<ledger::Block>
  create_genesis_block(const GenesisConfig &config);

  /**
   * Validate a genesis configuration
   */
  static common::Result<bool>
  validate_genesis_config(const GenesisConfig &config);
   ...
   */
  static std::vector<std::pair<common::PublicKey, uint64_t>>
  create_genesis_accounts(const GenesisConfig &config);
...
private:
  ...
  static common::Result<GenesisConfig>
  deserialize_from_json(const std::string &json);
...
private:
   ...
   */
  static common::Result<GenesisConfig> parse_creation_args(int argc,
                                                           char *argv[]);

  /**
   * Parse validator list from file
   */
  static common::Result<std::vector<GenesisValidator>>
  parse_validator_list(const std::string &filepath);
};
