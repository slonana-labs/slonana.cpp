...
#define CONSENSUS_TIMER_BLOCK_VALIDATION()                                     \
  auto _timer = slonana::monitoring::GlobalConsensusMetrics::instance()        \
...
#define CONSENSUS_TIMER_FORK_CHOICE()                                          \
  auto _timer = slonana::monitoring::GlobalConsensusMetrics::instance()        \
                    .create_fork_choice_timer()

#define CONSENSUS_TIMER_ROUND()                                                \
  auto _timer = slonana::monitoring::GlobalConsensusMetrics::instance()        \
                    .create_consensus_round_timer()

#define CONSENSUS_TIMER_POH_TICK()                                             \
  auto _timer = slonana::monitoring::GlobalConsensusMetrics::instance()        \
                    .create_poh_tick_timer()

#define CONSENSUS_TIMER_POH_VERIFICATION()                                     \
  auto _timer = slonana::monitoring::GlobalConsensusMetrics::instance()        \
                    .create_poh_verification_timer()
...
public:
  ...
  PerformanceReport generate_report(
      std::chrono::milliseconds analysis_period = std::chrono::minutes(5));
