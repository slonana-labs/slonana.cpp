...
enum class NetworkType { MAINNET, TESTNET, DEVNET, LOCALNET };
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<ClusterNode>>
      connected_nodes_;
  ...
  void discover_peers();
  void handle_discovery_response(const std::vector<uint8_t> &data,
  ...
  // Network I/O methods
  ssize_t read_from_peer_socket(const std::string &peer_id,
                                std::vector<uint8_t> &buffer);
  bool parse_cluster_message(const std::vector<uint8_t> &buffer,
                             size_t bytes_read, ClusterMessage &msg);
...
public:
  ...
  // Message handling
  void
  set_message_callback(std::function<void(const ClusterMessage &)> callback);
  void set_node_discovered_callback(
      std::function<void(const ClusterNode &)> callback);
  void set_node_disconnected_callback(
      std::function<void(const std::string &)> callback);

  // Communication
  bool send_message(const ClusterMessage &msg,
                    const std::string &target_node_id = "");
  bool broadcast_message(const ClusterMessage &msg);
  bool forward_transaction(const std::vector<uint8_t> &transaction_data);
  bool announce_block(uint64_t slot, const std::string &block_hash,
