...
private:
    ...
    // Shred tracking
    std::unordered_map<std::string, std::chrono::steady_clock::time_point> shred_timestamps_;
    ...
    // Callbacks
    std::function<void(const Shred&, const std::vector<TurbineNode>&)> send_callback_;
...
public:
     ...
     */
    void set_send_callback(std::function<void(const Shred&, const std::vector<TurbineNode>&)> callback);
    
    /**
     * Set receive callback for shred processing
     * @param callback Function to call when receiving shreds
     */
    void set_receive_callback(std::function<void(const Shred&, const std::string&)> callback);
     ...
     */
    void set_retransmit_params(uint32_t max_attempts, std::chrono::milliseconds timeout);
...
public:
     ...
     * @return true if signature is valid
     */
    static bool validate_signature(const Shred& shred, const std::vector<uint8_t>& expected_pubkey);
...
namespace shred_utils {
    ...
    std::vector<Shred> split_data_into_shreds(const std::vector<uint8_t>& data, 
                                             uint64_t slot, uint32_t start_index = 0);
