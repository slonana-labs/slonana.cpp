...
struct TurbineNode {
    ...
    bool operator==(const TurbineNode& other) const {
        return pubkey == other.pubkey && address == other.address && port == other.port;
    }
    
    bool operator<(const TurbineNode& other) const {
        if (pubkey != other.pubkey) return pubkey < other.pubkey;
...
private:
    ...
    std::vector<size_t> calculate_children_indices(size_t node_index, uint32_t fanout) const;
    std::vector<size_t> calculate_retransmit_indices(size_t node_index) const;
    
public:
    explicit TurbineTree(const TurbineNode& self_node, uint32_t fanout = DATA_PLANE_FANOUT);
    ~TurbineTree() = default;
