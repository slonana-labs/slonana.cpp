...
private:
    ...
    std::unordered_map<std::string, std::vector<uint64_t>> signature_subscriptions;
    ...
    void handle_client_connection(int client_socket, const std::string& client_addr);
    void handle_websocket_handshake(int client_socket);
    void handle_websocket_message(std::shared_ptr<WebSocketConnection> conn, const std::string& message);
    ...
    bool parse_websocket_frame(const std::vector<uint8_t>& frame, std::string& payload);
    std::vector<uint8_t> create_websocket_frame(const std::string& payload);
    
    // Subscription handlers
    void handle_account_subscribe(std::shared_ptr<WebSocketConnection> conn, const std::string& request);
    void handle_signature_subscribe(std::shared_ptr<WebSocketConnection> conn, const std::string& request);
    void handle_slot_subscribe(std::shared_ptr<WebSocketConnection> conn, const std::string& request);
    void handle_block_subscribe(std::shared_ptr<WebSocketConnection> conn, const std::string& request);
    void handle_unsubscribe(std::shared_ptr<WebSocketConnection> conn, const std::string& request);
...
public:
    ...
    void notify_account_change(const std::string& pubkey, const std::string& account_data);
    void notify_signature_status(const std::string& signature, const std::string& status);
    void notify_slot_change(uint64_t slot, const std::string& parent_slot, uint64_t root_slot);
    void notify_block_change(uint64_t slot, const std::string& block_hash, const std::string& block_data);
    void notify_program_account_change(const std::string& program_id, const std::string& account_pubkey, const std::string& account_data);
