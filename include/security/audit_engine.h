...
enum class AuditLevel { LOW, MEDIUM, HIGH, CRITICAL };
...
enum class AuditStatus { PENDING, IN_PROGRESS, COMPLETED, FAILED, SKIPPED };
...
private:
  ...
  void export_report(const AuditReport &report, const std::string &format,
                     const std::string &output_path);

public:
  SecurityAuditEngine(const AuditConfig &config,
  ...
  get_findings_by_category(AuditCategory category) const;
  AuditReport get_report();
  bool export_report(const std::string &output_path,
...
std::vector<AuditFinding>
filter_findings_by_severity(const std::vector<AuditFinding> &findings,
                            AuditLevel min_severity);
