...
struct LoadedAccount {
  ...
  LoadedAccount(const PublicKey &addr, const AccountInfo &acc, size_t size,
                Slot s, bool write, bool sign)
      : address(addr), account(acc), loaded_size(size), slot(s),
        is_writable(write), is_signer(sign) {}
};
...
struct LoadedTransaction {
  ...
  LoadedTransaction()
      : fee(0), rent(0), loaded_accounts_data_size(0),
        load_result(TransactionLoadResult::SUCCESS) {}

  bool is_success() const {
    return load_result == TransactionLoadResult::SUCCESS;
  }
};
...
public:
   ...
   */
  LoadedTransaction
  load_transaction_accounts(const std::vector<PublicKey> &account_keys,
   ...
   */
  std::optional<LoadedAccount> load_account(const PublicKey &address,
   ...
   */
  TransactionLoadResult validate_fee_payer(const PublicKey &fee_payer,
                                           Lamports fee_amount,
