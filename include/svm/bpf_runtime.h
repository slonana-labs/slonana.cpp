...
struct BpfExecutionResult {
  ...
  BpfExecutionResult()
      : success(false), return_value(0), compute_units_consumed(0) {}
...
public:
   ...
   */
  BpfExecutionResult execute(const BpfProgram &program,
                             const BpfExecutionContext &context);

  /**
   * Execute using interpreter mode
   */
  BpfExecutionResult execute_interpreter(const BpfProgram &program,
                                         const BpfExecutionContext &context);

  /**
   * Execute using JIT compilation
   */
  BpfExecutionResult execute_jit(const BpfProgram &program,
                                 const BpfExecutionContext &context);
...
private:
  ...
  BpfExecutionResult execute_internal(const BpfProgram &program,
                                      const BpfExecutionContext &context,
                                      bool use_jit);
};
