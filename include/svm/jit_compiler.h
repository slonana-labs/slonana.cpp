...
public:
  ...
  execute_native(void *function_ptr, const std::vector<AccountInfo> &accounts,
                 const std::vector<uint8_t> &instruction_data) = 0;
...
public:
  ...
  virtual void *
  get_function_pointer(const std::vector<uint8_t> &native_code) = 0;
  virtual bool
  supports_optimization_level(JITOptimizationLevel level) const = 0;
...
public:
  ...
  optimize_bytecode(const std::vector<uint8_t> &bytecode,
                    const ProgramProfile &profile);

private:
  std::vector<JITBasicBlock>
  build_control_flow_graph(const std::vector<uint8_t> &bytecode);
  void eliminate_dead_code(std::vector<JITBasicBlock> &blocks);
  void perform_constant_folding(std::vector<JITBasicBlock> &blocks);
  void inline_hot_functions(std::vector<JITBasicBlock> &blocks,
                            const ProgramProfile &profile);
  ...
  bool is_dead_instruction(const JITInstruction &instr,
                           const std::vector<JITBasicBlock> &blocks);
  bool is_constant_expression(const JITInstruction &instr);
  bool can_eliminate_bounds_check(const JITInstruction &instr,
                                  const std::vector<JITBasicBlock> &blocks);
};
...
public:
  ...
  void record_execution(const std::string &program_id,
                        uint64_t execution_time_us, uint64_t instruction_count,
                        const std::vector<std::string> &syscalls_used);

  void record_hot_instruction(const std::string &program_id,
                              uint64_t instruction_offset);

  ProgramProfile get_profile(const std::string &program_id) const;
  std::vector<std::string>
  get_hot_programs(uint64_t min_execution_count = 50) const;
...
public:
  ...
  void register_program(const std::string &program_id,
                        const std::vector<uint8_t> &bytecode);
  std::vector<uint8_t>
  get_program_bytecode(const std::string &program_id) const;
...
public:
  ...
  // Compilation control
  bool compile_program(const std::string &program_id,
                       const std::vector<uint8_t> &bytecode);
...
private:
  ...
  bool should_compile_program(const std::string &program_id,
                              const ProgramProfile &profile);
  void update_compilation_stats(uint64_t compilation_time_ms,
                                size_t bytecode_size, size_t native_size);

  ExecutionResult
  execute_interpreted(const std::string &program_id,
                      const std::vector<uint8_t> &bytecode,
                      const std::vector<AccountInfo> &accounts,
                      const std::vector<uint8_t> &instruction_data);

  ExecutionResult
  execute_compiled(JITProgram *program,
                   const std::vector<AccountInfo> &accounts,
                   const std::vector<uint8_t> &instruction_data);
};
...
public:
  static std::unique_ptr<IJITBackend>
  create_llvm_backend(const JITConfig &config);
  static std::unique_ptr<IJITBackend>
  create_cranelift_backend(const JITConfig &config);
  static std::unique_ptr<IJITBackend>
  create_native_backend(const JITConfig &config);
