 ...
 * Nonce account state
 */
enum class NonceState { UNINITIALIZED, INITIALIZED };
...
public:
   ...
   */
  bool initialize_nonce(const PublicKey &authority,
                        const Hash &recent_blockhash, Lamports fee_calculator);

  /**
   * Authorize new authority for nonce account
   */
  bool authorize_nonce(const PublicKey &current_authority,
                       const PublicKey &new_authority);

  /**
   * Withdraw from nonce account
   */
  bool withdraw_nonce(const PublicKey &authority, Lamports amount,
   ...
   */
  static std::optional<NonceData>
  parse_nonce_data(const std::vector<uint8_t> &data);

  /**
   * Create nonce account data
   */
  static std::vector<uint8_t>
  create_nonce_account_data(const NonceData &nonce_data);
