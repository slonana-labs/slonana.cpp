...
struct ExecutionTask {
  ...
  ExecutionTask(const ExecutionTask &other)
      : task_id(other.task_id), program_id(other.program_id),
        bytecode(other.bytecode), accounts(other.accounts),
        instruction_data(other.instruction_data),
        read_accounts(other.read_accounts),
        write_accounts(other.write_accounts), priority(other.priority),
  ...
  ExecutionTask(ExecutionTask &&other) noexcept
      : task_id(std::move(other.task_id)),
        ...
        priority(other.priority), submission_time(other.submission_time),
...
public:
  ...
  void register_account_access(const std::string &task_id,
  ...
  ConflictType detect_conflict(const ExecutionTask &task1,
  ...
  build_execution_groups(std::vector<ExecutionTask *> &tasks);
  bool has_cyclic_dependency(const std::vector<ExecutionTask *> &tasks);

private:
  bool is_read_write_conflict(const std::string &account,
                              const ExecutionTask &reader,
                              const ExecutionTask &writer);
  bool is_write_write_conflict(const std::string &account,
                               const ExecutionTask &writer1,
                               const ExecutionTask &writer2);
  void extract_account_accesses(const ExecutionTask &task,
...
public:
  ...
  bool begin_speculation(const std::string &task_id,
                         const std::vector<AccountInfo> &accounts);
  ExecutionResult execute_speculatively(const ExecutionTask &task);
  bool validate_speculation(const std::string &task_id,
...
private:
  ...
  bool compare_account_state(const AccountInfo &account,
...
public:
  ...
  void set_max_parallel_tasks(size_t max_tasks) {
    max_parallel_tasks_ = max_tasks;
  }
  void enable_speculation(bool enable) { enable_speculation_ = enable; }
  void set_thread_count(size_t count);

  // Task submission
  std::string submit_task(const std::string &program_id,
  ...
  std::future<ExecutionResult> submit_task_async(
      ...
      const std::vector<AccountInfo> &accounts,
      const std::vector<uint8_t> &instruction_data, uint64_t priority = 0);

  // Batch execution
  std::vector<ExecutionResult>
  execute_batch(const std::vector<ExecutionTask> &tasks,
                bool preserve_order = true);

  std::vector<std::future<ExecutionResult>>
  execute_batch_async(const std::vector<ExecutionTask> &tasks);
...
private:
  ...
  bool process_account_chunk(const ExecutionTask &task, size_t start_idx,
                             size_t end_idx);
  ExecutionResult execute_bytecode_parallel(const ExecutionTask &task);
  DecodedInstruction decode_instruction(const std::vector<uint8_t> &bytecode,
                                        size_t offset);
  bool is_parallelizable_instruction(uint8_t opcode);
  ExecutionResult execute_instruction_pipeline(
...
public:
  static std::unique_ptr<ParallelExecutor>
  create_optimized_executor(const ParallelExecutionConfig &config);

  static std::unique_ptr<ParallelExecutor> create_sequential_executor();

  static std::unique_ptr<ParallelExecutor>
  create_speculative_executor(const ParallelExecutionConfig &config);

  static ParallelExecutionConfig get_optimal_config();
  static void
  benchmark_configurations(std::vector<ParallelExecutionConfig> &configs);
};
