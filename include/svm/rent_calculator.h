...
public:
  ...
  struct RentConfig {
    ...
    RentConfig(Lamports per_byte, double threshold, Slot slots)
        : lamports_per_byte_year(per_byte), exemption_threshold(threshold),
   ...
   */
  Lamports calculate_rent_due(Lamports current_balance, size_t data_size,
                              Slot current_slot,
   ...
   */
  bool needs_rent_collection(Lamports balance, size_t data_size,
                             Slot current_slot, Slot last_rent_epoch) const;
  ...
  RentCollection collect_rent(Lamports current_balance, size_t data_size,
                              Slot current_slot,
                              Slot rent_epoch_start_slot) const;
   ...
   */
  static constexpr double SLOTS_PER_YEAR =
      365.25 * 24.0 * 60.0 * 60.0 * 2.0; // ~2 slots per second
