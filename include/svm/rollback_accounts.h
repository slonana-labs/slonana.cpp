...
public:
     ...
     */
    const std::optional<NonceInfo>& get_nonce_account() const { return nonce_account_; }
     ...
     */
    const AccountInfo& get_fee_payer_rollback_account() const { return fee_payer_rollback_account_; }
     ...
     */
    void apply_rollback(std::unordered_map<PublicKey, ProgramAccount>& accounts) const;
    
    /**
     * Create fee-only rollback (for failed transactions that only pay fees)
     */
    static RollbackAccounts create_fee_only_rollback(
        const PublicKey& fee_payer_address,
