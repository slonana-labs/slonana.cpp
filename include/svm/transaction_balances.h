...
public:
     ...
     */
    void record_pre_balances(const std::vector<PublicKey>& addresses, 
                           const std::unordered_map<PublicKey, ProgramAccount>& accounts);
    
    /**
     * Record post-execution balances
     */
    void record_post_balances(const std::vector<PublicKey>& addresses,
                            const std::unordered_map<PublicKey, ProgramAccount>& accounts);
...
private:
    ...
    Lamports get_account_balance(const PublicKey& address, 
...
public:
     ...
     */
    void collect_pre_balances(const std::vector<PublicKey>& addresses,
                            const std::unordered_map<PublicKey, ProgramAccount>& accounts);
    
    /**
     * Record post-execution balances for current transaction
     */
    void collect_post_balances(const std::vector<PublicKey>& addresses,
                             const std::unordered_map<PublicKey, ProgramAccount>& accounts);
    
    /**
     * Get balances for all collected transactions
     */
    const std::vector<TransactionBalances>& get_all_balances() const { return all_balances_; }
    
    /**
     * Get balances for specific transaction
     */
    std::optional<TransactionBalances> get_transaction_balances(size_t transaction_index) const;
     ...
     */
    bool has_unbalanced_transactions(const std::vector<Lamports>& fees = {}) const;
    ...
    CollectionStats get_collection_statistics(const std::vector<Lamports>& fees = {}) const;
...
namespace balance_utils {
     ...
     */
    bool validate_balance_conservation(const TransactionBalances& balances, Lamports fee_amount = 0);
     ...
     */
    std::vector<AccountBalance> find_largest_changes(const TransactionBalances& balances, size_t limit = 10);
