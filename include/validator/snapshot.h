...
public:
  ...
  bool create_incremental_snapshot(uint64_t slot, uint64_t base_slot,
  ...
  load_accounts_from_snapshot(const std::string &snapshot_path);
  ...
  void set_auto_snapshot_interval(uint64_t slots) {
    auto_snapshot_interval_ = slots;
  }
...
private:
  ...
  // Helper methods
  std::string generate_snapshot_filename(uint64_t slot,
                                         bool is_incremental = false,
                                         uint64_t base_slot = 0) const;
  bool compress_data(const std::vector<uint8_t> &input,
                     std::vector<uint8_t> &output) const;
  bool decompress_data(const std::vector<uint8_t> &input,
                       std::vector<uint8_t> &output) const;
  std::string calculate_hash(const std::vector<uint8_t> &data) const;

  // Serialization helpers
  std::vector<uint8_t>
  serialize_metadata(const SnapshotMetadata &metadata) const;
  ...
  AccountSnapshot deserialize_account(const std::vector<uint8_t> &data,
                                      size_t &offset) const;
  ...
  void update_ledger_metadata(size_t total_accounts,
                              size_t restored_accounts) const;
  void verify_ledger_consistency() const;

  // Production-ready account collection methods
  std::string calculate_block_hash(uint64_t slot,
                                   const std::string &ledger_path) const;
  bool collect_accounts_from_ledger(const std::string &ledger_path,
                                    uint64_t slot,
                                    std::vector<AccountSnapshot> &accounts,
  ...
  void parse_account_file(const std::string &file_path,
                          std::vector<AccountSnapshot> &accounts,
                          uint64_t &total_lamports) const;
  bool create_production_test_accounts(uint64_t slot,
                                       std::vector<AccountSnapshot> &accounts,
                                       uint64_t &total_lamports) const;
  bool
  collect_incremental_accounts(const std::string &ledger_path, uint64_t slot,
                               uint64_t base_slot,
...
public:
  ...
  // Configuration
  void set_full_snapshot_interval(uint64_t slots) {
    full_snapshot_interval_ = slots;
  }
  void set_incremental_snapshot_interval(uint64_t slots) {
    incremental_snapshot_interval_ = slots;
  }
  void set_cleanup_enabled(bool enabled) { cleanup_enabled_ = enabled; }
  void set_max_snapshots_to_keep(uint64_t count) {
    max_snapshots_to_keep_ = count;
  }
  ...
  std::chrono::system_clock::time_point get_last_snapshot_time() const {
...
public:
  explicit SnapshotStreamingService(
      std::shared_ptr<SnapshotManager> snapshot_manager);
  ~SnapshotStreamingService() = default;

  // Streaming operations
  bool start_snapshot_stream(const std::string &snapshot_path,
                             const std::string &peer_address,
                             size_t chunk_size = 1024 * 1024);
  std::vector<slonana::validator::SnapshotChunk>
  get_snapshot_chunks(const std::string &snapshot_path,
                      size_t chunk_size = 1024 * 1024) const;
  bool receive_snapshot_chunk(const slonana::validator::SnapshotChunk &chunk,
                              const std::string &output_path);

  // Verification
  bool verify_stream_integrity(
      const std::vector<slonana::validator::SnapshotChunk> &chunks) const;
  std::string calculate_stream_hash(
      const std::vector<slonana::validator::SnapshotChunk> &chunks) const;
...
private:
  ...
  // Helper methods
  std::vector<uint8_t>
  load_snapshot_data(const std::string &snapshot_path) const;
  bool save_snapshot_data(const std::string &output_path,
