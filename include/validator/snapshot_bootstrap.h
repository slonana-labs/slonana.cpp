 ...
 */
using BootstrapProgressCallback = std::function<void(const std::string& phase, 
                                                   uint64_t current, 
                                                   uint64_t total)>;
...
public:
    ...
    common::Result<bool> download_snapshot(const SnapshotInfo& info, std::string& local_path_out);
...
private:
    ...
    bool extract_snapshot_archive(const std::string& archive_path, const std::string& extract_dir);
