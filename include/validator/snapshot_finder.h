...
public:
  ...
  double measure_download_speed(const std::string &snapshot_url,
                                size_t test_bytes = 1024 * 1024);
  ...
  get_default_rpc_endpoints(const std::string &network = "mainnet-beta");
  static std::vector<std::string>
  get_snapshot_mirror_urls(const std::string &network = "mainnet-beta");
  static bool is_valid_snapshot_slot(uint64_t slot, uint64_t current_slot,
                                     uint64_t max_age);
...
private:
  ...
  // Helper methods
  void worker_thread_function(const std::vector<std::string> &rpc_urls,
                              size_t start_index, size_t end_index);

  common::Result<std::vector<std::string>>
  discover_cluster_nodes(const std::vector<std::string> &seed_endpoints);
  std::vector<std::string>
  extract_rpc_endpoints_from_cluster_response(const std::string &response);
  ...
  std::string build_snapshot_download_url(const std::string &rpc_url,
                                          uint64_t slot);
...
private:
  ...
  static void print_rpc_nodes(const std::vector<RpcNodeInfo> &nodes,
                              bool json_format = false);
  static void
  print_snapshot_qualities(const std::vector<SnapshotQuality> &qualities,
                           bool json_format = false);
};
