...
public:
  ...
  struct Config {
    ...
    bool enable_mock_devices =
        true; // Disable for testing scenarios requiring no devices
  };
...
private:
  ...
  create_wallet_for_device(const DeviceInfo &device);

  /**
   * @brief Update device state
   */
  void update_device_state(const std::string &device_id,
                           ConnectionStatus status);
