...
# Slonana 3-Node Cluster Benchmark Script
# Automated benchmarking script for Slonana C++ validator cluster
...
EXAMPLES:
    ...
    $SCRIPT_NAME --cluster /tmp/slonana_cluster --results /tmp/slonana_results
...
parse_arguments() {
    ...
    if [[ -z "$VALIDATOR_BIN" ]]; then
        if [[ -f "$BUILD_DIR/slonana_validator" ]]; then
            ...
            log_error "Slonana validator binary not found. Build it first with: cmake && make"
            exit 3
        fi
    fi
}
...
check_dependencies() {
    ...
    if [[ ! -f "$VALIDATOR_BIN" ]] && ! command -v "$VALIDATOR_BIN" &> /dev/null; then
...
setup_cluster() {
    log_info "Setting up 3-node Slonana cluster environment..."
...
start_validator_node() {
    ...
    log_info "Starting Slonana validator node $node_num..."
    ...
    cmd+=(--ledger-path "$ledger_dir")
    cmd+=(--identity "$identity_file")
    cmd+=(--rpc-bind-address "127.0.0.1:$rpc_port")
    cmd+=(--gossip-bind-address "127.0.0.1:$gossip_port")
    cmd+=(--log-level info)
    cmd+=(--network-id devnet)
    cmd+=(--allow-stale-rpc)
    cmd+=(--faucet-port "$((9900 + node_num - 1))")
    cmd+=(--rpc-faucet-address "127.0.0.1:$((9900 + node_num - 1))")
...
start_cluster() {
    log_info "Starting 3-node Slonana cluster..."

    # Configure system limits
    ulimit -n 1000000 2>/dev/null || ulimit -n 65536 2>/dev/null || ulimit -n 4096 2>/dev/null || true
...
{
  "validator_type": "slonana-3node-cluster",
...
EOF
    ...
    # Display summary
    echo ""
    echo "=== Slonana 3-Node Cluster Benchmark Results ==="
...
# Main execution
main() {
    log_info "Starting Slonana 3-node cluster benchmark..."
    ...
    log_success "Slonana 3-node cluster benchmark completed successfully!"
