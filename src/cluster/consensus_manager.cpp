...
ConsensusManager::ConsensusManager(const std::string &node_id,
  ...
  std::cout << "Consensus manager initialized for node: " << node_id_
            << std::endl;
}
...
bool ConsensusManager::propose_value(const std::vector<uint8_t> &value) {
  ...
  std::cout << "Proposed value at index " << index << " term " << current_term_
            << std::endl;
  return true;
}
...
void ConsensusManager::handle_vote_response(const VoteResponse &response) {
  ...
  if (current_state_ != NodeState::CANDIDATE ||
...
void ConsensusManager::handle_append_entries(
  ...
  if (request.term >= current_term_) {
    ...
    if (request.prev_log_index == 0 ||
      ...
      if (request.leader_commit > commit_index_) {
        commit_index_ =
            std::min(request.leader_commit, static_cast<uint64_t>(log_.size()));
...
void ConsensusManager::handle_append_entries_response(
    const std::string &peer_id, const AppendEntriesResponse &response) {
  std::lock_guard<std::mutex> lock(state_mutex_);
...
void ConsensusManager::send_heartbeats() {
  ...
  AppendEntriesRequest request{current_term_, node_id_, prev_log_index,
                               prev_log_term, {},       commit_index_};
  ...
  for (const auto &[peer_id, comm] : peers_) {
    ...
    if (it != peer_states_.end()) {
      ...
      uint64_t prev_log_term =
          (prev_log_index == 0) ? 0 : log_[prev_log_index - 1].term;

      AppendEntriesRequest request{current_term_, node_id_, prev_log_index,
                                   prev_log_term, {entry},  commit_index_};
...
bool ConsensusManager::is_log_up_to_date(uint64_t last_log_index,
...
void ConsensusManager::process_pending_operations() {
  // Comprehensive pending operation processing with timeout handling and retry
  // logic
  std::lock_guard<std::mutex> lock(state_mutex_);
  ...
  while (it != pending_operations_.end()) {
    ...
    if (elapsed > std::chrono::milliseconds(operation.timeout_ms)) {
      std::cout << "Consensus: Operation " << operation.id
                << " timed out after " << elapsed.count() << "ms" << std::endl;
      ...
      if (operation.retry_count < operation.max_retries) {
        ...
        std::cout << "Consensus: Operation " << operation.id << " failed after "
                  << operation.max_retries + 1 << " attempts" << std::endl;
  ...
  for (auto &operation : pending_operations_) {
    if (operation.confirmations >= required_confirmations_) {
      std::cout << "Consensus: Operation " << operation.id << " confirmed with "
                << operation.confirmations << " votes" << std::endl;
  ...
  pending_operations_.erase(
      std::remove_if(pending_operations_.begin(), pending_operations_.end(),
                     [this](const auto &op) {
                       return op.confirmations >= required_confirmations_;
                     }),
