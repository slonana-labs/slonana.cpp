...
FailoverManager::FailoverManager(const std::string &node_id,
  ...
  std::cout << "Failover manager initialized for node: " << node_id_
            << std::endl;
}
...
void FailoverManager::register_node(const std::string &node_id) {
  ...
  std::cout << "Registered node for failover monitoring: " << node_id
            << std::endl;
}
...
void FailoverManager::unregister_node(const std::string &node_id) {
  ...
  if (it != node_health_.end()) {
    ...
    std::cout << "Unregistered node from failover monitoring: " << node_id
              << std::endl;
  }
}

void FailoverManager::update_node_health(const std::string &node_id,
  ...
  if (it != node_health_.end()) {
    ...
    return health.is_responsive && health.is_available &&
           ...
           health.network_latency_ms <
               failover_config_.network_latency_threshold_ms;
  }

  return false;
}

bool FailoverManager::trigger_manual_failover(const std::string &node_id,
                                              const std::string &reason) {
  if (is_failover_cooldown_active()) {
    std::cout << "Failover cooldown active, manual failover rejected"
              << std::endl;
    return false;
  }

  std::cout << "Manual failover triggered for node: " << node_id
...
bool FailoverManager::force_leader_election() {
  ...
  if (current_state_ != FailoverState::NORMAL) {
    std::cout << "Cannot force leader election, failover in progress"
              << std::endl;
    return false;
  }
  ...
  if (!best_candidate.empty()) {
    std::cout << "Forcing leader election, selected candidate: "
...
void FailoverManager::enable_automatic_failover(bool enable) {
  ...
  std::cout << "Automatic failover " << (enable ? "enabled" : "disabled")
            << std::endl;
}
...
void FailoverManager::set_failover_callback(
    std::function<void(const FailoverEvent &)> callback) {
  failover_callback_ = callback;
}

void FailoverManager::set_health_change_callback(
...
void FailoverManager::trigger_node_failure_test(const std::string &node_id) {
  ...
  if (it != node_health_.end()) {
    ...
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
        end_time - start_time);
    std::cout << "Node failure test for " << node_id << " completed in "
              << duration.count()
              << "ms. Success: " << (failover_success ? "true" : "false")
              << std::endl;
  }
}

void FailoverManager::trigger_network_partition_test(
  ...
  for (const std::string &node_id : isolated_nodes) {
    ...
    if (it != node_health_.end()) {
      ...
      it->second.is_responsive = false;
      it->second.network_latency_ms =
          10000.0; // Very high latency indicates partition
...
bool FailoverManager::run_failover_test() {
  ...
  if (test_result) {
    ...
    bool leader_changed =
        (new_leader != original_leader && !new_leader.empty());

    std::cout << "Failover test " << (leader_changed ? "PASSED" : "FAILED")
              << std::endl;
    std::cout << "Original leader: " << original_leader
              << ", New leader: " << new_leader << std::endl;
  ...
  while (running_.load()) {
    ...
    for (const auto &node_id : nodes_to_check) {
      ...
      if (should_trigger_failover(node_id, trigger)) {
        std::cout << "Triggering automatic failover for node: " << node_id
                  << std::endl;
...
void FailoverManager::recovery_loop() {
  while (running_.load()) {
    ...
    for (const auto &node_id : failed_nodes) {
      ...
      if (last_recovery_attempts.find(node_id) ==
              last_recovery_attempts.end() ||
          now - last_recovery_attempts[node_id] > std::chrono::minutes(5)) {

        std::cout << "Attempting automatic recovery for failed node: "
                  << node_id << std::endl;
        bool recovery_success = attempt_node_recovery(node_id);
        last_recovery_attempts[node_id] = now;

        if (recovery_success) {
          std::cout << "Automatic recovery successful for node: " << node_id
                    << std::endl;
        }
      }
    }
...
bool FailoverManager::is_node_failed(const std::string &node_id) {
  ...
  bool high_resource_usage =
      (health.cpu_usage > failover_config_.cpu_threshold ||
       health.memory_usage > failover_config_.memory_threshold);
  bool high_latency = (health.network_latency_ms >
                       failover_config_.network_latency_threshold_ms);
  bool consecutive_failures =
      (failure_count >= failover_config_.max_consecutive_failures);

  return unresponsive || consecutive_failures ||
         (high_resource_usage && high_latency);
}

bool FailoverManager::should_trigger_failover(const std::string &node_id,
                                              FailoverTrigger &trigger) {
  if (is_failover_cooldown_active()) {
    return false;
  }
...
bool FailoverManager::execute_failover(const std::string &failed_node,
                                       FailoverTrigger trigger) {
  auto start_time = std::chrono::steady_clock::now();
  ...
  if (action_handler_) {
    if (failed_node == current_leader_) {
      ...
      promotion_success =
          action_handler_->promote_node_to_leader(replacement_node);
    ...
    traffic_switch_success =
        action_handler_->redirect_traffic(failed_node, replacement_node);
  }
  ...
  if (overall_success) {
    ...
    std::cout << "Failover completed successfully. New leader: "
              << replacement_node << std::endl;
  } else {
    std::lock_guard<std::mutex> lock(state_mutex_);
    current_state_ = FailoverState::NORMAL;
...

  {
    ...
    stats_.avg_failover_time_ms =
        (stats_.avg_failover_time_ms * (stats_.total_failovers - 1) +
         event.duration_ms) /
        stats_.total_failovers;
  }
...
std::string
FailoverManager::select_replacement_node(const std::string &failed_node) {
  std::lock_guard<std::mutex> lock(nodes_mutex_);
  ...
  for (const auto &pair : node_health_) {
    ...
    if (node_id == failed_node || !health.is_responsive ||
        !health.is_available) {
      continue;
    }
...
bool FailoverManager::validate_cluster_state() {
  ...
  double health_ratio =
      total_nodes > 0 ? static_cast<double>(healthy_nodes) / total_nodes : 0.0;
...
FailoverTrigger string_to_failover_trigger(const std::string &trigger) {
  if (trigger == "NODE_UNRESPONSIVE")
    return FailoverTrigger::NODE_UNRESPONSIVE;
  if (trigger == "NETWORK_PARTITION")
    return FailoverTrigger::NETWORK_PARTITION;
  if (trigger == "HEALTH_CHECK_FAILED")
    return FailoverTrigger::HEALTH_CHECK_FAILED;
  if (trigger == "MANUAL_FAILOVER")
    return FailoverTrigger::MANUAL_FAILOVER;
  if (trigger == "LOAD_THRESHOLD_EXCEEDED")
    return FailoverTrigger::LOAD_THRESHOLD_EXCEEDED;
...
FailoverState string_to_failover_state(const std::string &state) {
  ...
  if (state == "DETECTING_FAILURE")
    return FailoverState::DETECTING_FAILURE;
  if (state == "ELECTING_REPLACEMENT")
    return FailoverState::ELECTING_REPLACEMENT;
  if (state == "SWITCHING_TRAFFIC")
    return FailoverState::SWITCHING_TRAFFIC;
  if (state == "RECOVERY_IN_PROGRESS")
    return FailoverState::RECOVERY_IN_PROGRESS;
  if (state == "FAILED_OVER")
    return FailoverState::FAILED_OVER;
...
double calculate_node_score(const NodeHealth &health) {
  ...
  double latency_score =
      std::max(0.0, 100.0 - (health.network_latency_ms / 10.0));
  double error_score = std::max(
      0.0, 100.0 - std::min(100.0, static_cast<double>(health.error_count)));
...
std::vector<uint8_t> serialize_failover_event(const FailoverEvent &event) {
  ...
  serialized.insert(serialized.end(), event.event_id.begin(),
                    event.event_id.end());
  ...
  serialized.insert(serialized.end(), event.failed_node_id.begin(),
                    event.failed_node_id.end());

  uint32_t replacement_len = event.replacement_node_id.size();
  for (int i = 0; i < 4; i++) {
    serialized.push_back((replacement_len >> (i * 8)) & 0xFF);
  }
  serialized.insert(serialized.end(), event.replacement_node_id.begin(),
                    event.replacement_node_id.end());
...
FailoverEvent deserialize_failover_event(const std::vector<uint8_t> &data) {
  ...
  if (offset + failed_len > data.size())
    return event;
  event.failed_node_id.assign(data.begin() + offset,
                              data.begin() + offset + failed_len);
  offset += failed_len;
  ...
  if (offset + replacement_len > data.size())
    return event;
  event.replacement_node_id.assign(data.begin() + offset,
                                   data.begin() + offset + replacement_len);
  ...
  if (offset + reason_len > data.size())
    return event;
  event.reason.assign(data.begin() + offset,
                      data.begin() + offset + reason_len);
  offset += reason_len;
...
bool FailoverManager::handle_node_failure(const std::string &node_id) {
  ...
  try {
    ...
    std::cerr << "Failed to handle node failure for " << node_id << ": "
              << e.what() << std::endl;
    return false;
  }
}
...
bool FailoverManager::handle_network_partition(const std::string &node_id) {
  try {
    ...
    std::cout << "Handling network partition for node: " << node_id
              << std::endl;
    ...
    for (int attempt = 0; attempt < 3; ++attempt) {
      ...
      if (check_node_health(node_id)) {
        std::cout << "Node " << node_id << " recovered from partition"
                  << std::endl;
        return true;
      }
    }
    ...
    std::cerr << "Failed to handle network partition for " << node_id << ": "
              << e.what() << std::endl;
    return false;
  }
}

void FailoverManager::update_cluster_topology_after_partition(
    const std::vector<std::string> &isolated_nodes) {
  std::lock_guard<std::mutex> lock(nodes_mutex_);
  ...
  for (const auto &pair : node_health_) {
    if (std::find(isolated_nodes.begin(), isolated_nodes.end(), pair.first) ==
        isolated_nodes.end()) {
