...
SlackAlertChannel::SlackAlertChannel(const std::string &webhook_url,
                                     bool enabled)
...
std::string
SlackAlertChannel::format_slack_message(const LogEntry &entry) const {
  ...
  auto time_t = std::chrono::system_clock::to_time_t(entry.timestamp);
  msg << "*Time:* "
      << std::put_time(std::gmtime(&time_t), "%Y-%m-%d %H:%M:%S UTC") << "\n";
...
EmailAlertChannel::EmailAlertChannel(const std::string &smtp_server, int port,
                                     ...
                                     const std::string &to_email, bool enabled)
    : smtp_server_(smtp_server), port_(port), username_(username),
      password_(password), from_email_(from_email), to_email_(to_email),
      enabled_(enabled) {}
...
std::string
EmailAlertChannel::format_email_subject(const LogEntry &entry) const {
...
PrometheusAlertChannel::PrometheusAlertChannel(bool enabled)
    : enabled_(enabled) {}
...
std::unique_ptr<IAlertChannel>
AlertChannelFactory::create_console_channel(bool enabled) {
  return std::make_unique<ConsoleAlertChannel>(enabled);
}

std::unique_ptr<IAlertChannel>
AlertChannelFactory::create_slack_channel(const std::string &webhook_url,
                                          bool enabled) {
...
std::unique_ptr<IAlertChannel> AlertChannelFactory::create_email_channel(
    const std::string &smtp_server, int port, const std::string &username,
    const std::string &password, const std::string &from_email,
    const std::string &to_email, bool enabled) {
...
std::unique_ptr<IAlertChannel>
AlertChannelFactory::create_file_channel(const std::string &file_path,
                                         bool enabled) {
  return std::make_unique<FileAlertChannel>(file_path, enabled);
}

std::unique_ptr<IAlertChannel>
AlertChannelFactory::create_prometheus_channel(bool enabled) {
