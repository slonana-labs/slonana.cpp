...
Tower::Tower(uint64_t root_slot) : root_slot_(root_slot), last_vote_slot_(root_slot) {
    tower_slots_.reserve(MAX_TOWER_HEIGHT);
}
...
bool Tower::can_vote_on_slot(uint64_t slot) const {
    ...
    for (const auto& tower_slot : tower_slots_) {
        uint64_t lockout_period = calculate_lockout_period(tower_slot.confirmation_count);
...
void Tower::record_vote(uint64_t slot) {
    ...
    for (const auto& tower_slot : tower_slots_) {
        uint64_t lockout_period = calculate_lockout_period(tower_slot.confirmation_count);
        if (slot > tower_slot.slot && slot <= tower_slot.slot + lockout_period) {
            throw std::runtime_error("Cannot vote on slot " + std::to_string(slot) + " - locked out by slot " + std::to_string(tower_slot.slot));
        }
    }
...
bool Tower::is_valid() const {
    ...
    for (size_t i = 0; i < tower_slots_.size(); ++i) {
        for (size_t j = i + 1; j < tower_slots_.size(); ++j) {
            uint64_t lockout_period = calculate_lockout_period(tower_slots_[i].confirmation_count);
            // A later slot conflicts if it's within the lockout period of an earlier slot
...
void Tower::reset_to_root(uint64_t new_root_slot) {
    ...
    std::cout << "🔄 Tower BFT: Reset to root slot " << new_root_slot << std::endl;
}
...
void Tower::update_confirmation_count(uint64_t slot, uint64_t new_count) {
    ...
    for (auto& tower_slot : tower_slots_) {
        if (tower_slot.slot == slot) {
            ...
            std::cout << "📈 Tower BFT: Updated confirmation count for slot " 
                      << slot << " to " << new_count << std::endl;
            break;
        }
    }
}
...
std::vector<uint8_t> Tower::serialize() const {
    ...
    for (const auto& tower_slot : tower_slots_) {
        // Slot number (8 bytes)
        for (int i = 0; i < 8; ++i) {
            result.push_back(static_cast<uint8_t>((tower_slot.slot >> (i * 8)) & 0xFF));
        }
        
        // Confirmation count (8 bytes)
        for (int i = 0; i < 8; ++i) {
            result.push_back(static_cast<uint8_t>((tower_slot.confirmation_count >> (i * 8)) & 0xFF));
        }
    }
...
bool TowerBftManager::process_slot(uint64_t slot, uint64_t parent_slot) {
    ...
    bool can_vote = tower_->can_vote_on_slot(slot) && vote_state_->is_valid_vote(slot);
...
bool TowerBftManager::cast_vote(uint64_t slot) {
    ...
    try {
        ...
        vote_state_->process_vote(slot, 0); // timestamp not used in current implementation
        
        return true;
    } catch (const std::exception& e) {
        std::cerr << "Error casting vote on slot " << slot << ": " << e.what() << std::endl;
        return false;
    }
}

void TowerBftManager::set_vote_callback(std::function<void(uint64_t slot, bool can_vote)> callback) {
    std::lock_guard<std::mutex> lock(manager_mutex_);
...
const Tower& TowerBftManager::get_tower() const {
    return *tower_;
}
