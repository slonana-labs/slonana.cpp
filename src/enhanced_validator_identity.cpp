...
EnhancedValidatorIdentity::EnhancedValidatorIdentity(
    const std::string &storage_path, bool enable_secure_management)
    : use_secure_management_(enable_secure_management),
      legacy_identity_loaded_(false) {

  if (use_secure_management_) {
    secure_identity_ =
        security::SecureKeyManagerFactory::create_secure_identity(storage_path);
  }
}

EnhancedValidatorIdentity::~EnhancedValidatorIdentity() { shutdown(); }
...
common::Result<bool> EnhancedValidatorIdentity::initialize() {
  if (use_secure_management_ && secure_identity_) {
    ...
    if (!init_result.is_ok()) {
      ...
      std::cerr << "🚨 CRITICAL SECURITY WARNING: Secure key management "
    ...
    std::cout << "✅ Secure key lifecycle management initialized with "
...
EnhancedValidatorIdentity::load_validator_identity(
...
std::vector<uint8_t> EnhancedValidatorIdentity::generate_validator_identity() {
  if (use_secure_management_ && secure_identity_) {
    ...
    if (create_result.is_ok()) {
      std::cout << "✅ Generated secure validator identity: "
                << create_result.value() << "\
";
      ...
      std::cout << "⚠️  Secure identity generation failed: "
                << create_result.error() << "\
";
    }
  }
...
EnhancedValidatorIdentity::create_secure_identity() {
  if (!use_secure_management_ || !secure_identity_) {
...
common::Result<std::string>
EnhancedValidatorIdentity::rotate_identity_if_needed() {
  if (!use_secure_management_ || !secure_identity_) {
    return common::Result<std::string>("Not enabled");
  }
...
common::Result<common::PublicKey>
EnhancedValidatorIdentity::get_current_public_key() {
  if (use_secure_management_ && secure_identity_) {
...
common::Result<security::SecureBuffer>
EnhancedValidatorIdentity::get_current_private_key() {
  if (use_secure_management_ && secure_identity_) {
    return secure_identity_->get_private_key();
  }

  return common::Result<security::SecureBuffer>(
      "Private key access requires secure management mode");
}

common::Result<bool> EnhancedValidatorIdentity::export_to_legacy_format(
...
common::Result<EnhancedValidatorIdentity::IdentityInfo>
EnhancedValidatorIdentity::get_identity_info() {
  IdentityInfo info = {};
  ...
  if (use_secure_management_ && secure_identity_) {
    ...
    if (status_result.is_ok()) {
      ...
      return common::Result<IdentityInfo>(
          "Failed to get secure identity status: " + status_result.error());
    }
  } else {
