...
int GenesisCliTool::create_genesis_command(int argc, char *argv[]) {
  if (argc < 3) {
    ...
    std::cout << "  --network-type TYPE        Network type (mainnet, testnet, "
                 "devnet)"
              << std::endl;
    std::cout
        << "  --initial-validators FILE  Path to initial validators JSON file"
        << std::endl;
    std::cout << "  --initial-supply AMOUNT    Initial token supply"
              << std::endl;
    std::cout << "  --token-symbol SYMBOL       Network token symbol (e.g., "
                 "SOL, SLON)"
              << std::endl;
    std::cout << "  --base-unit-name NAME       Base unit name (e.g., "
                 "lamports, aldrins)"
              << std::endl;
    std::cout << "  --inflation-rate RATE      Annual inflation rate (0.0-1.0)"
              << std::endl;
    std::cout << "  --epoch-length LENGTH      Epoch length in slots"
              << std::endl;
    std::cout
        << "  --output FILE              Output genesis configuration file"
        << std::endl;
    return 1;
  }
  ...
  if (!parse_result.is_ok()) {
    std::cerr << "Error parsing arguments: " << parse_result.error()
              << std::endl;
    return 1;
  }
  ...
  if (!validation_result.is_ok()) {
    std::cerr << "Invalid genesis configuration: " << validation_result.error()
              << std::endl;
    return 1;
  }
  ...
  if (!save_result.is_ok()) {
    std::cerr << "Failed to save genesis configuration: " << save_result.error()
              << std::endl;
    return 1;
  }
  ...
  std::cout << "Network: " << config.network_id
            << " (Chain ID: " << config.chain_id << ")" << std::endl;
...
int GenesisCliTool::verify_genesis_command(int argc, char *argv[]) {
  if (argc < 3) {
    std::cout << "Usage: slonana-genesis verify CONFIG_FILE [EXPECTED_HASH]"
              << std::endl;
    return 1;
  }
  ...
  if (!load_result.is_ok()) {
    std::cerr << "Failed to load genesis configuration: " << load_result.error()
              << std::endl;
    return 1;
  }
...
int GenesisCliTool::info_genesis_command(int argc, char *argv[]) {
  ...
  std::cout << "Network Type: "
            << GenesisManager::network_type_to_string(config.network_type)
            << std::endl;
  ...
  std::cout << "Total Supply: " << config.economics.total_supply << " "
            << config.economics.token_symbol << std::endl;
  std::cout << "Initial Inflation Rate: "
            << (config.economics.initial_inflation_rate * 100) << "%"
            << std::endl;
  std::cout << "Foundation Rate: " << (config.economics.foundation_rate * 100)
            << "%" << std::endl;
  std::cout << "Foundation Allocation: "
            << (config.economics.foundation_allocation * 100) << "%"
            << std::endl;
  std::cout << "Team Allocation: " << (config.economics.team_allocation * 100)
            << "%" << std::endl;
  std::cout << "Community Allocation: "
            << (config.economics.community_allocation * 100) << "%"
            << std::endl;
  std::cout << "Validator Allocation: "
            << (config.economics.validator_allocation * 100) << "%"
            << std::endl;
  std::cout << "Min Validator Stake: " << config.economics.min_validator_stake
            << " " << config.economics.base_unit_name << std::endl;
  std::cout << "Min Delegation: " << config.economics.min_delegation << " "
            << config.economics.base_unit_name << std::endl;
  ...
  std::cout << "Target Tick Duration: " << config.target_tick_duration_us
  ...
  for (size_t i = 0; i < config.genesis_validators.size(); ++i) {
    ...
    std::cout << "    Stake: " << validator.stake_amount << " "
              << config.economics.base_unit_name << std::endl;
    std::cout << "    Commission: " << (validator.commission_rate / 100.0)
              << "%" << std::endl;
...
void GenesisCliTool::print_usage() {
  ...
  std::cout << "  create-network    Create a new genesis configuration"
            << std::endl;
  std::cout << "  verify           Verify a genesis configuration file"
            << std::endl;
  std::cout << "  info             Display genesis configuration information"
            << std::endl;
  std::cout << std::endl;
  std::cout << "Examples:" << std::endl;
  std::cout << "  # Create mainnet genesis" << std::endl;
  std::cout << "  slonana-genesis create-network --network-type mainnet \\"
            << std::endl;
...
common::Result<GenesisConfig>
GenesisCliTool::parse_creation_args(int argc, char *argv[]) {
  ...
  if (!validators_file.empty()) {
    ...
    if (!validators_result.is_ok()) {
      return common::Result<GenesisConfig>("Failed to parse validators: " +
...
common::Result<std::vector<GenesisValidator>>
GenesisCliTool::parse_validator_list(const std::string &filepath) {
  std::ifstream file(filepath);
  if (!file.is_open()) {
    return common::Result<std::vector<GenesisValidator>>(
        "Failed to open validators file: " + filepath);
  }
  ...
  if (content.front() != '[' || content.back() != ']') {
    return common::Result<std::vector<GenesisValidator>>(
        "Invalid validator JSON format: must be an array");
  }

  // Helper function to parse individual validator objects
  auto parse_validator =
      [](const std::string &validator_json) -> std::optional<GenesisValidator> {
    ...
    auto extract_hex_field =
        [&validator_json](const std::string &field_name,
                          size_t expected_bytes) -> std::vector<uint8_t> {
    ...
    auto extract_string_field =
        [&validator_json](const std::string &field_name) -> std::string {
      std::string search = "\"" + field_name + "\":\"";
      size_t pos = validator_json.find(search);
      if (pos == std::string::npos)
        return "";
    ...
    auto extract_numeric_field =
        [&validator_json](const std::string &field_name) -> uint64_t {
      std::string search = "\"" + field_name + "\":";
      size_t pos = validator_json.find(search);
      if (pos == std::string::npos)
        return 0;
      ...
      size_t start = pos + search.length();
      while (start < validator_json.length() &&
             std::isspace(validator_json[start]))
        start++;

      size_t end = start;
      while (
          end < validator_json.length() &&
    ...
    if (validator.identity.empty() || validator.vote_account.empty() ||
        validator.stake_account.empty()) {
      return std::nullopt;
    }
  ...
  std::string array_content =
      content.substr(1, content.length() - 2); // Remove [ and ]
  ...
  for (size_t i = 0; i < array_content.length(); ++i) {
    if (array_content[i] == '{') {
      ...
      if (brace_count == 0) {
        ...
        std::string validator_json =
            array_content.substr(current_start, i - current_start + 1);
  ...
  if (validators.empty()) {
    ...
    for (size_t i = 0; i < 32; ++i) {
      default_validator.identity[i] =
          static_cast<uint8_t>((now >> (i % 64)) ^ (i * 0x67));
      default_validator.vote_account[i] =
          static_cast<uint8_t>((now >> ((i + 11) % 64)) ^ (i * 0x89));
      default_validator.stake_account[i] =
          static_cast<uint8_t>((now >> ((i + 23) % 64)) ^ (i * 0xAB));
    }
