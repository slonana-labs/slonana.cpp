...
GenesisConfig GenesisManager::create_network_config(NetworkType network_type) {
  ...
  case NetworkType::MAINNET:
    ...
    config.bootstrap_entrypoints = {"mainnet-seed1.slonana.org:8001",
  ...
  case NetworkType::TESTNET:
    ...
    config.bootstrap_entrypoints = {"testnet-seed1.slonana.org:8001",
  ...
  default:
    ...
    config.bootstrap_entrypoints = {"devnet-seed1.slonana.org:8001"};
    break;
  }
...
MainnetConfig GenesisManager::create_mainnet_config() {
  ...
  mainnet_config.network_discovery.dns_seeds = {"mainnet-seeds.slonana.org",
                                                "seeds.slonana.org"};
...
GenesisManager::load_genesis_config(const std::string &filepath) {
  std::ifstream file(filepath);
  if (!file.is_open()) {
    return common::Result<GenesisConfig>(
        "Failed to open genesis config file: " + filepath);
  }
...
common::Result<bool>
GenesisManager::save_genesis_config(const GenesisConfig &config,
                                    const std::string &filepath) {
  auto validation_result = validate_genesis_config(config);
  if (!validation_result.is_ok()) {
    return common::Result<bool>("Invalid genesis configuration: " +
...
common::Result<ledger::Block>
GenesisManager::create_genesis_block(const GenesisConfig &config) {
  auto validation_result = validate_genesis_config(config);
  if (!validation_result.is_ok()) {
    return common::Result<ledger::Block>("Invalid genesis configuration: " +
                                         validation_result.error());
  }
...
common::Result<bool>
GenesisManager::validate_genesis_config(const GenesisConfig &config) {
...
common::Hash GenesisManager::compute_genesis_hash(const GenesisConfig &config) {
  ...
  if (EVP_DigestUpdate(ctx, serialized_config.data(),
                       serialized_config.size()) != 1) {
    EVP_MD_CTX_free(ctx);
...
std::vector<std::pair<common::PublicKey, uint64_t>>
GenesisManager::create_genesis_accounts(const GenesisConfig &config) {
...
std::vector<NetworkEntrypoint> GenesisManager::get_mainnet_entrypoints() {
  return {
      {"mainnet-seed1.slonana.org", 8001, true, "us-east",
       "Primary US East seed"},
      {"mainnet-seed2.slonana.org", 8001, true, "eu-west",
       "Primary EU West seed"},
      {"mainnet-seed3.slonana.org", 8001, true, "asia-pac",
       "Primary Asia Pacific seed"},
      {"seed1.slonana.org", 8001, false, "us-west", "Secondary US West seed"},
      {"seed2.slonana.org", 8001, false, "eu-central",
       "Secondary EU Central seed"}};
}

std::vector<NetworkEntrypoint> GenesisManager::get_testnet_entrypoints() {
  return {{"testnet-seed1.slonana.org", 8001, true, "us-east",
           "Primary testnet seed"},
          {"testnet-seed2.slonana.org", 8001, true, "eu-west",
           "Secondary testnet seed"}};
}
...
std::string GenesisManager::serialize_to_json(const GenesisConfig &config) {
  ...
  json << "\"initial_inflation_rate\":"
       << config.economics.initial_inflation_rate << ",";
  ...
  json << "\"vote_timeout_slashing\":" << config.economics.vote_timeout_slashing
       << ",";
  json << "\"equivocation_slashing\":"
       << config.economics.equivocation_slashing;
  json << "},";
...
common::Result<GenesisConfig>
GenesisManager::deserialize_from_json(const std::string &json) {
  ...
  auto extract_string_value =
      [&json](const std::string &key) -> std::optional<std::string> {
  ...
  auto extract_numeric_value =
      [&json](const std::string &key) -> std::optional<int64_t> {
    std::string search_key = "\"" + key + "\":";
    size_t pos = json.find(search_key);
    if (pos == std::string::npos)
      return std::nullopt;
  ...
  if (network_id) {
    if (network_id->length() > 64) {
      return common::Result<GenesisConfig>(
          "Network ID too long (max 64 characters)");
    }
  ...
  if (token_symbol) {
    if (token_symbol->length() > 10) {
      return common::Result<GenesisConfig>(
          "Token symbol too long (max 10 characters)");
    }
  ...
  if (base_unit_name) {
    if (base_unit_name->length() > 20) {
      return common::Result<GenesisConfig>(
          "Base unit name too long (max 20 characters)");
    }
  ...
  if (initial_inflation_rate && *initial_inflation_rate >= 0 &&
      *initial_inflation_rate <= 100) {
    config.economics.initial_inflation_rate =
        static_cast<double>(*initial_inflation_rate) / 100.0;
  } else {
  ...
  if (min_validator_stake && *min_validator_stake > 0) {
    config.economics.min_validator_stake =
        static_cast<uint64_t>(*min_validator_stake);
  } else {
...
EconomicConfig
GenesisManager::create_default_economics(NetworkType network_type) {
...
bool GenesisManager::validate_economics(const EconomicConfig &economics) {
  if (economics.total_supply == 0)
    return false;
  if (economics.initial_inflation_rate < 0.0 ||
      economics.initial_inflation_rate > 1.0)
    return false;
  if (economics.foundation_rate < 0.0 || economics.foundation_rate > 1.0)
    return false;
  if (economics.min_validator_stake == 0)
    return false;
  if (economics.min_delegation == 0)
    return false;
  if (economics.vote_timeout_slashing < 0.0 ||
      economics.vote_timeout_slashing > 1.0)
    return false;
  if (economics.equivocation_slashing < 0.0 ||
      economics.equivocation_slashing > 1.0)
    return false;
