...
BuddyAllocator::BuddyAllocator(size_t size) 
    ...
    std::cout << "Buddy allocator initialized with " << total_size_ << " bytes" << std::endl;
}
...
size_t BuddyAllocator::find_free_block(int order) {
    ...
    // Split larger block
    for (int larger_order = order + 1; larger_order <= max_order_; ++larger_order) {
...
SlabAllocator::SlabAllocator(size_t object_size, size_t slab_size)
    ...
    std::cout << "Slab allocator initialized: " << object_size_ << " byte objects, "
              << objects_per_slab_ << " objects per slab" << std::endl;
}
...
SlabStats SlabAllocator::get_stats() const {
    ...
    stats.current_objects = allocated_objects_.load() - deallocated_objects_.load();
    stats.total_memory_used = slabs_.size() * slab_size_;
    stats.utilization = static_cast<double>(stats.current_objects) / (slabs_.size() * objects_per_slab_);
...
void SlabAllocator::allocate_new_slab() {
    ...
    for (size_t i = 0; i < objects_per_slab_ - 1; ++i) {
        *reinterpret_cast<void**>(ptr + i * object_size_) = ptr + (i + 1) * object_size_;
    }
    *reinterpret_cast<void**>(ptr + (objects_per_slab_ - 1) * object_size_) = free_list_;
    
    free_list_ = memory;
    
    std::cout << "Allocated new slab: " << objects_per_slab_ << " objects" << std::endl;
}
...
RegionalAllocator::RegionalAllocator(size_t region_size) 
    ...
    allocate_new_region();
    std::cout << "Regional allocator initialized with " << region_size_ << " byte regions" << std::endl;
}
...
void RegionalAllocator::allocate_new_region() {
    ...
    std::cout << "Allocated new region: " << regions_.size() << " total regions" << std::endl;
}

// GarbageCollector implementation
GarbageCollector::GarbageCollector() : running_(false), collection_threshold_(1024 * 1024) {
...
void GarbageCollector::register_object(void* ptr, size_t size, std::function<void()> destructor) {
...
GCStats GarbageCollector::get_stats() const {
    ...
    stats.avg_collection_time_us = stats.collections_performed > 0 ? 
        stats.total_collection_time_us / stats.collections_performed : 0;
...
MemoryManager::MemoryManager(const MemoryConfig& config) 
    : config_(config), buddy_allocator_(nullptr), regional_allocator_(nullptr), gc_(nullptr) {
...
void* MemoryManager::allocate(size_t size, MemoryType type) {
    ...
    if (ptr && gc_) {
        ...
        gc_->register_object(ptr, size, [this, ptr, type]() {
            deallocate(ptr, type);
        });
    }
...
void MemoryManager::initialize_allocators() {
    if (config_.enable_buddy_allocator) {
        buddy_allocator_ = std::make_unique<BuddyAllocator>(config_.buddy_pool_size);
    }
    
    if (config_.enable_regional_allocator) {
        regional_allocator_ = std::make_unique<RegionalAllocator>(config_.regional_size);
    }
    ...
    if (config_.enable_slab_allocators) {
        for (size_t size : {16, 32, 64, 128, 256, 512, 1024, 2048, 4096}) {
            slab_allocators_[size] = std::make_unique<SlabAllocator>(size, config_.slab_size);
        }
    }
}
