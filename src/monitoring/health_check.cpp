...
NetworkHealthCheck::NetworkHealthCheck(const std::vector<std::string>& test_endpoints)
...
HealthCheckResult NetworkHealthCheck::check() {
    ...
    try {
        if (test_endpoints_.empty()) {
            ...
            double health_ratio = static_cast<double>(healthy_endpoints) / test_endpoints_.size();
            ...
            result.metadata["endpoints_total"] = std::to_string(test_endpoints_.size());
...
HealthCheckResult DiskSpaceHealthCheck::check() {
    ...
    try {
        if (!std::filesystem::exists(path_)) {
            ...
            result.metadata["total_space_bytes"] = std::to_string(space_info.capacity);
            ...
            result.metadata["critical_threshold"] = std::to_string(critical_threshold_);
        }
    } catch (const std::exception& e) {
        result.status = HealthStatus::UNHEALTHY;
...
MemoryHealthCheck::MemoryHealthCheck(double warning_threshold, double critical_threshold)
    : warning_threshold_(warning_threshold)
    , critical_threshold_(critical_threshold) {
}
    ...
    try {
        ...
        if (!meminfo.is_open()) {
            ...
            if (total_memory > 0 && available_memory > 0) {
                double used_ratio = static_cast<double>(total_memory - available_memory) / total_memory;
                ...
                result.metadata["available_memory_bytes"] = std::to_string(available_memory);
                result.metadata["used_ratio"] = std::to_string(used_ratio);
                result.metadata["warning_threshold"] = std::to_string(warning_threshold_);
                result.metadata["critical_threshold"] = std::to_string(critical_threshold_);
            } else {
...
void HealthMonitor::start_monitoring() {
    ...
    monitoring_thread_ = std::thread(&HealthMonitor::monitoring_thread_func, this);
}
...
SystemHealth HealthMonitor::check_all() {
    ...
    for (const auto& [name, health_check] : health_checks_) {
        try {
            ...
            error_result.message = "Health check threw exception: " + std::string(e.what());
            error_result.timestamp = std::chrono::system_clock::now();
            system_health.component_results.push_back(error_result);
        }
    }
    
    system_health.overall_status = calculate_overall_status(system_health.component_results);
    ...
    summary << "System health: " << health_status_to_string(system_health.overall_status)
...
std::optional<HealthCheckResult> HealthMonitor::check_component(const std::string& component_name) {
    std::lock_guard<std::mutex> lock(health_checks_mutex_);
...
std::vector<HealthCheckResult> HealthMonitor::get_component_history(
...
HealthStatus HealthMonitor::calculate_overall_status(const std::vector<HealthCheckResult>& results) {
    if (results.empty()) {
        return HealthStatus::UNKNOWN;
    }
