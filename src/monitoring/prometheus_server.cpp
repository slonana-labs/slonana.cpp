...
PrometheusServer::PrometheusServer(uint16_t port, std::shared_ptr<IMetricsRegistry> registry)
    : port_(port), registry_(registry), running_(false), exporter_(std::make_unique<PrometheusExporter>()) {
...
bool PrometheusServer::start() {
    ...
    if (bind(server_socket_, (struct sockaddr*)&address, sizeof(address)) < 0) {
        std::cerr << "Failed to bind socket for Prometheus server on port " << port_ << std::endl;
    ...
    if (listen(server_socket_, 10) < 0) {
        std::cerr << "Failed to listen on socket for Prometheus server" << std::endl;
        close(server_socket_);
        return false;
    }
...
void PrometheusServer::server_loop() {
    while (running_.load()) {
        ...
        std::thread client_thread(&PrometheusServer::handle_client, this, client_socket);
...
void PrometheusServer::handle_client(int client_socket) {
    ...
    } else if (request.find("GET /health") == 0 || request.find("GET /health ") != std::string::npos) {
