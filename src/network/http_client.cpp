...
HttpResponse HttpClient::solana_rpc_call(const std::string& rpc_url, const std::string& method, 
...
bool HttpClient::download_file(const std::string& url, const std::string& local_path, 
    ...
    if (!response.success) {
        std::cerr << "Failed to download file: " << response.error_message << std::endl;
        return false;
    }
    ...
    std::cout << "Download completed: " << response.body.size() << " bytes" << std::endl;
    return true;
}

HttpResponse HttpClient::execute_request(const std::string& url, const std::string& method, 
...
std::string HttpClient::build_header_string(const std::map<std::string, std::string>& headers) {
