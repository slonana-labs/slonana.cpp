...
TurbineTree::TurbineTree(const TurbineNode &self_node, uint32_t fanout)
    : self_node_(self_node), fanout_(fanout),
      max_retransmits_(MAX_RETRANSMIT_PEERS) {}
...
std::vector<size_t>
TurbineTree::calculate_children_indices(size_t node_index,
...
std::vector<size_t>
TurbineTree::calculate_retransmit_indices(size_t node_index) const {
  ...
  for (size_t i = 0;
       i < nodes_.size() && retransmit_peers.size() < MAX_RETRANSMIT_PEERS;
       ++i) {
    if (i == node_index)
      continue; // Skip self
...
TurbineTree::get_children(const TurbineNode &node) const {
  std::lock_guard<std::mutex> lock(tree_mutex_);
...
std::vector<TurbineNode>
TurbineTree::get_retransmit_peers(const TurbineNode &node) const {
  std::lock_guard<std::mutex> lock(tree_mutex_);
...
std::optional<TurbineNode>
TurbineTree::get_parent(const TurbineNode &node) const {
  std::lock_guard<std::mutex> lock(tree_mutex_);
...
TurbineTree::TreeStats TurbineTree::get_stats() const {
  ...
  stats.tree_height = static_cast<size_t>(
      std::ceil(std::log(nodes_.size()) / std::log(fanout_)));
  ...
  stats.average_children_per_node =
      nodes_.size() > 0 ? static_cast<double>(total_children) / nodes_.size()
                        : 0.0;

  return stats;
}

bool TurbineTree::update_node_stake(const TurbineNode &node,
                                    uint32_t new_stake) {
...
std::vector<uint8_t> TurbineTree::serialize() const {
  ...
  for (const auto &node : nodes_) {
    ...
    for (int i = 0; i < 4; ++i) {
      result.push_back(
          static_cast<uint8_t>((node.stake_weight >> (i * 8)) & 0xFF));
    }
  }
...
bool TurbineTree::deserialize(const std::vector<uint8_t> &data) {
  ...
  for (uint32_t i = 0; i < node_count; ++i) {
    ...
    // Address
    node.address.assign(data.begin() + offset,
                        data.begin() + offset + addr_len);
    offset += addr_len;
...
size_t estimate_tree_height(size_t network_size, uint32_t fanout) {
  ...
  return static_cast<size_t>(
      std::ceil(std::log(network_size) / std::log(fanout)));
}
...
uint64_t hash_node(const TurbineNode &node, uint64_t seed) {
  ...
  std::string node_str = node.address + ":" + std::to_string(node.port) + "_" +
                         std::to_string(seed);
  for (auto byte : node.pubkey) {
