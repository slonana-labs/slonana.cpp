...
AdvancedProfiler::MeasurementScope::MeasurementScope(
    AdvancedProfiler &profiler, const std::string &operation_name)
...
AdvancedProfiler::MeasurementScope::~MeasurementScope() {
  ...
  // Record measurement
  profiler_.record_measurement(operation_name_,
...
uint64_t
AdvancedProfiler::start_measurement(const std::string &operation_name) {
...
void AdvancedProfiler::end_measurement(uint64_t measurement_id) {
  ...
  if (it != active_measurements_.end()) {
    ...
    double duration_ns =
        cycles_to_nanoseconds(point.end_cycles - point.start_cycles);
    ...
    record_measurement(point.name, duration_ns, point.cache_misses,
                       point.branch_misses);
  }
}

void AdvancedProfiler::record_measurement(const std::string &operation_name,
                                          double latency_ns,
  ...
  point.end_cycles =
      static_cast<uint64_t>(latency_ns / cycles_to_nanoseconds(1));
...
void AdvancedProfiler::set_monitoring_enabled(bool enabled) {
  ...
  if (enabled && !was_enabled) {
    ...
    monitoring_thread_ =
        std::make_unique<std::thread>(&AdvancedProfiler::monitoring_loop, this);
...
bool AdvancedProfiler::export_to_json(const std::string &filename) const {
  ...
  for (const auto &[name, stats] : all_stats) {
    ...
    file << "      \"mean_throughput_ops_per_sec\": "
...
bool AdvancedProfiler::export_to_csv(const std::string &filename) const {
  ...
  // CSV header
  file << "operation,sample_count,mean_latency_ns,p50_latency_ns,p95_latency_"
          "ns,p99_latency_ns,p999_latency_ns,throughput_ops_per_sec\n";

  auto all_stats = get_all_stats();
  for (const auto &[name, stats] : all_stats) {
    file << name << "," << stats.sample_count << "," << stats.mean_latency_ns
         << "," << stats.p50_latency_ns << "," << stats.p95_latency_ns << ","
         << stats.p99_latency_ns << "," << stats.p999_latency_ns << ","
         << stats.mean_throughput_ops_per_sec << "\n";
  }
...
#ifdef __linux__
  counters = {"cpu-cycles", "cache-misses", "branch-misses", "instructions",
              "cache-references"};
#endif
...
double AdvancedProfiler::cycles_to_nanoseconds(uint64_t cycles) {
  // Assume 3.0 GHz CPU for conversion (would be detected dynamically in real
  // implementation)
  const double cpu_freq_ghz = 3.0;
  return static_cast<double>(cycles) / cpu_freq_ghz;
}

AdvancedProfiler::PerformanceStats AdvancedProfiler::calculate_stats(
    const std::vector<MeasurementPoint> &measurements) const {
  ...
  if (!measurements.empty()) {
    ...
    stats.mean_cache_misses_per_op =
        static_cast<double>(total_cache_misses) / measurements.size();
    stats.mean_branch_misses_per_op =
        static_cast<double>(total_branch_misses) / measurements.size();
    stats.instructions_per_cycle =
        2.5; // Placeholder - would calculate from actual counters
  }
...
std::vector<double> AdvancedProfiler::calculate_percentiles(
    std::vector<double> values, const std::vector<double> &percentiles) {
  ...
  for (const auto &[name, stat] : stats) {
    ...
    if (stat.p95_latency_ns > config_.latency_alert_threshold_ms * 1e6) {
      std::cout << "ALERT: High latency detected for " << name
                << " - P95: " << (stat.p95_latency_ns / 1e6) << "ms"
                << std::endl;
    }

    // Check throughput alerts
    if (stat.mean_throughput_ops_per_sec <
        config_.throughput_alert_threshold_ops_per_sec) {
