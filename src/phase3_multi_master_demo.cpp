...
int main() {
    ...
    try {
        ...
        auto multi_master_manager = std::make_shared<MultiMasterManager>("node1", config);
        auto topology_manager = std::make_shared<NetworkTopologyManager>("node1", config);
        auto load_balancer = std::make_shared<DistributedLoadBalancer>("lb1", config);
        auto coordinator = std::make_shared<MultiMasterCoordinator>("coord1", config);
        ...
        std::cout << "✅ Multi-master components initialized successfully" << std::endl;
        ...
        std::cout << "\n🎯 Demonstrating Phase 3 Multi-Master Features:" << std::endl;
        ...
        if (response1.success) {
            std::cout << "   ✅ Request routed to: " << response1.selected_server 
                      << " (" << response1.server_address << ":" << response1.server_port << ")" << std::endl;
        }
        ...
        if (response2.success) {
            std::cout << "   ✅ Request routed to: " << response2.selected_server 
                      << " (" << response2.server_address << ":" << response2.server_port << ")" << std::endl;
        }
        ...
        coordinator->coordinate_master_promotion("node1", MasterRole::GLOBAL_MASTER);
        ...
        std::cout << "   • Total Masters: " << master_stats.total_masters << std::endl;
        std::cout << "   • Healthy Masters: " << master_stats.healthy_masters << std::endl;
        
        auto network_metrics = topology_manager->get_network_metrics();
        std::cout << "   • Network Nodes: " << network_metrics.total_nodes << std::endl;
        std::cout << "   • Active Nodes: " << network_metrics.active_nodes << std::endl;
        std::cout << "   • Network Partitions: " << network_metrics.partitions_count << std::endl;
        
        auto lb_stats = load_balancer->get_statistics();
        std::cout << "   • Load Balancer Requests: " << lb_stats.total_requests << std::endl;
        std::cout << "   • Successful Requests: " << lb_stats.successful_requests << std::endl;
        std::cout << "   • Active Backend Servers: " << lb_stats.active_backends << std::endl;
        
        auto coord_stats = coordinator->get_statistics();
        std::cout << "   • Coordination Events: " << coord_stats.total_coordination_events << std::endl;
        std::cout << "   • Successful Failovers: " << coord_stats.successful_failovers << std::endl;
        
        // 10. Run for a few seconds to show activity
        std::cout << "\n⏱️  Running multi-master network for 5 seconds..." << std::endl;
        ...
        std::cout << "\n🎉 Phase 3 Multi-Master Network Setup Demo Completed Successfully!" << std::endl;
        std::cout << "\n📋 Phase 3 Features Demonstrated:" << std::endl;
        std::cout << "   ✅ Multi-Master Architecture with Role Specialization" << std::endl;
        std::cout << "   ✅ Network Topology Management with Regional Partitioning" << std::endl;
        std::cout << "   ✅ Distributed Load Balancing with Geographic Awareness" << std::endl;
        std::cout << "   ✅ Cross-Master Coordination and Global Consensus" << std::endl;
        std::cout << "   ✅ Automatic Failover and Master Election" << std::endl;
        std::cout << "   ✅ Cross-Region Network Links and Synchronization" << std::endl;
