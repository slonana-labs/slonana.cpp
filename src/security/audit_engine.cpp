...
SecurityAuditEngine::SecurityAuditEngine(const AuditConfig &config,
                                         const std::string &target_path)
    : config_(config), target_path_(target_path) {

  std::cout << "Security audit engine initialized for target: " << target_path_
            << std::endl;
...
bool SecurityAuditEngine::run_audit() {
  ...
  for (auto &rule : rules_) {
    if (config_.enabled_categories.empty() ||
        std::find(
            config_.enabled_categories.begin(),
            config_.enabled_categories.end(),
            audit_utils::audit_category_to_string(rule->get_category())) !=
            config_.enabled_categories.end()) {
      ...
      findings_.insert(findings_.end(), rule_findings.begin(),
                       rule_findings.end());
    }
  }
  ...
  for (auto &tool : external_tools_) {
    std::cout << "Executing external tool: " << tool->get_tool_name()
              << std::endl;
    ...
    if (tool->is_available()) {
      ...
      findings_.insert(findings_.end(), tool_findings.begin(),
                       tool_findings.end());
    }
  }
  ...
  std::cout << "Security audit completed in " << duration.count() << " seconds"
            << std::endl;
  std::cout << "Rules executed: " << rules_executed
            << ", passed: " << rules_passed << std::endl;
...
bool SecurityAuditEngine::execute_rule(IAuditRule &rule) {
  try {
    ...
    std::cerr << "Error executing rule " << rule.get_rule_id() << ": "
              << e.what() << std::endl;
    return false;
  }
}
...
bool SecurityAuditEngine::execute_external_tool(IAuditTool &tool) {
  try {
    ...
    std::cerr << "Error executing tool " << tool.get_tool_name() << ": "
              << e.what() << std::endl;
    return false;
  }
}
...
SecurityAuditEngine::get_findings_by_severity(AuditLevel severity) const {
...
std::vector<AuditFinding>
SecurityAuditEngine::get_findings_by_category(AuditCategory category) const {
  std::vector<AuditFinding> filtered;
  for (const auto &finding : findings_) {
...
AuditReport SecurityAuditEngine::get_report() {
  ...
  // Count rules with medium/low findings as warnings (still passed but with
  // notes)
...
bool SecurityAuditEngine::export_report(const std::string &output_path,
  ...
  try {
    ...
    if (format == "json") {
      ...
      file << json_report;
      file.close();
      std::cout << "Security audit report exported to: " << output_path
                << std::endl;
      return true;
    } else if (format == "html") {
...
bool SecurityAuditEngine::validate_target_path() const {
  return std::filesystem::exists(target_path_) &&
         std::filesystem::is_directory(target_path_);
}
...
std::string audit_category_to_string(AuditCategory category) {
  switch (category) {
  case AuditCategory::CODE_ANALYSIS:
    return "code_analysis";
  case AuditCategory::NETWORK_SECURITY:
    return "network_security";
  case AuditCategory::CRYPTOGRAPHIC_VALIDATION:
    return "cryptographic_validation";
  case AuditCategory::ACCESS_CONTROL:
    return "access_control";
  case AuditCategory::INPUT_VALIDATION:
    return "input_validation";
  case AuditCategory::DEPENDENCY_SCAN:
    return "dependency_scan";
  case AuditCategory::CONFIGURATION_REVIEW:
    return "configuration_review";
  case AuditCategory::RUNTIME_ANALYSIS:
...
uint64_t get_current_timestamp() { return std::time(nullptr); }
...
std::string format_report_json(const AuditReport &report) {
  ...
  for (size_t i = 0; i < report.findings.size(); ++i) {
    ...
    json << "      \"severity\": \"" << audit_level_to_string(finding.severity)
         << "\",\n";
    json << "      \"category\": \""
         << audit_category_to_string(finding.category) << "\",\n";
...
std::string format_report_html(const AuditReport &report) {
  ...
  html << "<!DOCTYPE html>\n<html>\n<head>\n";
  html << "<title>Security Audit Report - " << report.audit_name
       << "</title>\n";
  html << "<style>\n";
  ...
  html << "<p><strong>Critical Findings:</strong> " << report.critical_findings
       << "</p>\n";
  html << "<p><strong>High Findings:</strong> " << report.high_findings
       << "</p>\n";
  html << "<p><strong>Medium Findings:</strong> " << report.medium_findings
       << "</p>\n";
  html << "<p><strong>Low Findings:</strong> " << report.low_findings
       << "</p>\n";
  html << "<h2>Detailed Findings</h2>\n";
  html << "<table>\n";
  html << "<tr><th>ID</th><th>Severity</th><th>Category</th><th>Title</"
