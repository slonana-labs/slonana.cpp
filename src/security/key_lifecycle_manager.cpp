...
KeyRotationScheduler::~KeyRotationScheduler() { stop_scheduler(); }

void KeyRotationScheduler::set_default_policy(const KeyRotationPolicy &policy) {
  policy_ = policy;
}

void KeyRotationScheduler::set_key_policy(const std::string &key_id,
                                          const KeyRotationPolicy &policy) {
  key_policies_[key_id] = policy;
}

KeyRotationPolicy
KeyRotationScheduler::get_policy(const std::string &key_id) const {
...
Result<bool> KeyRotationScheduler::start_scheduler() {
  ...
  rotation_thread_ = std::make_unique<std::thread>(
      &KeyRotationScheduler::rotation_worker_thread, this);
  ...
  while (is_running_) {
    ...
    if (policy_.add_jitter) {
      ...
      std::uniform_int_distribution<> dis(
          0, static_cast<int>(ROTATION_CHECK_JITTER_MAX.count()));
    ...
    if (scheduler_cv_.wait_for(lock, wait_time,
...
bool KeyRotationScheduler::should_rotate_key(
    const std::string &key_id, const KeyMetadata &metadata) const {
...
Result<SecureBuffer>
KeyRotationScheduler::generate_new_key(const std::string &key_type) const {
...
Result<std::string>
KeyRotationScheduler::rotate_key(const std::string &key_id) {
  // Get current key metadata
  auto metadata_result = key_store_->get_key_metadata(key_id);
  if (!metadata_result.is_ok()) {
    return Result<std::string>("Failed to get key metadata: " +
  ...
  if (!new_key_result.is_ok()) {
    return Result<std::string>("Failed to generate new key: " +
                               new_key_result.error());
  }
  ...
  new_metadata.expires_at =
      new_metadata.created_at + get_policy(key_id).rotation_interval;
  ...
  // Store new key
  auto store_result =
      key_store_->store_key(new_key_id, new_key_result.value(), new_metadata);
  if (!store_result.is_ok()) {
    return Result<std::string>("Failed to store new key: " +
                               store_result.error());
  }
...
Result<bool> KeyRotationScheduler::revoke_key(const std::string &key_id,
                                              const std::string &reason) {
  auto metadata_result = key_store_->get_key_metadata(key_id);
  if (!metadata_result.is_ok()) {
    return Result<bool>("Failed to get key metadata: " +
                        metadata_result.error());
  }
...
Result<std::vector<std::string>> KeyRotationScheduler::check_expired_keys() {
  ...
  if (!keys_result.is_ok()) {
    return Result<std::vector<std::string>>("Failed to list keys: " +
                                            keys_result.error());
  }
...
CryptographicKeyManager::~CryptographicKeyManager() { shutdown(); }

Result<bool> CryptographicKeyManager::initialize() {
  // Check entropy quality
  if (!key_utils::check_entropy_quality()) {
    return Result<bool>(
        "Insufficient system entropy for secure key generation");
  }

  // Initialize key store if needed
  if (auto *file_store =
          dynamic_cast<EncryptedFileKeyStore *>(key_store_.get())) {
    auto init_result = file_store->initialize_storage();
    if (!init_result.is_ok()) {
      return Result<bool>("Failed to initialize key storage: " +
                          init_result.error());
    }
  }
  ...
  if (!scheduler_result.is_ok()) {
    return Result<bool>("Failed to start rotation scheduler: " +
...
Result<SecureBuffer>
CryptographicKeyManager::generate_cryptographically_secure_key(
    size_t key_size) const {
  return key_utils::generate_secure_random(key_size);
}

std::string
CryptographicKeyManager::generate_key_id(const std::string &key_type) const {
...
Result<std::string> CryptographicKeyManager::generate_validator_identity_key() {
  ...
  if (!key_result.is_ok()) {
    return Result<std::string>("Failed to generate validator key: " +
                               key_result.error());
  }
  ...
  metadata.authorized_operations = {"sign_blocks", "sign_transactions",
                                    "consensus_voting"};

  // Store key
  auto store_result =
      key_store_->store_key(key_id, key_result.value(), metadata);
  if (!store_result.is_ok()) {
    return Result<std::string>("Failed to store validator key: " +
                               store_result.error());
  }
...
Result<std::string> CryptographicKeyManager::generate_session_key() {
  auto key_result = generate_cryptographically_secure_key(32);
  if (!key_result.is_ok()) {
    return Result<std::string>("Failed to generate session key: " +
                               key_result.error());
  }
  ...
  metadata.created_at = std::chrono::system_clock::now();
  metadata.expires_at =
      metadata.created_at + std::chrono::hours(24); // Sessions expire daily
  metadata.use_count = 0;
  metadata.is_revoked = false;
  metadata.authorized_operations = {"encrypt_communications",
                                    "authenticate_requests"};

  auto store_result =
      key_store_->store_key(key_id, key_result.value(), metadata);
  if (!store_result.is_ok()) {
    return Result<std::string>("Failed to store session key: " +
                               store_result.error());
  }
...
Result<std::string>
CryptographicKeyManager::generate_key(const std::string &key_type,
                                      size_t key_size) {
...
Result<SecureBuffer>
CryptographicKeyManager::get_key(const std::string &key_id) {
  ...
  const auto &metadata = metadata_result.value();
  if (metadata.is_revoked) {
    return Result<SecureBuffer>("Key has been revoked: " +
...
Result<KeyMetadata>
CryptographicKeyManager::get_key_info(const std::string &key_id) {
...
Result<std::string>
CryptographicKeyManager::rotate_key(const std::string &key_id) {
  return rotation_scheduler_->rotate_key(key_id);
}

Result<bool> CryptographicKeyManager::revoke_key(const std::string &key_id,
                                                 const std::string &reason) {
  return rotation_scheduler_->revoke_key(key_id, reason);
}

Result<bool>
CryptographicKeyManager::extend_key_expiry(const std::string &key_id,
...
Result<std::string>
CryptographicKeyManager::import_legacy_key(const std::string &file_path,
  ...
  std::vector<uint8_t> private_key_data(file_data.begin(),
                                        file_data.begin() + 32);
  ...
  if (!store_result.is_ok()) {
    return Result<std::string>("Failed to store imported key: " +
                               store_result.error());
  }
...
Result<bool>
CryptographicKeyManager::export_key_for_backup(const std::string &key_id,
  ...
  auto encrypted_result = static_cast<EncryptedFileKeyStore *>(key_store_.get())
                              ->encrypt_data_for_backup(key_result.value());
  if (!encrypted_result.is_ok()) {
    return Result<bool>("Failed to encrypt backup: " +
  ...
  backup_file.write(reinterpret_cast<const char *>(encrypted_data.data()),
                    encrypted_data.size());
  backup_file.close();
...
void CryptographicKeyManager::set_rotation_policy(
    const KeyRotationPolicy &policy) {
...
Result<bool>
CryptographicKeyManager::set_primary_validator_key(const std::string &key_id) {
...
CryptographicKeyManager::KeyManagerStats
CryptographicKeyManager::get_stats() const {
...
Result<bool>
CryptographicKeyManager::emergency_revoke_all_keys(const std::string &reason) {
  auto keys_result = key_store_->list_keys();
  if (!keys_result.is_ok()) {
    return Result<bool>("Failed to list keys: " + keys_result.error());
  }
...
Result<std::vector<std::string>> CryptographicKeyManager::audit_key_usage() {
  ...
  for (const auto &key_id : keys_result.value()) {
    ...
    if (metadata_result.is_ok()) {
      ...
      entry << "Key: " << key_id << ", Type: " << metadata.key_type
