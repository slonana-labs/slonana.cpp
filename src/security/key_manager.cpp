...
Result<KeyMetadata> KeyMetadata::from_json(const std::string &json_str) {
  try {
    ...
    metadata.authorized_operations =
        j.value("authorized_operations", std::vector<std::string>{});
...
bool KeyMetadata::validate_schema() const {
  ...
  // Validate key type
  const std::vector<std::string> valid_types = {"validator_identity", "session",
                                                "backup", "temporary"};
  if (std::find(valid_types.begin(), valid_types.end(), key_type) ==
      valid_types.end()) {
    return false;
  }
...
bool SecureBuffer::compare(const SecureBuffer &other) const {
  ...
  return key_utils::secure_compare(data_.data(), other.data_.data(),
                                   data_.size());
}
...
EncryptedFileKeyStore::EncryptedFileKeyStore(const std::string &storage_path)
    : storage_path_(storage_path),
      master_key_(AES_256_KEY_SIZE) { // Initialize with proper size
  ...
  if (key_result.is_ok()) {
    ...
    std::copy(key_data.data(), key_data.data() + key_data.size(),
              master_key_.data());

    // Warn user about random key generation
    std::cerr << "⚠️  WARNING: Generated random master key. Keys may be "
                 "unrecoverable if storage is lost!"
              << std::endl;
    std::cerr
        << "    Consider using initialize_with_passphrase() for production use."
        << std::endl;
  }
}
...
EncryptedFileKeyStore::EncryptedFileKeyStore(const std::string &storage_path,
    ...
    : storage_path_(storage_path),
      master_key_(master_key.copy()) { // Use copy method
}
...
std::string
EncryptedFileKeyStore::get_key_file_path(const std::string &key_id) const {
  return storage_path_ + "/" + key_id + ".key";
}

std::string
EncryptedFileKeyStore::get_metadata_file_path(const std::string &key_id) const {
  return storage_path_ + "/" + key_id + ".meta";
}

Result<SecureBuffer>
EncryptedFileKeyStore::encrypt_data(const SecureBuffer &plaintext) const {
  ...
  if (EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, master_key_.data(),
                         iv.data()) != 1) {
    EVP_CIPHER_CTX_free(ctx);
  ...
  // Finalize encryption
  if (EVP_EncryptFinal_ex(ctx, ciphertext.data() + iv.size() + len, &len) !=
      1) {
    EVP_CIPHER_CTX_free(ctx);
...
Result<SecureBuffer>
EncryptedFileKeyStore::decrypt_data(const SecureBuffer &ciphertext) const {
  if (ciphertext.size() < 28 ||
      master_key_.empty()) { // IV(12) + min_data(0) + tag(16)
  ...
  if (EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, master_key_.data(),
                         iv) != 1) {
    EVP_CIPHER_CTX_free(ctx);
  ...
  // Decrypt data
  if (EVP_DecryptUpdate(ctx, plaintext.data(), &len, encrypted_data,
                        encrypted_size) != 1) {
    EVP_CIPHER_CTX_free(ctx);
...
Result<bool> EncryptedFileKeyStore::store_key(const std::string &key_id,
  ...
  key_file.write(reinterpret_cast<const char *>(encrypted_data.data()),
                 encrypted_data.size());
  key_file.close();
  ...
  meta_file << "created_at="
            << std::chrono::duration_cast<std::chrono::seconds>(
                   metadata.created_at.time_since_epoch())
                   .count()
            << "\n";
  meta_file << "expires_at="
            << std::chrono::duration_cast<std::chrono::seconds>(
                   metadata.expires_at.time_since_epoch())
                   .count()
            << "\n";
...
Result<SecureBuffer>
EncryptedFileKeyStore::load_key(const std::string &key_id) {
  std::string key_path = get_key_file_path(key_id);
...
Result<KeyMetadata>
EncryptedFileKeyStore::get_key_metadata(const std::string &key_id) {
  std::string meta_path = get_metadata_file_path(key_id);
...
Result<bool>
EncryptedFileKeyStore::update_metadata(const std::string &key_id,
                                       const KeyMetadata &metadata) {
  ...
  try {
    if (!std::filesystem::exists(storage_path_)) {
      return Result<std::vector<std::string>>(
          keys); // Return empty list if directory doesn't exist
    }
    ...
    return Result<std::vector<std::string>>("Failed to scan key directory: " +
                                            std::string(e.what()));
  }
...
Result<bool> EncryptedFileKeyStore::initialize_storage() {
  ...
  if (!std::filesystem::exists(storage_dir)) {
    ...
    if (!std::filesystem::create_directories(storage_dir, ec)) {
      return Result<bool>("Failed to create storage directory: " +
                          ec.message());
    }
    ...
    if (ec) {
      return Result<bool>("Failed to set directory permissions: " +
                          ec.message());
    }
  }
...
Result<bool>
EncryptedFileKeyStore::change_master_key(const SecureBuffer &new_master_key) {
...
Result<bool>
EncryptedFileKeyStore::secure_delete_file(const std::string &file_path) const {
  try {
    ...
    if (file_size > 0) {
      ...
      out_file.write(reinterpret_cast<const char *>(pass1_data.data()),
                     file_size);
      out_file.flush();
      out_file.seekp(0);
      ...
      out_file.write(reinterpret_cast<const char *>(pass2_data.data()),
                     file_size);
      out_file.flush();
      out_file.seekp(0);
      ...
      if (RAND_bytes(pass3_data.data(), file_size) == 1) {
        out_file.write(reinterpret_cast<const char *>(pass3_data.data()),
                       file_size);
        out_file.flush();
      }
...
Result<bool> EncryptedFileKeyStore::initialize_with_passphrase(
  ...
  auto derived_key_result =
      key_utils::derive_key_from_passphrase(passphrase, salt);
  if (!derived_key_result.is_ok()) {
    return Result<bool>("Failed to derive key from passphrase: " +
                        derived_key_result.error());
  }

  // Clear current master key and set new one
  master_key_.secure_wipe();
  auto derived_key_data = derived_key_result.value().copy();
  std::copy(derived_key_data.begin(), derived_key_data.end(),
            master_key_.data());
...
Result<bool> EncryptedFileKeyStore::prompt_for_passphrase_if_needed() {
  ...
  if (!isatty(STDIN_FILENO)) {
    return Result<bool>(
        "Non-interactive environment, cannot prompt for passphrase");
  }
...
std::string
EncryptedFileKeyStore::get_passphrase_from_user(const std::string &prompt) {
...
Result<SecureBuffer>
derive_key_from_passphrase(const std::string &passphrase,
  ...
  if (PKCS5_PBKDF2_HMAC(passphrase.c_str(), passphrase.length(), salt.data(),
