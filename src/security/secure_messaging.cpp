...
// Platform-specific includes for directory scanning
#ifdef _WIN32
#include <windows.h>
#else
#include <dirent.h>
...
std::vector<uint8_t> SecureMessage::serialize() const {
  ...
  result.insert(result.end(), sender_public_key.begin(),
                sender_public_key.end());
...
SecureMessage::deserialize(const std::vector<uint8_t> &data) {
  ...
  try {
    ...
    msg.message_type =
        std::string(data.begin() + offset, data.begin() + offset + type_len);
    offset += type_len;
    ...
    msg.sender_public_key.assign(data.begin() + offset,
    ...
    msg.signature.assign(data.begin() + offset,
    ...
    return Result<SecureMessage>("Deserialization failed: " +
                                 std::string(e.what()));
  }
}
...
Result<bool> TlsContextManager::initialize() {
  ...
  if (!client_ctx_) {
    return Result<bool>("Failed to create client SSL context: " +
                        get_openssl_error());
  }
  ...
  if (!server_ctx_) {
    return Result<bool>("Failed to create server SSL context: " +
                        get_openssl_error());
  }
...
bool TlsContextManager::load_certificates() {
  ...
  if (!config_.tls_cert_path.empty()) {
    if (SSL_CTX_use_certificate_file(server_ctx_, config_.tls_cert_path.c_str(),
                                     SSL_FILETYPE_PEM) <= 0) {
      std::cerr << "Failed to load server certificate from "
                << config_.tls_cert_path << ": " << get_openssl_error()
                << std::endl;
      return false;
    }
    if (SSL_CTX_use_certificate_file(client_ctx_, config_.tls_cert_path.c_str(),
                                     SSL_FILETYPE_PEM) <= 0) {
      std::cerr << "Failed to load client certificate from "
                << config_.tls_cert_path << ": " << get_openssl_error()
                << std::endl;
      return false;
    }
    std::cout << "✅ Loaded TLS certificate from " << config_.tls_cert_path
              << std::endl;
  }

  // Load private key files
  if (!config_.tls_key_path.empty()) {
    if (SSL_CTX_use_PrivateKey_file(server_ctx_, config_.tls_key_path.c_str(),
                                    SSL_FILETYPE_PEM) <= 0) {
      std::cerr << "Failed to load server private key from "
                << config_.tls_key_path << ": " << get_openssl_error()
                << std::endl;
      return false;
    }
    if (SSL_CTX_use_PrivateKey_file(client_ctx_, config_.tls_key_path.c_str(),
                                    SSL_FILETYPE_PEM) <= 0) {
      std::cerr << "Failed to load client private key from "
                << config_.tls_key_path << ": " << get_openssl_error()
                << std::endl;
      return false;
    }
    ...
    if (SSL_CTX_check_private_key(client_ctx_) != 1) {
      std::cerr << "Client private key does not match certificate: "
                << get_openssl_error() << std::endl;
      return false;
    }

    std::cout << "✅ Loaded and verified TLS private key from "
              << config_.tls_key_path << std::endl;
  }
  ...
  if (config_.require_mutual_auth) {
    ...
    if (!config_.ca_cert_path.empty()) {
      if (SSL_CTX_load_verify_locations(
              client_ctx_, config_.ca_cert_path.c_str(), nullptr) != 1) {
        std::cerr << "Failed to load CA certificate for client from "
                  << config_.ca_cert_path << ": " << get_openssl_error()
                  << std::endl;
        return false;
      }
      if (SSL_CTX_load_verify_locations(
              server_ctx_, config_.ca_cert_path.c_str(), nullptr) != 1) {
        std::cerr << "Failed to load CA certificate for server from "
                  << config_.ca_cert_path << ": " << get_openssl_error()
                  << std::endl;
        return false;
      }
      std::cout << "✅ Loaded CA certificate for mutual TLS from "
                << config_.ca_cert_path << std::endl;
    } else {
      std::cout << "⚠️  Mutual TLS enabled but no CA certificate specified - "
...
std::unique_ptr<SSL, void (*)(SSL *)>
TlsContextManager::create_ssl_connection(bool is_client) {
...
bool TlsContextManager::verify_peer_certificate(
    SSL *ssl, const std::string &expected_peer_id) {
  if (!ssl) {
    return false;
  }
...
bool MessageCrypto::load_peer_keys() {
  ...
  if (!dir_check.good()) {
    std::cerr << "⚠️  Peer keys directory does not exist: "
              << config_.verification_keys_dir << std::endl;
    return true; // Non-fatal - allow operation without peer keys
  }

  // Scan directory for .pem files and load public keys
  try {
// Use platform-specific directory scanning
#ifdef _WIN32
    std::string search_path = config_.verification_keys_dir + "\\*.pem";
    WIN32_FIND_DATA find_data;
    HANDLE hFind = FindFirstFile(search_path.c_str(), &find_data);

    if (hFind == INVALID_HANDLE_VALUE) {
      std::cout << "ℹ️  No peer key files found in: "
                << config_.verification_keys_dir << std::endl;
      return true;
    }
    ...
    do {
      ...
      std::string full_path = config_.verification_keys_dir + "\\" + filename;
#else
    // Unix/Linux directory scanning
    DIR *dir = opendir(config_.verification_keys_dir.c_str());
    if (!dir) {
      std::cout << "ℹ️  Unable to open peer keys directory: "
                << config_.verification_keys_dir << std::endl;
      return true;
    }
    ...
    while ((entry = readdir(dir)) != nullptr) {
      ...
      // Extract peer ID from filename (remove .pem extension)
      std::string peer_id = filename.substr(0, filename.length() - 4);

      // Load the public key from PEM file
      FILE *key_file = fopen(full_path.c_str(), "r");
      if (!key_file) {
        std::cerr << "⚠️  Failed to open peer key file: " << full_path
                  << std::endl;
        continue;
      }
      ...
      if (!pkey) {
        std::cerr << "⚠️  Failed to parse peer key from: " << full_path
                  << std::endl;
        continue;
      }

      // Store the EVP_PKEY pointer in peer_keys_ map
      peer_keys_[peer_id] = pkey;
      keys_loaded++;
      std::cout << "✅ Loaded peer key for: " << peer_id << std::endl;
#ifndef _WIN32
    }
    ...
    std::cout << "ℹ️  Loaded " << keys_loaded
              << " peer public keys from: " << config_.verification_keys_dir
              << std::endl;

  } catch (const std::exception &e) {
    std::cerr << "⚠️  Error loading peer keys: " << e.what() << std::endl;
    return false;
  }
...
Result<SecureMessage>
MessageCrypto::protect_message(const std::vector<uint8_t> &plaintext,
  ...
  try {
    if (config_.enable_message_encryption) {
      ...
      auto encrypt_result =
          encrypt_aes_gcm(plaintext, symmetric_key, msg.iv, msg.auth_tag);
      if (!encrypt_result.is_ok()) {
        return Result<SecureMessage>("AES-GCM encryption failed: " +
    ...
    if (config_.enable_message_signing && signing_key_) {
      ...
      if (!pubkey_result.is_ok()) {
        return Result<SecureMessage>("Failed to extract public key: " +
                                     pubkey_result.error());
      }
      ...
      if (!signature_result.is_ok()) {
        return Result<SecureMessage>("Ed25519 signing failed: " +
    ...
    return Result<SecureMessage>("Message protection failed: " +
                                 std::string(e.what()));
  }
}

Result<std::vector<uint8_t>>
MessageCrypto::unprotect_message(const SecureMessage &secure_msg,
                                 const std::string &sender_id) {

  // Validate message freshness
  if (config_.enable_replay_protection &&
      !validate_message_freshness(secure_msg)) {
  ...
  if (config_.enable_message_encryption) {
    ...
    if (!derive_symmetric_key(symmetric_key)) {
      return Result<std::vector<uint8_t>>(
          "Failed to derive symmetric key for decryption");
    }

    // Decrypt with AES-GCM
    auto decrypt_result =
        decrypt_aes_gcm(secure_msg.encrypted_payload, symmetric_key,
                        secure_msg.iv, secure_msg.auth_tag);
    if (!decrypt_result.is_ok()) {
      return Result<std::vector<uint8_t>>("AES-GCM decryption failed: " +
...
bool MessageCrypto::verify_message_signature(
    const SecureMessage &secure_msg,
    const std::vector<uint8_t> &sender_public_key) {
  if (sender_public_key.size() != 32) {
    std::cerr << "Invalid public key size: " << sender_public_key.size()
              << " (expected 32)" << std::endl;
    return false;
  }

  if (secure_msg.signature.size() != 64) {
    std::cerr << "Invalid signature size: " << secure_msg.signature.size()
  ...
  try {
    ...
    auto verify_result =
        verify_ed25519(signature_data, secure_msg.signature, sender_public_key);
    if (!verify_result.is_ok()) {
      std::cerr << "Ed25519 verification failed: " << verify_result.error()
                << std::endl;
      return false;
    }
...
bool MessageCrypto::validate_message_freshness(
    const SecureMessage &secure_msg) {
  uint64_t current_time = get_current_timestamp_ms();

  // Check if message is within valid time window
  if (current_time >
      secure_msg.timestamp_ms + (config_.message_ttl_seconds * 1000)) {
    return false;
  }
...
uint64_t MessageCrypto::generate_nonce() {
  ...
  uint64_t nonce;
  if (RAND_bytes(reinterpret_cast<unsigned char *>(&nonce), sizeof(nonce)) !=
      1) {
    ...
    nonce = (static_cast<uint64_t>(timestamp & 0xFFFFFFFF) << 32) |
            (gen() & 0xFFFFFFFF);
  }
...
void MessageCrypto::add_used_nonce(uint64_t nonce) {
  ...
  if (used_nonces_.size() > config_.nonce_cache_size) {
    ...
    auto oldest = std::min_element(
        used_nonces_.begin(), used_nonces_.end(),
...
bool MessageCrypto::derive_symmetric_key(std::vector<uint8_t> &key) {
  ...
  if (EVP_PKEY_get_raw_private_key(signing_key_, raw_key.data(),
                                   &raw_key_len) != 1) {
    std::cerr << "Failed to extract raw private key: " << get_openssl_error()
              << std::endl;
    return false;
  }
...
Result<std::vector<uint8_t>> MessageCrypto::encrypt_aes_gcm(
    ...
    const std::vector<uint8_t> &iv, std::vector<uint8_t> &auth_tag) {
  ...
  try {
    ...
    EVP_CIPHER_CTX_free(ctx);
    return Result<std::vector<uint8_t>>("AES-GCM encryption failed: " +
                                        std::string(e.what()));
  }
}
...
Result<std::vector<uint8_t>> MessageCrypto::decrypt_aes_gcm(
    ...
    const std::vector<uint8_t> &iv, const std::vector<uint8_t> &auth_tag) {
...
Result<std::vector<uint8_t>> MessageCrypto::extract_public_key() {
  ...
  if (EVP_PKEY_get_raw_public_key(signing_key_, pub_key.data(), &pub_key_len) !=
      1) {
    return Result<std::vector<uint8_t>>("Failed to extract public key: " +
                                        get_openssl_error());
  }
...
std::vector<uint8_t>
MessageCrypto::create_signature_data(const SecureMessage &msg) {
  ...
  // Encrypted payload
  data.insert(data.end(), msg.encrypted_payload.begin(),
              msg.encrypted_payload.end());

  return data;
}

Result<std::vector<uint8_t>>
MessageCrypto::sign_ed25519(const std::vector<uint8_t> &data) {
  if (!signing_key_) {
    return Result<std::vector<uint8_t>>("No signing key available");
  }
  ...
  try {
    if (EVP_DigestSignInit(md_ctx, nullptr, nullptr, nullptr, signing_key_) !=
        1) {
      throw std::runtime_error("Failed to initialize signing");
    }

    if (EVP_DigestSign(md_ctx, signature.data(), &sig_len, data.data(),
                       data.size()) != 1) {
    ...
    EVP_MD_CTX_free(md_ctx);
    return Result<std::vector<uint8_t>>("Ed25519 signing failed: " +
                                        std::string(e.what()));
  }
}

Result<bool>
MessageCrypto::verify_ed25519(const std::vector<uint8_t> &data,
  ...
  try {
    ...
    EVP_MD_CTX_free(md_ctx);
    EVP_PKEY_free(pub_key);
    return Result<bool>("Ed25519 verification failed: " +
                        std::string(e.what()));
  }
}
...
Result<bool> SecureMessaging::initialize() {
  ...
  if (config_.enable_message_signing || config_.enable_message_encryption) {
    ...
    if (!crypto_result.is_ok()) {
      return Result<bool>("Message crypto initialization failed: " +
                          crypto_result.error());
    }
  }
...
Result<std::vector<uint8_t>>
SecureMessaging::prepare_outbound_message(const std::vector<uint8_t> &plaintext,
                                          const std::string &message_type,
                                          const std::string &recipient_id) {

  auto protect_result =
      message_crypto_.protect_message(plaintext, message_type, recipient_id);
...
Result<std::vector<uint8_t>> SecureMessaging::process_inbound_message(
    const std::vector<uint8_t> &encrypted_data, const std::string &sender_id) {
  ...
  auto unprotect_result =
      message_crypto_.unprotect_message(deserialize_result.value(), sender_id);
  if (!unprotect_result.is_ok()) {
    ...
    } else if (unprotect_result.error().find("freshness") !=
               std::string::npos) {
...
bool SecureMessaging::is_peer_trusted(const std::string &peer_id) const {
  ...
  if (it == trusted_peers_.end()) {
    std::cerr << "⚠️  Peer " << peer_id << " is not in trusted peer list"
              << std::endl;
    return false;
  }
...
void SecureMessaging::add_trusted_peer(const std::string &peer_id,
                                       const std::vector<uint8_t> &public_key) {
  if (public_key.size() != 32) {
    std::cerr << "Invalid public key size for peer " << peer_id << ": "
              << public_key.size() << " (expected 32)" << std::endl;
    return;
  }
...
SecureChannel::SecureChannel(
    std::unique_ptr<SSL, void (*)(SSL *)> ssl_connection)
...
Result<size_t> SecureChannel::send(const std::vector<uint8_t> &data) {
  ...
  int bytes_sent =
      SSL_write(ssl_.get(), data.data(), static_cast<int>(data.size()));
  if (bytes_sent <= 0) {
    int error = SSL_get_error(ssl_.get(), bytes_sent);
    return Result<size_t>("SSL_write failed with error: " +
                          std::to_string(error));
  }
...
Result<std::vector<uint8_t>> SecureChannel::receive(size_t max_bytes) {
  ...
  int bytes_received =
      SSL_read(ssl_.get(), buffer.data(), static_cast<int>(max_bytes));

  if (bytes_received <= 0) {
    int error = SSL_get_error(ssl_.get(), bytes_received);
    return Result<std::vector<uint8_t>>("SSL_read failed with error: " +
                                        std::to_string(error));
  }
...
std::string SecureChannel::get_peer_identity() const {
  ...
  char *subject_name =
      X509_NAME_oneline(X509_get_subject_name(peer_cert), nullptr, 0);
