...
SecureValidatorIdentity::~SecureValidatorIdentity() { shutdown(); }

Result<bool> SecureValidatorIdentity::initialize() {
  // Create key store
  auto key_store =
      SecureKeyManagerFactory::create_encrypted_file_store(storage_path_);
  if (!key_store) {
  ...
  if (!init_result.is_ok()) {
    return Result<bool>("Failed to initialize key manager: " +
                        init_result.error());
  }
...
Result<std::string> SecureValidatorIdentity::create_new_identity() {
  ...
  if (!key_result.is_ok()) {
    return Result<std::string>("Failed to generate identity: " +
                               key_result.error());
  }
  ...
  if (primary_identity_key_id_.empty()) {
    ...
    if (!set_result.is_ok()) {
      return Result<std::string>("Failed to set primary key: " +
                                 set_result.error());
    }
  }
...
Result<std::string> SecureValidatorIdentity::import_legacy_identity(
  ...
  auto import_result =
      key_manager_->import_legacy_key(keypair_path, "validator_identity");
  if (!import_result.is_ok()) {
    return Result<std::string>("Failed to import legacy key: " +
                               import_result.error());
  }
...
Result<bool>
SecureValidatorIdentity::set_primary_identity(const std::string &key_id) {
  if (!key_manager_) {
...
Result<common::PublicKey> SecureValidatorIdentity::get_public_key() {
  ...
  if (!key_result.is_ok()) {
    return Result<common::PublicKey>("Failed to get key: " +
                                     key_result.error());
  }
  ...
  // libsodium expects a 64-byte secret key (32 private + 32 public), but we
  // only have 32-byte private For proper Ed25519, we need to derive the full
  ...
  if (crypto_sign_seed_keypair(public_key_bytes, full_secret_key,
                               private_key.data()) != 0) {
...
Result<std::string> SecureValidatorIdentity::rotate_identity() {
  ...
  if (rotate_result.is_ok()) {
    ...
    if (!set_result.is_ok()) {
      return Result<std::string>("Failed to set new primary identity: " +
                                 set_result.error());
    }
...
Result<bool>
SecureValidatorIdentity::revoke_identity(const std::string &reason) {
  if (!key_manager_ || primary_identity_key_id_.empty()) {
    return Result<bool>("No primary identity to revoke");
  }

  auto revoke_result =
      key_manager_->revoke_key(primary_identity_key_id_, reason);
...
Result<common::PublicKey>
SecureValidatorIdentity::generate_legacy_compatible_identity() {
  // Create new identity and return public key in legacy format
  auto create_result = create_new_identity();
  if (!create_result.is_ok()) {
    return Result<common::PublicKey>("Failed to create identity: " +
                                     create_result.error());
  }
...
Result<bool>
SecureValidatorIdentity::export_for_legacy_use(const std::string &output_path) {
  if (!key_manager_ || primary_identity_key_id_.empty()) {
  ...
  std::copy(private_key.data(), private_key.data() + 32,
            legacy_keypair.begin());
  ...
  file.write(reinterpret_cast<const char *>(legacy_keypair.data()),
             legacy_keypair.size());
  file.close();
...
Result<SecureValidatorIdentity::IdentityStatus>
SecureValidatorIdentity::get_status() {
  if (!key_manager_ || primary_identity_key_id_.empty()) {
  ...
  if (!metadata_result.is_ok()) {
    return Result<IdentityStatus>("Failed to get key info: " +
...
Result<std::vector<SecureValidatorIdentity::IdentityStatus>>
SecureValidatorIdentity::list_all_identities() {
  if (!key_manager_) {
  ...
  auto keys_result = key_manager_->list_keys();
  if (!keys_result.is_ok()) {
    return Result<std::vector<IdentityStatus>>("Failed to list keys: " +
                                               keys_result.error());
  }
...
std::shared_ptr<KeyStore> SecureKeyManagerFactory::create_encrypted_file_store(
    const std::string &storage_path) {
  return std::make_shared<EncryptedFileKeyStore>(storage_path);
}

std::shared_ptr<KeyStore> SecureKeyManagerFactory::create_encrypted_file_store(
    const std::string &storage_path, const std::string &passphrase) {
  // Derive key from passphrase
  std::vector<uint8_t> salt = {0x73, 0x6c, 0x6f, 0x6e,
                               0x61, 0x6e, 0x61, 0x00}; // "slonana\0"
...
std::shared_ptr<CryptographicKeyManager>
SecureKeyManagerFactory::create_key_manager(std::shared_ptr<KeyStore> store) {
  return std::make_shared<CryptographicKeyManager>(store);
}

std::unique_ptr<SecureValidatorIdentity>
SecureKeyManagerFactory::create_secure_identity(
    const std::string &storage_path) {
