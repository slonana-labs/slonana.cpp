...
public:
    ...
    std::optional<LoadedAccount> load_account_internal(
        ...
        LoadedAccount loaded_account(
            address,
            *account_opt,
            account_opt->data.size() + 128, // Account metadata overhead
            callback_->get_slot(),
            is_writable,
            is_signer
        );
    ...
    TransactionLoadResult validate_account_constraints(
...
LoadedTransaction AccountLoader::load_transaction_accounts(
    ...
    for (size_t i = 0; i < account_keys.size(); ++i) {
        ...
        auto loaded_account = impl_->load_account_internal(account_keys[i], writable, signer);
        if (!loaded_account) {
            loaded_tx.load_result = TransactionLoadResult::ACCOUNT_NOT_FOUND;
            loaded_tx.error_message = "Account not found: " + std::string(account_keys[i].begin(), account_keys[i].end());
            return loaded_tx;
        }
    ...
    for (const auto& account : loaded_tx.accounts) {
        if (account.account.lamports == 0) {
            loaded_tx.rent += impl_->callback_->calculate_rent(account.account.data.size());
        }
    }
...
std::optional<LoadedAccount> AccountLoader::load_account(
    const PublicKey& address,
    bool is_writable,
    bool is_signer
) {
...
TransactionLoadResult AccountLoader::validate_fee_payer(
    const PublicKey& fee_payer,
    Lamports fee_amount,
