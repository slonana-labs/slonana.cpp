...
AdvancedProgramCache::AdvancedProgramCache(const Configuration& config) : config_(config) {
  ...
  if (config_.enable_precompilation) {
    precompilation_thread_ = std::thread(&AdvancedProgramCache::precompilation_worker_loop, this);
  }
  
  std::cout << "AdvancedProgramCache initialized with max size: " << config_.max_cache_size 
            << ", max memory: " << config_.max_memory_usage_mb << "MB" << std::endl;
}
...
std::shared_ptr<CompiledProgram> AdvancedProgramCache::get_program(const PublicKey& program_id) {
...
bool AdvancedProgramCache::cache_program(const PublicKey& program_id, const std::vector<uint8_t>& bytecode) {
  ...
  std::cout << "Cached program: " << program_id.size() << " bytes, compilation time: " 
            << program->compilation_cost_us << "μs" << std::endl;
  
  return true;
}

bool AdvancedProgramCache::precompile_program(const PublicKey& program_id, const std::vector<uint8_t>& bytecode) {
  if (!config_.enable_precompilation) {
    return false;
  }
  ...
  std::cout << "Precompiled program: " << program_id.size() << " bytes" << std::endl;
...
bool AdvancedProgramCache::compile_program(CompiledProgram& program) {
  ...
  bool success = jit_compile_bytecode(program.bytecode, program.compiled_code, program.compilation_cost_us);
  
  if (success) {
    auto end_time = std::chrono::high_resolution_clock::now();
    program.compilation_cost_us = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
    program.memory_usage = program.bytecode.size() + program.compiled_code.size();
...
bool AdvancedProgramCache::jit_compile_bytecode(const std::vector<uint8_t>& bytecode, 
  ...
  try {
    ...
    auto end_time = std::chrono::high_resolution_clock::now();
    compilation_time_us = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
...
bool AdvancedProgramCache::should_evict() const {
  return cache_map_.size() >= config_.max_cache_size * config_.eviction_threshold ||
         is_memory_pressure();
}
...
void AdvancedProgramCache::garbage_collect() {
  ...
  auto cutoff_time = now - std::chrono::hours(1); // Remove programs not accessed in 1 hour
  ...
  if (!to_remove.empty()) {
    std::cout << "GC: Removed " << to_remove.size() << " unused programs" << std::endl;
  }
}

void AdvancedProgramCache::gc_worker_loop() {
  while (!should_stop_) {
    std::this_thread::sleep_for(std::chrono::seconds(config_.gc_frequency_seconds));
...
void AdvancedProgramCache::print_cache_statistics() const {
  ...
  std::cout << "Precompiled: " << stats.precompiled_programs.load() << std::endl;
  std::cout << "Memory usage: " << (stats.total_memory_usage.load() / 1024) << " KB" << std::endl;
  std::cout << "Avg compilation time: " << stats.get_avg_compilation_time_us() << " μs" << std::endl;
  std::cout << "===============================" << std::endl;
}

// ProgramCacheManager implementation
ProgramCacheManager::ProgramCacheManager(const AdvancedProgramCache::Configuration& config) {
  cache_ = std::make_unique<AdvancedProgramCache>(config);
}

bool ProgramCacheManager::load_program(const PublicKey& program_id, const std::vector<uint8_t>& bytecode) {
  return cache_->cache_program(program_id, bytecode);
}

std::shared_ptr<CompiledProgram> ProgramCacheManager::execute_program(const PublicKey& program_id) {
  return cache_->get_program(program_id);
}

void ProgramCacheManager::warm_up_cache(const std::vector<PublicKey>& program_ids) {
  std::cout << "Warming up cache with " << program_ids.size() << " programs..." << std::endl;
...
void ProgramCacheManager::start_performance_monitoring() {
  ...
  monitoring_thread_ = std::thread(&ProgramCacheManager::monitoring_worker_loop, this);
}
...
void ProgramCacheManager::log_performance_metrics() {
  ...
  std::cout << "[CACHE MONITOR] Hit rate: " << (stats.get_hit_rate() * 100) 
            ...
            << ", Memory: " << (stats.total_memory_usage.load() / 1024) << "KB" << std::endl;
}
