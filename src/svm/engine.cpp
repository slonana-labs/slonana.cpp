...
ProgramAccount ProgramAccount::deserialize(const std::vector<uint8_t> &data) {
  ...
  if (data.size() >= 32 + 8 + 4 + data_len) {
    account.data.assign(data.begin() + 32 + 8 + 4,
                        data.begin() + 32 + 8 + 4 + data_len);
  }
...
Instruction Instruction::deserialize(const std::vector<uint8_t> &data) {
  ...
  if (offset + 4 <= data.size()) {
    ...
    if (offset + data_len <= data.size()) {
      instruction.data.assign(data.begin() + offset,
                              data.begin() + offset + data_len);
    }
  }
...
public:
  ...
  Impl() {
    ...
    // System program ID is always 11111111111111111111111111111111 (32 bytes of
    // 0x00)
...
PublicKey SystemProgram::get_program_id() const { return impl_->program_id_; }

ExecutionOutcome SystemProgram::execute(const Instruction &instruction,
  ...
  case 0: // CreateAccount
    if (instruction.accounts.size() >= 2) {
      outcome.result =
          handle_create_account_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
  ...
  case 2: // Transfer
    if (instruction.accounts.size() >= 2) {
      outcome.result =
          handle_transfer_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
  ...
  case 3: // CreateAccountWithSeed
    if (instruction.accounts.size() >= 2) {
      outcome.result = handle_create_account_with_seed_instruction(
          instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
      outcome.error_details =
          "CreateAccountWithSeed requires at least 2 accounts";
    }
    break;
  ...
  case 4: // AdvanceNonceAccount
    if (instruction.accounts.size() >= 1) {
      outcome.result =
          handle_advance_nonce_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
  ...
  case 5: // WithdrawNonceAccount
    if (instruction.accounts.size() >= 2) {
      outcome.result =
          handle_withdraw_nonce_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
      outcome.error_details =
          "WithdrawNonceAccount requires at least 2 accounts";
    }
    break;
  ...
  case 6: // InitializeNonceAccount
    if (instruction.accounts.size() >= 1) {
      outcome.result =
          handle_initialize_nonce_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
      outcome.error_details =
          "InitializeNonceAccount requires at least 1 account";
    }
    break;
  ...
  case 7: // AuthorizeNonceAccount
    if (instruction.accounts.size() >= 1) {
      outcome.result =
          handle_authorize_nonce_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
      outcome.error_details =
          "AuthorizeNonceAccount requires at least 1 account";
    }
    break;
  ...
  case 8: // Allocate
    if (instruction.accounts.size() >= 1) {
      outcome.result =
          handle_allocate_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
  ...
  case 9: // AllocateWithSeed
    if (instruction.accounts.size() >= 1) {
      outcome.result =
          handle_allocate_with_seed_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
  ...
  case 10: // AssignWithSeed
    if (instruction.accounts.size() >= 1) {
      outcome.result =
          handle_assign_with_seed_instruction(instruction, context.accounts);
    } else {
      outcome.result = ExecutionResult::INVALID_INSTRUCTION;
  ...
  default:
    outcome.result = ExecutionResult::INVALID_INSTRUCTION;
    outcome.error_details = "Unknown system program instruction type: " +
                            std::to_string(instruction_type);
    break;
  }
...
ExecutionResult SystemProgram::handle_create_account_instruction(
...
ExecutionResult SystemProgram::handle_assign_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
...
ExecutionResult SystemProgram::handle_transfer_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
...
ExecutionResult SystemProgram::handle_create_account_with_seed_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
  std::cout << "SystemProgram: CreateAccountWithSeed instruction executed"
            << std::endl;
  return ExecutionResult::SUCCESS;
}

ExecutionResult SystemProgram::handle_advance_nonce_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
...
ExecutionResult SystemProgram::handle_withdraw_nonce_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
...
ExecutionResult SystemProgram::handle_initialize_nonce_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
  std::cout << "SystemProgram: InitializeNonce instruction executed"
            << std::endl;
  return ExecutionResult::SUCCESS;
}

ExecutionResult SystemProgram::handle_authorize_nonce_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
  std::cout << "SystemProgram: AuthorizeNonce instruction executed"
            << std::endl;
  return ExecutionResult::SUCCESS;
}

ExecutionResult SystemProgram::handle_allocate_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
...
ExecutionResult SystemProgram::handle_allocate_with_seed_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
  std::cout << "SystemProgram: AllocateWithSeed instruction executed"
            << std::endl;
  return ExecutionResult::SUCCESS;
}

ExecutionResult SystemProgram::handle_assign_with_seed_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) const {
  std::cout << "SystemProgram: AssignWithSeed instruction executed"
            << std::endl;
  return ExecutionResult::SUCCESS;
}
...
common::Result<bool>
ExecutionEngine::load_program(const ProgramAccount &program) {
  if (!program.executable) {
    return common::Result<bool>("Account is not executable");
  }

  impl_->loaded_programs_[program.program_id] = program;
  std::cout << "Loaded program: " << program.program_id.size() << " bytes ID"
            << std::endl;
...
bool ExecutionEngine::is_program_loaded(const PublicKey &program_id) const {
  ...
  if (impl_->loaded_programs_.find(program_id) !=
      impl_->loaded_programs_.end()) {
    return true;
  }
...
void ExecutionEngine::set_compute_budget(uint64_t max_compute_units) {
  ...
  std::cout << "Set compute budget to " << max_compute_units << " units"
            << std::endl;
}

void ExecutionEngine::set_feature_set(
    const std::vector<std::string> &features) {
  impl_->feature_set_ = features;
  std::cout << "Updated feature set with " << features.size() << " features"
            << std::endl;
}
...
common::Result<bool>
AccountManager::create_account(const ProgramAccount &account) {
  ...
  std::cout << "Created account with " << account.lamports << " lamports"
            << std::endl;
  return common::Result<bool>(true);
}

std::optional<ProgramAccount>
AccountManager::get_account(const PublicKey &pubkey) const {
...
common::Result<bool>
AccountManager::update_account(const ProgramAccount &account) {
  impl_->pending_changes_[account.pubkey] = account;
  std::cout << "Updated account" << std::endl;
  return common::Result<bool>(true);
}

std::vector<ProgramAccount>
AccountManager::get_program_accounts(const PublicKey &program_id) const {
...
std::vector<ProgramAccount>
AccountManager::get_accounts_by_owner(const PublicKey &owner) const {
  std::vector<ProgramAccount> result;
...
common::Lamports
AccountManager::get_account_balance(const PublicKey &pubkey) const {
...
common::Result<bool> AccountManager::collect_rent(common::Epoch epoch) {
  ...
  // Production implementation: Calculate and collect rent from accounts based
  // on Solana rent schedule
  ...
  for (auto &[pubkey, account] : impl_->accounts_) {
    ...
    uint64_t epochs_per_year =
        365 * 24 * 60 * 60 / 432000; // Assuming ~432s per epoch
    ...
    if (account.lamports >= rent_per_epoch) {
      ...
      std::cout << "Collected " << rent_per_epoch
                << " lamports rent from account (size: " << account_size
                << " bytes)" << std::endl;
    } else {
      ...
      if (account.lamports > 0) {
        ...
        std::cout << "Account closed due to insufficient rent funds"
                  << std::endl;
      }
    }
  }
...
ExecutionResult ExecutionEngine::handle_create_account_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
  // Parse CreateAccount instruction data
  if (instruction.data.size() <
      1 + 8 + 8 + 32) { // type + lamports + space + owner
...
ExecutionResult ExecutionEngine::handle_transfer_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
ExecutionResult ExecutionEngine::handle_assign_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
ExecutionResult ExecutionEngine::handle_create_account_with_seed_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
  // Implementation similar to create_account but with seed-based address
  // derivation
  return handle_create_account_instruction(instruction, accounts);
}

ExecutionResult ExecutionEngine::handle_advance_nonce_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
ExecutionResult ExecutionEngine::handle_withdraw_nonce_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
ExecutionResult ExecutionEngine::handle_initialize_nonce_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
ExecutionResult ExecutionEngine::handle_authorize_nonce_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
ExecutionResult ExecutionEngine::handle_allocate_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
ExecutionResult ExecutionEngine::handle_allocate_with_seed_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
ExecutionResult ExecutionEngine::handle_assign_with_seed_instruction(
    const Instruction &instruction,
    std::unordered_map<PublicKey, ProgramAccount> &accounts) {
