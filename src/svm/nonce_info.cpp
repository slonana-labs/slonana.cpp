...
bool NonceInfo::advance_nonce(const Hash& new_blockhash, Lamports fee_calculator) {
...
bool NonceInfo::initialize_nonce(const PublicKey& authority, const Hash& recent_blockhash, Lamports fee_calculator) {
...
bool NonceInfo::authorize_nonce(const PublicKey& current_authority, const PublicKey& new_authority) {
...
bool NonceInfo::withdraw_nonce(const PublicKey& authority, Lamports amount, Lamports remaining_balance) {
...
AccountInfo NonceInfo::get_updated_account() const {
    return account_;
}
...
std::optional<NonceData> NonceInfo::parse_nonce_data(const std::vector<uint8_t>& data) {
    if (data.size() < 4) {
...
std::vector<uint8_t> NonceInfo::create_nonce_account_data(const NonceData& nonce_data) {
    std::vector<uint8_t> result;
