...
Lamports RentCalculator::calculate_rent(size_t data_size) const {
  ...
  Lamports yearly_rent =
      static_cast<Lamports>(data_size) * config_.lamports_per_byte_year;
...
Lamports RentCalculator::minimum_balance(size_t data_size) const {
  ...
  return static_cast<Lamports>(static_cast<double>(yearly_rent) *
                               config_.exemption_threshold);
}
...
Lamports RentCalculator::calculate_rent_due(Lamports current_balance,
                                            size_t data_size, Slot current_slot,
                                            Slot rent_epoch_start_slot) const {
...
bool RentCalculator::needs_rent_collection(Lamports balance, size_t data_size,
                                           Slot current_slot,
                                           Slot last_rent_epoch) const {
  ...
  Slot rent_epoch =
      rent_utils::calculate_rent_epoch(current_slot, config_.slots_per_epoch);
  return rent_epoch > last_rent_epoch;
}

RentCalculator::RentCollection
RentCalculator::collect_rent(Lamports current_balance, size_t data_size,
                             Slot current_slot,
                             Slot rent_epoch_start_slot) const {
  ...
  Lamports rent_due = calculate_rent_due(current_balance, data_size,
                                         current_slot, rent_epoch_start_slot);
...
RentExemptStatus get_rent_status(const RentCalculator &calculator,
                                 Lamports balance, size_t data_size) {
  if (data_size == 0) {
...
std::string
format_rent_collection(const RentCalculator::RentCollection &collection) {
  std::stringstream ss;
  ss << "RentCollection{";
