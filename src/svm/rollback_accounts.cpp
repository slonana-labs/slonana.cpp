...
RollbackAccounts::RollbackAccounts(
    std::optional<NonceInfo> nonce_account, const PublicKey &fee_payer_address,
    const AccountInfo &fee_payer_rollback_account, Slot fee_payer_rent_epoch)
...
void RollbackAccounts::apply_rollback(
...
RollbackAccounts
RollbackAccounts::create_fee_only_rollback(const PublicKey &fee_payer_address,
  ...
  return RollbackAccounts(std::nullopt, // No nonce account
                          fee_payer_address, rollback_account,
...
std::string RollbackAccounts::get_summary() const {
  ...
  ss << "fee_payer="
     << std::string(
            fee_payer_address_.begin(),
            std::min(fee_payer_address_.begin() + 8, fee_payer_address_.end()))
     << "...";
  ss << ", fee_payer_balance=" << fee_payer_rollback_account_.lamports;
  if (nonce_account_) {
    ss << ", nonce_account="
       << std::string(nonce_account_->get_address().begin(),
                      std::min(nonce_account_->get_address().begin() + 8,
                               nonce_account_->get_address().end()))
       << "...";
  }
  ss << "}";
  return ss.str();
...
RollbackAccounts create_rollback_from_transaction(
  ...
  if (fee_payer_it == original_accounts.end()) {
    throw std::invalid_argument(
        "Fee payer account not found in original accounts");
  }
  ...
  fee_payer_rollback.is_signer =
      false; // Will be set appropriately during transaction
  fee_payer_rollback.is_writable = true; // Fee payer is always writable

  return RollbackAccounts(nonce_info, fee_payer, fee_payer_rollback,
                          fee_payer_account.rent_epoch);
}
...
void apply_selective_rollback(
  ...
  if (std::find(accounts_to_rollback.begin(), accounts_to_rollback.end(),
                fee_payer) != accounts_to_rollback.end()) {
    auto fee_payer_it = accounts.find(fee_payer);
    if (fee_payer_it != accounts.end()) {
      const AccountInfo &rollback_account =
          rollback.get_fee_payer_rollback_account();
      ProgramAccount &account = fee_payer_it->second;
  ...
  if (rollback.has_nonce_account()) {
    ...
    if (std::find(accounts_to_rollback.begin(), accounts_to_rollback.end(),
                  nonce_address) != accounts_to_rollback.end()) {
      auto nonce_it = accounts.find(nonce_address);
      if (nonce_it != accounts.end()) {
...
std::string format_rollback_debug(const RollbackAccounts &rollback) {
  ...
  ss << "  Fee Payer: "
     << std::string(rollback.get_fee_payer_address().begin(),
                    std::min(rollback.get_fee_payer_address().begin() + 16,
                             rollback.get_fee_payer_address().end()))
     << "...\n";
  ss << "  Fee Payer Balance: "
     << rollback.get_fee_payer_rollback_account().lamports << "\n";
  ss << "  Fee Payer Data Size: "
     << rollback.get_fee_payer_rollback_account().data.size() << "\n";
  ...
  if (rollback.has_nonce_account()) {
    const NonceInfo &nonce_info = *rollback.get_nonce_account();
    ss << "  Nonce Account: "
       << std::string(nonce_info.get_address().begin(),
                      std::min(nonce_info.get_address().begin() + 16,
                               nonce_info.get_address().end()))
       << "...\n";
    ss << "  Nonce State: "
       << (nonce_info.get_state() == NonceState::INITIALIZED ? "INITIALIZED"
                                                             : "UNINITIALIZED")
       << "\n";
  } else {
  ...
  ss << "  Total Rollback Size: " << calculate_rollback_size(rollback)
     << " bytes";
