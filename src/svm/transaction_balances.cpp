...
void TransactionBalances::record_pre_balances(
...
void TransactionBalances::record_post_balances(
    const std::vector<PublicKey> &addresses,
    const std::unordered_map<PublicKey, ProgramAccount> &accounts) {
...
std::optional<AccountBalance>
TransactionBalances::get_balance(const PublicKey &address) const {
...
TransactionBalances::BalanceStats TransactionBalances::get_statistics() const {
  ...
  for (const auto &balance : balances_) {
    ...
    if (change > 0) {
      ...
      stats.max_decrease =
          std::max(stats.max_decrease, static_cast<Lamports>(-change));
    }
  }
...
Lamports TransactionBalances::get_account_balance(
...
void BalanceCollector::collect_pre_balances(
    const std::vector<PublicKey> &addresses,
    const std::unordered_map<PublicKey, ProgramAccount> &accounts) {
  if (!transaction_in_progress_) {
    start_transaction();
...
void BalanceCollector::collect_post_balances(
    const std::vector<PublicKey> &addresses,
    const std::unordered_map<PublicKey, ProgramAccount> &accounts) {
  if (!transaction_in_progress_) {
...
BalanceCollector::get_transaction_balances(size_t transaction_index) const {
...
bool BalanceCollector::has_unbalanced_transactions(
    const std::vector<Lamports> &fees) const {
...
BalanceCollector::CollectionStats BalanceCollector::get_collection_statistics(
    const std::vector<Lamports> &fees) const {
  ...
  for (size_t i = 0; i < all_balances_.size(); ++i) {
    ...
    for (const auto &balance : changed) {
      ...
      stats.total_balance_changes +=
          std::abs(static_cast<int64_t>(balance.balance_change()));
    }
  }
...
find_largest_changes(const TransactionBalances &balances, size_t limit) {
  auto changed = balances.get_changed_balances();
...
std::string format_balance_changes(const TransactionBalances &balances) {
  ...
  for (const auto &balance : changed) {
    ss << "  Account: "
       << std::string(balance.address.begin(), balance.address.begin() + 8)
       << "..." << " Change: " << static_cast<int64_t>(balance.balance_change())
       << " lamports" << " (" << balance.pre_balance << " -> "
       << balance.post_balance << ")\n";
  }

  auto stats = balances.get_statistics();
  ss << "Total change: " << static_cast<int64_t>(stats.total_change)
     << " lamports";
...
check_for_suspicious_changes(const TransactionBalances &balances) {
  ...
  for (const auto &balance : balances.get_changed_balances()) {
    ...
    if (change < -static_cast<int64_t>(LARGE_DECREASE_THRESHOLD)) {
      alerts.push_back(
          {balance.address, change, "Large balance decrease detected"});
    } else if (change > static_cast<int64_t>(LARGE_INCREASE_THRESHOLD)) {
      alerts.push_back(
          {balance.address, change, "Large balance increase detected"});
    }

    // Check for complete balance drain
    if (balance.pre_balance > 0 && balance.post_balance == 0) {
      alerts.push_back({balance.address, change, "Account completely drained"});
    }
  }
