...
void print_usage() {
  ...
  std::cout << "Options:\n";
  std::cout << "  --storage-path <path>  Key storage directory (default: "
               "~/.slonana/keys)\n";
...
int main(int argc, char *argv[]) {
  ...
  std::string storage_path =
      std::string(getenv("HOME") ? getenv("HOME") : ".") + "/.slonana/keys";
  ...
  if (!init_result.is_ok()) {
    std::cerr << "Failed to initialize key manager: " << init_result.error()
              << std::endl;
    return 1;
  }
  ...
  if (command == "generate") {
    ...
    if (result.is_ok()) {
      std::cout << "✅ Generated new validator identity: " << result.value()
                << std::endl;
    ...
    if (result.is_ok()) {
      std::cout << "✅ Imported validator identity: " << result.value()
                << std::endl;
    } else {
      std::cerr << "❌ Failed to import identity: " << result.error()
                << std::endl;
      return 1;
    }
    ...
    if (result.is_ok()) {
      std::cout << "✅ Rotated to new identity: " << result.value()
                << std::endl;
    } else {
      std::cerr << "❌ Failed to rotate identity: " << result.error()
                << std::endl;
      return 1;
    }
    ...
    if (result.is_ok()) {
      ...
      std::cout << "  Created: "
                << std::chrono::duration_cast<std::chrono::seconds>(
                       status.created_at.time_since_epoch())
                       .count()
                << " (unix timestamp)\n";
      std::cout << "  Expires: "
                << std::chrono::duration_cast<std::chrono::seconds>(
                       status.expires_at.time_since_epoch())
                       .count()
                << " (unix timestamp)\n";
      std::cout << "  Use Count: " << status.use_count << "\n";
      std::cout << "  Revoked: " << (status.is_revoked ? "yes" : "no") << "\n";
      std::cout << "  Needs Rotation: "
                << (status.needs_rotation ? "yes" : "no") << "\n";
    ...
    if (result.is_ok()) {
      ...
      std::cerr << "❌ Failed to list identities: " << result.error()
                << std::endl;
      return 1;
    }
    ...
    if (result.is_ok()) {
      ...
      std::cout << "⚠️  Remember to set proper file permissions: chmod 600 "
                << output_path << std::endl;
    } else {
      std::cerr << "❌ Failed to export identity: " << result.error()
                << std::endl;
      return 1;
    }
    ...
    if (result.is_ok()) {
      ...
      std::cerr << "❌ Failed to revoke identity: " << result.error()
                << std::endl;
      return 1;
    }
    ...
    if (key_manager) {
      ...
      if (audit_result.is_ok()) {
        ...
        std::cerr << "❌ Failed to generate audit report: "
                  << audit_result.error() << std::endl;
        return 1;
      }
    }
    ...
    if (key_manager) {
      ...
      if (cleanup_result.is_ok()) {
        ...
        std::cerr << "❌ Cleanup failed: " << cleanup_result.error()
                  << std::endl;
        return 1;
      }
    }
