...
SnapshotFinder::SnapshotFinder(const SnapshotFinderConfig& config)
    ...
    std::cout << "   • Max snapshot age: " << config_.max_snapshot_age << " slots" << std::endl;
    std::cout << "   • Min download speed: " << config_.min_download_speed << " MB/s" << std::endl;
}
...
std::vector<std::string> SnapshotFinder::get_default_rpc_endpoints(const std::string& network) {
...
std::vector<std::string> SnapshotFinder::get_snapshot_mirror_urls(const std::string& network) {
    if (network == "mainnet-beta" || network == "mainnet") {
        return {
            "https://snapshots.rpcpool.com",
            "https://snapshots.solana.com",
            "https://snapshot.solanalabs.com"
        };
    } else if (network == "testnet") {
        return {
            "https://snapshots.rpcpool.com/testnet",
            "https://snapshots.solana.com/testnet"
        };
    } else if (network == "devnet") {
        return {
            "https://snapshots.rpcpool.com/devnet",
...
common::Result<std::vector<RpcNodeInfo>> SnapshotFinder::discover_rpc_nodes() {
    ...
    size_t endpoints_per_thread = std::max(size_t(1), rpc_endpoints.size() / config_.threads_count);
    size_t actual_threads = std::min(static_cast<size_t>(config_.threads_count), rpc_endpoints.size());
    
    std::cout << "   • Testing " << rpc_endpoints.size() << " endpoints" << std::endl;
    ...
    for (size_t i = 0; i < actual_threads; ++i) {
        ...
        size_t end_idx = (i == actual_threads - 1) ? rpc_endpoints.size() : (i + 1) * endpoints_per_thread;
    ...
    std::cout << std::endl;
    std::cout << "✅ Discovery completed. Found " << discovered_nodes_.size() << " healthy nodes" << std::endl;
    ...
    std::sort(discovered_nodes_.begin(), discovered_nodes_.end(),
        [](const RpcNodeInfo& a, const RpcNodeInfo& b) {
            if (a.healthy != b.healthy) return a.healthy > b.healthy;
...
void SnapshotFinder::worker_thread_function(
    ...
    for (size_t i = start_index; i < end_index && !shutdown_requested_.load(); ++i) {
...
common::Result<RpcNodeInfo> SnapshotFinder::test_rpc_node(const std::string& rpc_url) {
    RpcNodeInfo node;
    node.url = rpc_url;
    ...
    try {
        ...
        node.latency_ms = std::chrono::duration<double, std::milli>(end_time - start_time).count();
        
        if (!slot_result.is_ok()) {
            return common::Result<RpcNodeInfo>("Failed to get slot: " + slot_result.error());
        }
        ...
        if (node.latency_ms > config_.max_latency) {
            return common::Result<RpcNodeInfo>("Latency too high: " + std::to_string(node.latency_ms) + "ms");
        }
        ...
        if (response.success) {
            ...
            std::string version_field = network::rpc_utils::extract_json_field(response.body, "result");
            if (!version_field.empty()) {
                node.version = network::rpc_utils::extract_json_field(version_field, "solana-core");
            }
        }
        
        // Check version criteria
        if (!meets_version_criteria(node.version)) {
            return common::Result<RpcNodeInfo>("Version does not meet criteria: " + node.version);
        }
        ...
        return common::Result<RpcNodeInfo>("Exception testing RPC: " + std::string(e.what()));
    }
}

common::Result<uint64_t> SnapshotFinder::get_current_slot(const std::string& rpc_url) {
    auto response = http_client_->solana_rpc_call(rpc_url, "getSlot", "[{\"commitment\":\"finalized\"}]");
    
    if (!response.success) {
        return common::Result<uint64_t>("HTTP request failed: " + response.error_message);
    }
    
    if (network::rpc_utils::is_rpc_error(response.body)) {
        return common::Result<uint64_t>("RPC error: " + network::rpc_utils::extract_error_message(response.body));
    }
    ...
    try {
        ...
        return common::Result<uint64_t>("Failed to parse slot number: " + std::string(e.what()));
    }
}

common::Result<uint64_t> SnapshotFinder::get_snapshot_slot(const std::string& rpc_url) {
    ...
    std::string full_slot = network::rpc_utils::extract_json_field(result_field, "full");
    if (!full_slot.empty()) {
        try {
...
bool SnapshotFinder::meets_version_criteria(const std::string& version) const {
    if (version.empty()) return true;  // Allow if version unknown
...
common::Result<std::vector<SnapshotQuality>> SnapshotFinder::find_best_snapshots() {
    ...
    if (!discovery_result.is_ok()) {
        return common::Result<std::vector<SnapshotQuality>>(discovery_result.error());
    }
    
    auto healthy_nodes = discovery_result.value();
    if (healthy_nodes.empty()) {
        return common::Result<std::vector<SnapshotQuality>>("No healthy RPC nodes found");
    }
    
    std::cout << "🧪 Testing snapshot quality from " << healthy_nodes.size() << " nodes..." << std::endl;
    ...
    for (const auto& node : healthy_nodes) {
        if (node.snapshot_slot == 0) continue;  // No snapshot available
        
        // Check snapshot age
        if (current_slot > 0 && !is_valid_snapshot_slot(node.snapshot_slot, current_slot, config_.max_snapshot_age)) {
            continue;
        }
        ...
        quality.age_slots = current_slot > node.snapshot_slot ? current_slot - node.snapshot_slot : 0;
        quality.download_url = build_snapshot_download_url(node.url, node.snapshot_slot);
        
        // Test download speed with small sample
        quality.download_speed_mbps = measure_download_speed(quality.download_url, 512 * 1024);  // 512KB test
    ...
    std::cout << "✅ Found " << snapshot_qualities_.size() << " quality snapshots" << std::endl;
...
double SnapshotFinder::measure_download_speed(const std::string& snapshot_url, size_t test_bytes) {
    if (snapshot_url.empty()) return 0.0;
    
    auto start_time = std::chrono::steady_clock::now();
    
    // Download a small portion to test speed
    auto response = http_client_->get(snapshot_url + "?range=0-" + std::to_string(test_bytes));
...
std::string SnapshotFinder::build_snapshot_download_url(const std::string& rpc_url, uint64_t slot) {
...
bool SnapshotFinder::meets_quality_criteria(const SnapshotQuality& quality) const {
    if (quality.download_speed_mbps < config_.min_download_speed) {
        return false;
    }
    
    if (config_.max_download_speed > 0.0 && quality.download_speed_mbps > config_.max_download_speed) {
        return false;
    }
...
bool SnapshotFinder::is_valid_snapshot_slot(uint64_t slot, uint64_t current_slot, uint64_t max_age) {
    if (current_slot <= slot) return true;  // Future or current slot
...
double SnapshotFinder::calculate_quality_score(const SnapshotQuality& quality) {
    ...
    double speed_score = std::min(quality.download_speed_mbps / 100.0, 1.0);  // Normalize to 0-1
    double latency_score = std::max(0.0, 1.0 - (quality.latency_ms / 1000.0));  // Lower latency is better
    double age_score = std::max(0.0, 1.0 - (quality.age_slots / 10000.0));  // Recent snapshots preferred
...
common::Result<bool> SnapshotFinder::download_snapshot_from_best_source(
    ...
    std::function<void(const std::string&, uint64_t, uint64_t)> progress_callback) {
    ...
    if (!best_result.is_ok()) {
        return common::Result<bool>("Failed to find best snapshot: " + best_result.error());
    }
    ...
    bool success = http_client_->download_file(best_quality.download_url, output_path, download_progress_cb);
    
    if (!success) {
        return common::Result<bool>("Download failed from: " + best_quality.download_url);
    }
    ...
    if (!fs::exists(output_path)) {
        return common::Result<bool>("Downloaded file does not exist: " + output_path);
    }
...
SnapshotFinderConfig SnapshotFinderCli::parse_config_from_args(int argc, char* argv[]) {
...
int SnapshotFinderCli::run_find_command(int argc, char* argv[]) {
    ...
    std::cout << "🔍 Slonana Snapshot Finder - Finding best snapshots..." << std::endl;
...
int SnapshotFinderCli::run_download_command(int argc, char* argv[]) {
    ...
    // Progress callback
    auto progress_cb = [](const std::string& phase, uint64_t current, uint64_t total) {
        if (total > 0) {
    ...
    auto result = finder.download_snapshot_from_best_source(output_dir, downloaded_path, progress_cb);
    std::cout << std::endl;
...
int SnapshotFinderCli::run_test_rpc_command(int argc, char* argv[]) {
    ...
    std::cout << "   Health: " << (node.healthy ? "✅ Healthy" : "❌ Unhealthy") << std::endl;
    std::cout << "   Latency: " << std::fixed << std::setprecision(1) << node.latency_ms << "ms" << std::endl;
    std::cout << "   Current Slot: " << node.slot << std::endl;
    if (node.snapshot_slot > 0) {
        std::cout << "   Snapshot Slot: " << node.snapshot_slot << std::endl;
        std::cout << "   Snapshot Age: " << (node.slot - node.snapshot_slot) << " slots" << std::endl;
    }
...
void SnapshotFinderCli::print_download_usage() {
    ...
    std::cout << "  --output-dir DIR              Output directory (default: ./snapshots)" << std::endl;
    ...
    std::cout << "  --network NETWORK             Network (mainnet, testnet, devnet)" << std::endl;
...
void SnapshotFinderCli::print_test_rpc_usage() {
    ...
    std::cout << "  RPC_URL                      RPC endpoint URL to test" << std::endl;
    std::cout << std::endl;
    std::cout << "Options:" << std::endl;
    std::cout << "  --max-latency MS             Maximum acceptable latency (default: 100)" << std::endl;
    std::cout << "  --version VERSION            Expected version filter" << std::endl;
    std::cout << "  -h, --help                   Show this help" << std::endl;
}

void SnapshotFinderCli::print_find_usage() {
    std::cout << "Usage: slonana snapshot-find [OPTIONS]" << std::endl;
    std::cout << "Find optimal Solana snapshots from multiple RPC sources" << std::endl;
    std::cout << std::endl;
    std::cout << "Options:" << std::endl;
    std::cout << "  -t, --threads-count COUNT     Number of concurrent threads (default: 100)" << std::endl;
    std::cout << "  -r, --rpc-address URL         Primary RPC for current slot (default: mainnet)" << std::endl;
    std::cout << "  --slot SLOT                   Target specific slot (default: latest)" << std::endl;
    std::cout << "  --version VERSION             Filter by exact version" << std::endl;
    std::cout << "  --wildcard-version VERSION    Filter by version pattern (e.g., '1.18')" << std::endl;
    std::cout << "  --max-snapshot-age SLOTS      Maximum snapshot age in slots (default: 1300)" << std::endl;
    std::cout << "  --min-download-speed MB/s     Minimum download speed (default: 60)" << std::endl;
    std::cout << "  --max-download-speed MB/s     Maximum download speed limit" << std::endl;
    std::cout << "  --max-latency MS              Maximum RPC latency in milliseconds (default: 100)" << std::endl;
    std::cout << "  --network NETWORK             Network (mainnet, testnet, devnet)" << std::endl;
    std::cout << "  --json                        Output in JSON format" << std::endl;
    std::cout << "  --with-private-rpc            Include private RPC endpoints" << std::endl;
    std::cout << "  --measurement                 Enable detailed measurement mode" << std::endl;
    std::cout << "  -h, --help                    Show this help" << std::endl;
}

void SnapshotFinderCli::print_snapshot_qualities(const std::vector<SnapshotQuality>& qualities, bool json_format) {
    if (json_format) {
        std::cout << "{\n  \"snapshots\": [" << std::endl;
        for (size_t i = 0; i < qualities.size(); ++i) {
            ...
            std::cout << "    }" << (i < qualities.size() - 1 ? "," : "") << std::endl;
        }
        std::cout << "  ]\n}" << std::endl;
    } else {
        ...
        for (size_t i = 0; i < qualities.size(); ++i) {
            ...
            std::cout << "   Speed: " << std::fixed << std::setprecision(1) << q.download_speed_mbps << " MB/s, ";
            std::cout << "Latency: " << q.latency_ms << "ms, ";
            std::cout << "Age: " << q.age_slots << " slots" << std::endl;
            std::cout << "   Quality Score: " << std::setprecision(3) << q.quality_score << std::endl;
