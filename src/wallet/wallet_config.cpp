...
std::string WalletConfigManager::validate_config(const HardwareWalletConfig& config) {
    // Basic validation
    ...
    // Security validation
    if (config.default_security.max_signing_attempts == 0 || config.default_security.max_signing_attempts > 10) {
    ...
    for (const auto& device : config.devices) {
        ...
        if (!ConfigValidator::validate_derivation_path(device.default_derivation_path)) {
            return "Invalid derivation path for device " + device.device_id + ": " + device.default_derivation_path;
        }
    }
...
HardwareWalletConfig WalletConfigManager::merge_configs(
    ...
    // Merge device configurations (override devices replace base devices with same ID)
...
std::optional<DeviceConfig> WalletConfigManager::get_device_config(
...
void WalletConfigManager::set_device_config(
    HardwareWalletConfig& config,
...
bool WalletConfigManager::remove_device_config(
    HardwareWalletConfig& config,
    const std::string& device_id
) {
...
ValidationResult ConfigValidator::validate(const HardwareWalletConfig& config) {
    ...
    for (size_t i = 0; i < config.devices.size(); ++i) {
        ...
        if (!device_result.is_valid()) {
            device_result.field_path = "devices[" + std::to_string(i) + "]." + device_result.field_path;
...
ValidationResult ConfigValidator::validate_security(const SecurityConfig& security) {
    ValidationResult result;
    
    if (security.max_signing_attempts == 0 || security.max_signing_attempts > 10) {
        result.error = ConfigError::INVALID_VALUE_RANGE;
...
ValidationResult ConfigValidator::validate_device(const DeviceConfig& device) {
    ...
    if (!validate_derivation_path(device.default_derivation_path)) {
        ...
        result.message = "Invalid derivation path: " + device.default_derivation_path;
...
std::string ConfigValidator::get_error_message(ConfigError error) {
    switch (error) {
        case ConfigError::NONE: return "No error";
        case ConfigError::INVALID_JSON: return "Invalid JSON format";
        case ConfigError::MISSING_REQUIRED_FIELD: return "Missing required field";
        case ConfigError::INVALID_VALUE_RANGE: return "Value out of valid range";
        case ConfigError::INVALID_DERIVATION_PATH: return "Invalid derivation path format";
        case ConfigError::DUPLICATE_DEVICE_ID: return "Duplicate device identifier";
        case ConfigError::INVALID_TIMEOUT_VALUE: return "Invalid timeout value";
        case ConfigError::UNSUPPORTED_SECURITY_SETTING: return "Unsupported security setting";
