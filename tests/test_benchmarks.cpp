...
private:
  ...
  struct BenchmarkResult {
    ...
    void print() const {
      std::cout << std::setw(30) << std::left << name
                << " | Latency: " << std::setw(8) << std::right << std::fixed
                << std::setprecision(2) << avg_latency_us << "μs"
                << " | Throughput: " << std::setw(8) << std::right << std::fixed
                << std::setprecision(0) << throughput_ops_per_sec << " ops/s"
                << " | Memory: " << std::setw(6) << std::right << std::fixed
                << std::setprecision(1) << memory_usage_mb << "MB" << std::endl;
    }
  };
  ...
  BenchmarkResult measure_performance(const std::string &name, Func &&func,
...
public:
  ...
  void run_all_benchmarks() {
    ...
    std::cout << "Comparing with Anza/Agave Reference Implementation"
              << std::endl;
...
private:
  void benchmark_account_operations() {
    ...
    auto create_result = measure_performance(
        "Account Creation",
        [&]() {
    ...
    auto lookup_result = measure_performance(
        "Account Lookup",
        [&]() {
          auto &account = test_accounts[rng_() % test_accounts.size()];
    ...
    auto update_result = measure_performance(
        "Account Update",
        [&]() {
          auto &account = test_accounts[rng_() % test_accounts.size()];
    ...
    auto program_query_result = measure_performance(
  ...
  void benchmark_transaction_processing() {
    ...
    execution_engine->register_builtin_program(
        std::make_unique<SystemProgram>());
    ...
    auto single_instr_result = measure_performance(
        "Single Instruction",
        [&]() {
    ...
    auto multi_instr_result = measure_performance(
        "Multi-Instruction Tx",
        [&]() {
          std::vector<Instruction> transaction;
          for (int i = 0; i < 5; ++i) { // 5 instructions per transaction
            transaction.push_back(
                test_instructions[rng_() % test_instructions.size()]);
          }
    ...
    auto compute_result = measure_performance(
        "Compute Intensive",
        [&]() {
          ...
          heavy_instr.data =
              generate_random_bytes(1024); // Large instruction data
  ...
  void benchmark_block_processing() {
    ...
    auto block_creation_result = measure_performance(
        "Block Creation",
        [&]() {
    ...
    auto block_validation_result = measure_performance(
        "Block Validation",
        [&]() {
          // Simulate block validation
          std::vector<uint8_t> block_data =
              generate_random_bytes(1024 * 1024); // 1MB block
    ...
    auto slot_processing_result = measure_performance(
        "Slot Processing",
        [&]() {
          // Simulate processing a slot with multiple blocks
          for (int i = 0; i < 3; ++i) { // 3 blocks per slot average
            auto block_data =
                generate_random_bytes(512 * 1024); // 512KB per block
  ...
  void benchmark_rpc_performance() {
    ...
    auto account_info_result = measure_performance(
        "getAccountInfo RPC",
        [&]() {
          ...
          std::string request =
              R"({"jsonrpc":"2.0","id":"test","method":"getAccountInfo","params":[")" +
              std::string(reinterpret_cast<const char *>(pubkey.data()), 32) +
              R"("]})";

          // Simulate response generation
          std::string response =
              R"({"jsonrpc":"2.0","result":{"context":{"slot":)" +
              std::to_string(rng_() % 1000000) +
    ...
    auto balance_result = measure_performance(
        "getBalance RPC",
        [&]() {
          auto pubkey = generate_random_pubkey();
          ...
          std::string request =
              R"({"jsonrpc":"2.0","id":"test","method":"getBalance","params":[")" +
              std::string(reinterpret_cast<const char *>(pubkey.data()), 32) +
              R"("]})";
          ...
          std::string response =
              R"({"jsonrpc":"2.0","result":{"context":{"slot":)" +
              std::to_string(rng_() % 1000000) + R"(},"value":)" +
              std::to_string(balance) + R"(},"id":"test"})";
        },
        10000);
    results_.push_back(balance_result);
    balance_result.print();

    auto block_result = measure_performance(
        "getBlock RPC",
        [&]() {
          ...
          std::string response =
              R"({"jsonrpc":"2.0","result":{"blockHeight":)" +
              std::to_string(slot) + R"(,"blockTime":)" +
              std::to_string(
                  std::chrono::duration_cast<std::chrono::seconds>(
                      std::chrono::system_clock::now().time_since_epoch())
                      .count()) +
    ...
    auto program_accounts_result = measure_performance(
        "getProgramAccounts RPC",
        [&]() {
  ...
  void benchmark_network_operations() {
    ...
    auto gossip_message_result = measure_performance(
        ...
        [&]() {
          ...
          bool should_propagate =
              (message_data[0] % 3) == 0; // 33% propagation rate
    ...
    auto peer_discovery_result = measure_performance(
        "Peer Discovery",
        [&]() {
          // Simulate peer discovery and connection establishment
          std::vector<std::string> potential_peers;
          for (int i = 0; i < 10; ++i) {
            potential_peers.push_back("192.168.1." + std::to_string(100 + i) +
                                      ":8001");
          }
    ...
    auto tx_broadcast_result = measure_performance(
  ...
  void benchmark_consensus_operations() {
    ...
    auto vote_processing_result = measure_performance(
        "Vote Processing",
        [&]() {
    ...
    auto leader_schedule_result = measure_performance(
        "Leader Schedule Calc",
        [&]() {
    ...
    auto poh_verification_result = measure_performance(
        "PoH Verification",
        [&]() {
  ...
  void benchmark_memory_efficiency() {
    ...
    auto memory_allocation_result = measure_performance(
        "Memory Allocation",
        [&]() {
    ...
    auto account_cache_result = measure_performance(
        "Account Cache Access",
        [&]() {
  ...
  void benchmark_concurrency() {
    ...
    auto parallel_tx_result = measure_performance(
        "Parallel Transaction Proc",
        [&]() {
    ...
    auto concurrent_rpc_result = measure_performance(
  ...
  void print_summary_report() {
    ...
    std::cout << "  Average Latency: " << std::fixed << std::setprecision(2)
              << avg_latency << "μs" << std::endl;
    std::cout << "  Average Throughput: " << std::fixed << std::setprecision(0)
              << avg_throughput << " ops/s" << std::endl;
    std::cout << "  Total Memory Delta: " << std::fixed << std::setprecision(1)
  ...
  void generate_comparison_report() {
    ...
    for (const auto &result : results_) {
      ...
      // Estimated Agave performance (these would be real measurements in
      // practice)
    ...
    std::cout << std::setw(30) << std::left << "Operation" << " | "
              ...
              << std::setw(10) << std::right << "Ratio" << " | Status"
              << std::endl;
    std::cout << std::string(80, '-') << std::endl;
    ...
    for (const auto &comp : comparisons) {
      ...
      std::cout << std::setw(30) << std::left << comp.operation << " | "
                ...
                << std::setprecision(0) << comp.slonana_throughput << "op/s"
                << " | " << std::setw(10) << std::right << std::fixed
                << std::setprecision(0) << comp.agave_throughput << "op/s"
                << " | " << std::setw(8) << std::right << std::fixed
                << std::setprecision(2) << comp.performance_ratio << "x"
                << " | " << status << std::endl;
    ...
    std::cout << "  Better Performance: " << better_count << " operations"
              << std::endl;
    std::cout << "  Similar Performance: " << similar_count << " operations"
              << std::endl;
    std::cout << "  Worse Performance: " << worse_count << " operations"
              << std::endl;
    std::cout << "  Average Performance Ratio: " << std::fixed
              << std::setprecision(2) << avg_ratio << "x" << std::endl;

    if (avg_ratio >= 1.2) {
      std::cout
          << "  🎉 OUTSTANDING - slonana.cpp significantly outperforms Agave!"
          << std::endl;
    } else if (avg_ratio >= 1.0) {
      std::cout << "  ✅ EXCELLENT - slonana.cpp matches or exceeds Agave "
                   "performance!"
                << std::endl;
    } else if (avg_ratio >= 0.8) {
      std::cout
          << "  👍 GOOD - slonana.cpp performance is competitive with Agave"
          << std::endl;
    } else {
      std::cout
          << "  📈 OPTIMIZATION NEEDED - Performance gap with Agave identified"
          << std::endl;
    }

    std::cout << "\n* Agave performance estimates based on public benchmarks "
                 "and documentation"
              << std::endl;
    std::cout << "  For precise comparison, run identical benchmarks against "
                 "live Agave validator"
              << std::endl;
  ...
  void save_benchmark_results() {
    ...
    results_file << "{\n";
    results_file << "  \"benchmark_timestamp\": \""
                 << std::chrono::duration_cast<std::chrono::seconds>(
                        std::chrono::system_clock::now().time_since_epoch())
                        .count()
                 << "\",\n";
    ...
    for (size_t i = 0; i < results_.size(); ++i) {
      ...
      results_file << "      \"avg_latency_us\": " << result.avg_latency_us
                   << ",\n";
      results_file << "      \"throughput_ops_per_sec\": "
...
int main() {
  std::cout
      << "🚀 Starting comprehensive benchmarks for slonana.cpp validator..."
      << std::endl;
  ...
  std::cout << "📈 Compare these results with Anza/Agave validator performance "
               "metrics"
            << std::endl;
  std::cout
      << "🔧 Use these insights to optimize critical performance bottlenecks"
      << std::endl;
