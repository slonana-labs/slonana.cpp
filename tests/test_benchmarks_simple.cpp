...
private:
  void benchmark_core_operations() {
    ...
    // Hash operations
    auto hash_result = measure_performance(
        "Hash Generation",
        [&]() {
    ...
    // Data serialization
    auto serialize_result = measure_performance(
        "Data Serialization",
        [&]() {
    ...
    // Data parsing
    auto parse_result = measure_performance(
        "Data Parsing",
        [&]() {
  ...
  void benchmark_crypto_operations() {
    ...
    auto sig_verify_result = measure_performance(
    ...
    auto hash_chain_result = measure_performance(
        "Hash Chain Verification",
        [&]() {
          ...
          for (int i = 0; i < 100; ++i) {
            ...
            for (int j = 0; j < 32; ++j) {
              current_hash[j] =
                  static_cast<uint8_t>((current_hash[j] + i + j) % 256);
            }
          }
        },
        1000);
    ...
    auto merkle_result = measure_performance(
  ...
  void benchmark_data_structures() {
    ...
    auto queue_result = measure_performance(
        "Transaction Queue Ops",
        [&]() {
    ...
    // Vote tracking
    auto vote_result = measure_performance(
        "Vote Tracking",
        [&]() {
  ...
  void benchmark_network_simulation() {
    ...
    auto msg_serialize_result = measure_performance(
        "Message Serialization",
        [&]() {
    ...
    auto gossip_result = measure_performance(
        "Gossip Propagation",
        [&]() {
          std::vector<std::string> peers = {"peer1", "peer2", "peer3", "peer4",
                                            "peer5"};
  ...
  void benchmark_memory_operations() {
    ...
    auto alloc_result = measure_performance(
        "Memory Allocation",
        [&]() {
    ...
    // Cache simulation
    auto cache_result = measure_performance(
        "Cache Access",
        [&]() {
  ...
  void benchmark_json_processing() {
    ...
    auto json_parse_result = measure_performance(
        "JSON-RPC Parsing",
        [&]() {
          std::string request = R"({
                "jsonrpc": "2.0",
                "id": "test-)" + std::to_string(rng_() % 10000) +
                                R"(",
                "method": "getAccountInfo",
                "params": [")" + std::to_string(rng_() % 1000000) +
                                R"("]
            })";
          ...
          bool valid =
              (jsonrpc_pos != std::string::npos &&
               id_pos != std::string::npos && method_pos != std::string::npos &&
    ...
    auto json_response_result = measure_performance(
        "JSON Response Generation",
        [&]() {
          ...
          response << R"({"jsonrpc":"2.0","result":{"context":{"slot":)" << slot
                   << R"(},"value":)" << balance << R"(},"id":"test"})";
