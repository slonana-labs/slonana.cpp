...
public:
    BpfRuntimeTester() : rng_(std::chrono::steady_clock::now().time_since_epoch().count()) {}
    ...
    bool test_complete_isa_coverage() {
        ...
        std::vector<InstructionTest> instruction_tests = {
            ...
            {0x07, "ADD64_IMM", {0x07, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00}, true},
            {0x0f, "ADD64_REG", {0x0f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            {0x17, "SUB64_IMM", {0x17, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00}, true},
            {0x1f, "SUB64_REG", {0x1f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            {0x27, "MUL64_IMM", {0x27, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00}, true},
            {0x2f, "MUL64_REG", {0x2f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            {0x37, "DIV64_IMM", {0x37, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00}, true},
            {0x3f, "DIV64_REG", {0x3f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            
            // Bitwise operations
            {0x47, "OR64_IMM", {0x47, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00}, true},
            {0x4f, "OR64_REG", {0x4f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            {0x57, "AND64_IMM", {0x57, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00}, true},
            {0x5f, "AND64_REG", {0x5f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            {0x67, "LSH64_IMM", {0x67, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00}, true},
            {0x6f, "LSH64_REG", {0x6f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            {0x77, "RSH64_IMM", {0x77, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00}, true},
            {0x7f, "RSH64_REG", {0x7f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            
            // Memory operations
            {0x18, "LDDW", {0x18, 0x00, 0x00, 0x00, 0x12, 0x34, 0x56, 0x78, 0x00, 0x00, 0x00, 0x00, 0x9a, 0xbc, 0xde, 0xf0}, true},
            ...
            {0x15, "JEQ_IMM", {0x15, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            {0x1d, "JEQ_REG", {0x1d, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            {0x25, "JGT_IMM", {0x25, 0x00, 0x01, 0x00, 0x10, 0x00, 0x00, 0x00}, true},
            {0x2d, "JGT_REG", {0x2d, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            
            // Exit instruction
            {0x95, "EXIT", {0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, true},
            
            // Invalid instructions (should fail verification)
            {0xFF, "INVALID", {0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, false}
        };
        ...
        for (const auto& test : instruction_tests) {
            ...
            if (verification_result == test.should_succeed) {
                std::cout << "  ✅ " << test.name << " - verification as expected" << std::endl;
                passed++;
            } else {
                std::cout << "  ❌ " << test.name << " - unexpected verification result" << std::endl;
            }
        }
        
        double success_rate = (double)passed / total * 100.0;
        std::cout << "ISA Coverage: " << passed << "/" << total << " (" << std::fixed << std::setprecision(1) << success_rate << "%)" << std::endl;
    ...
    bool test_memory_management_safety() {
        ...
        std::vector<MemoryTest> memory_tests = {
            {
                "Valid Stack Access",
                {
                    0x18, 0x01, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Load stack address
            ...
            },
            {
                "Stack Overflow",
                {
                    0x18, 0x01, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Load invalid address
                    0x7b, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Try to store (should fail)
                    0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  // Exit
                },
                false,
            ...
            },
            {
                ...
                {
                    0x18, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Load null
                    0x79, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Try to dereference (should fail)
                    0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  // Exit
                },
                false,
            ...
            },
            {
                ...
                {
                    0x18, 0x01, 0x00, 0x00, 0x01, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Load unaligned address
                    0x79, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Try unaligned access
                    0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  // Exit
                },
                false,
        ...
        for (const auto& test : memory_tests) {
            ...
            if (execution_succeeded == test.should_succeed) {
                ...
                std::cout << "  ❌ " << test.name << " - unexpected result: " << test.description << std::endl;
            }
        }
        
        double success_rate = (double)passed / total * 100.0;
        std::cout << "Memory Safety: " << passed << "/" << total << " (" << std::fixed << std::setprecision(1) << success_rate << "%)" << std::endl;
    ...
    bool test_jit_compilation_optimization() {
        ...
        auto interpreter_time = std::chrono::high_resolution_clock::now() - start_time;
        ...
        auto interpreter_us = std::chrono::duration_cast<std::chrono::microseconds>(interpreter_time).count();
        auto jit_us = std::chrono::duration_cast<std::chrono::microseconds>(jit_time).count();
        
        std::cout << "  Interpreter execution: " << interpreter_us << "μs" << std::endl;
        std::cout << "  JIT execution: " << jit_us << "μs" << std::endl;
        
        if (jit_us > 0) {
            double speedup = (double)interpreter_us / jit_us;
            std::cout << "  JIT speedup: " << std::fixed << std::setprecision(2) << speedup << "x" << std::endl;
            
            bool results_match = interpreter_result.return_value == jit_result.return_value;
            bool jit_faster = jit_us < interpreter_us || speedup > 1.5; // Allow for JIT warmup
            
            if (results_match && jit_faster) {
                std::cout << "  ✅ JIT compilation working correctly" << std::endl;
                return true;
            } else {
                std::cout << "  ❌ JIT issues: results_match=" << results_match << ", faster=" << jit_faster << std::endl;
                return false;
            }
        } else {
            std::cout << "  ⚠️  JIT compilation not available or failed" << std::endl;
            return interpreter_result.is_success(); // At least interpreter should work
        }
    }
    ...
    bool test_parallel_execution() {
        ...
        auto sequential_ms = std::chrono::duration_cast<std::chrono::milliseconds>(sequential_time).count();
        auto parallel_ms = std::chrono::duration_cast<std::chrono::milliseconds>(parallel_time).count();
        
        std::cout << "  Sequential execution: " << sequential_ms << "ms" << std::endl;
        std::cout << "  Parallel execution: " << parallel_ms << "ms" << std::endl;
        
        if (parallel_ms > 0) {
            double speedup = (double)sequential_ms / parallel_ms;
            std::cout << "  Parallel speedup: " << std::fixed << std::setprecision(2) << speedup << "x" << std::endl;
            ...
            if (all_consistent && speedup > 1.5) {
                ...
                std::cout << "  ❌ Parallel execution issues: consistent=" << all_consistent 
    ...
    bool test_security_model() {
        ...
        std::vector<SecurityTest> security_tests = {
            {
                "Infinite Loop Protection",
                {
                    ...
                    0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  // Exit (unreachable)
                },
                false, false
            },
            {
                ...
                true, false // Should pass verification but fail execution due to compute limit
            },
            {
                "Division by Zero",
                {
                    ...
                    0x3f, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // r0 /= r1 (divide by zero)
                    0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  // exit
                },
                true, false // Should fail at runtime
            },
            {
                "Valid Program",
                {
                ...
                true, true // Should pass both verification and execution
            }
        };
        ...
        for (const auto& test : security_tests) {
            ...
            bool test_passed = (verification_passed == test.should_pass_verification) &&
            ...
            if (test_passed) {
                ...
                std::cout << "  ❌ " << test.name << " (verification: " << verification_passed 
                         << ", execution: " << execution_passed << ")" << std::endl;
            }
        }
        
        double success_rate = (double)passed / total * 100.0;
        std::cout << "Security Model: " << passed << "/" << total << " (" << std::fixed << std::setprecision(1) << success_rate << "%)" << std::endl;
    ...
    bool test_performance_benchmarking() {
        ...
        std::vector<BenchmarkTest> benchmarks = {
            ...
            },
            {
                "Memory Operations",
                {
                    0x18, 0x01, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Load address
                    0xb7, 0x02, 0x00, 0x00, 0x42, 0x00, 0x00, 0x00, // r2 = 0x42
                    0x7b, 0x21, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Store r2 to memory
                    0x79, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Load from memory
        ...
        for (const auto& benchmark : benchmarks) {
            ...
            for (int i = 0; i < iterations; ++i) {
                ...
                if (!result.is_success()) {
                    std::cout << "  ❌ " << benchmark.name << " - execution failed" << std::endl;
                    break;
                }
            }
            ...
            if (elapsed_us > 0) {
                uint64_t ops_per_sec = (uint64_t)((double)iterations * 1000000.0 / elapsed_us);
                std::cout << "  " << benchmark.name << ": " << ops_per_sec << " ops/sec";
                
                if (ops_per_sec >= benchmark.expected_min_ops_per_sec) {
                    std::cout << " ✅" << std::endl;
                    passed++;
                } else {
                    std::cout << " ❌ (minimum: " << benchmark.expected_min_ops_per_sec << ")" << std::endl;
                }
                
                total_ops_per_sec += ops_per_sec;
            } else {
                std::cout << "  ❌ " << benchmark.name << " - timing failed" << std::endl;
            }
        }
        
        if (total > 0) {
            uint64_t avg_ops_per_sec = total_ops_per_sec / total;
            std::cout << "Average performance: " << avg_ops_per_sec << " ops/sec" << std::endl;
        }
        
        double success_rate = (double)passed / total * 100.0;
        std::cout << "Performance Benchmarks: " << passed << "/" << total << " (" << std::fixed << std::setprecision(1) << success_rate << "%)" << std::endl;
