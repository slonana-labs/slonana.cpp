...
bool test_circuit_breaker_race_condition() {
  std::cout << "\n=== Testing Circuit Breaker Race Condition Fix ==="
            << std::endl;
  ...
  for (int t = 0; t < num_threads; ++t) {
    threads.emplace_back([&, t]() {
      for (int i = 0; i < operations_per_thread; ++i) {
        ...
        auto result =
            breaker.execute([&]() { return test_operation(should_fail); });

        if (result.is_err() && result.error().find("Circuit breaker is OPEN") !=
                                   std::string::npos) {
          circuit_open_count++;
        }
  ...
  std::cout << "Circuit open blocks: " << circuit_open_count.load()
            << std::endl;
  std::cout << "Final circuit state: " << static_cast<int>(breaker.get_state())
            << std::endl;

  // The test passes if we don't crash (race condition would cause
  // crashes/undefined behavior) and if the circuit breaker shows some blocked
  // operations
  if (circuit_open_count.load() > 0) {
    std::cout << "✅ Circuit breaker race condition fix working - blocked "
                 "operations when OPEN"
              << std::endl;
    return true;
  } else {
    std::cout
        << "⚠️  Circuit breaker didn't reach OPEN state in multithreaded test"
        << std::endl;
...
bool test_degradation_manager_improved_matching() {
  std::cout << "\n=== Testing Degradation Manager Improved Matching ==="
            << std::endl;
  ...
  std::vector<std::string> should_pass = {
      "read",        "get",         "query",        "read_account",
      "get_balance", "query_block", "account_read", "balance_get"};

  // Test cases that should fail in READ_ONLY mode (previous bug would allow
  // these)
  std::vector<std::string> should_fail = {"create",
                                          "update",
                                          "delete",
                                          "write",
  ...
  for (const auto &op : should_pass) {
    if (!manager.is_operation_allowed("test", op)) {
      std::cout << "❌ Operation '" << op
                << "' should be allowed in READ-only mode" << std::endl;
      all_correct = false;
    }
  }
  ...
  for (const auto &op : should_fail) {
    if (manager.is_operation_allowed("test", op)) {
      std::cout << "❌ Operation '" << op
                << "' should be blocked in read-only mode" << std::endl;
      all_correct = false;
    }
  }

  if (all_correct) {
    std::cout << "✅ Degradation manager improved matching working correctly"
              << std::endl;
  }
...
bool test_jitter_overflow_protection() {
  ...
  policy.initial_delay =
      std::chrono::milliseconds(10000); // Large initial delay
  ...
  try {
    ...
    if (duration < std::chrono::milliseconds(30000)) { // 30 seconds max
      ...
      std::cout << "❌ Jitter calculation took too long: " << duration.count()
                << "ms" << std::endl;
      return false;
    }
...
int main() {
  ...
  if (all_passed) {
    ...
    std::cout
        << "🎉 Critical race conditions and security issues have been resolved!"
        << std::endl;
    return 0;
  } else {
