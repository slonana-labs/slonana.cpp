...
void test_network_type_utilities() {
  ...
  ASSERT_EQ(ClusterConnection::network_type_to_string(NetworkType::MAINNET),
            "mainnet");
  ASSERT_EQ(ClusterConnection::network_type_to_string(NetworkType::TESTNET),
            "testnet");
  ASSERT_EQ(ClusterConnection::network_type_to_string(NetworkType::DEVNET),
            "devnet");
  ASSERT_EQ(ClusterConnection::network_type_to_string(NetworkType::LOCALNET),
            "localnet");

  // Test reverse conversion
  ASSERT_TRUE(ClusterConnection::string_to_network_type("mainnet") ==
              NetworkType::MAINNET);
  ASSERT_TRUE(ClusterConnection::string_to_network_type("testnet") ==
              NetworkType::TESTNET);
  ASSERT_TRUE(ClusterConnection::string_to_network_type("devnet") ==
              NetworkType::DEVNET);
  ASSERT_TRUE(ClusterConnection::string_to_network_type("localnet") ==
              NetworkType::LOCALNET);

  // Test bootstrap nodes
  auto mainnet_nodes =
      ClusterConnection::get_default_bootstrap_nodes(NetworkType::MAINNET);
  ASSERT_GT(mainnet_nodes.size(), 0);

  auto devnet_nodes =
      ClusterConnection::get_default_bootstrap_nodes(NetworkType::DEVNET);
...
void test_cluster_utilities() {
  ...
  ASSERT_TRUE(cluster_utils::is_valid_cluster_address(
  ...
  auto handshake =
      cluster_utils::create_handshake_message("test_node", "1.0.0");
...
void test_cluster_message_handling() {
  ...
  bool block_announced =
      connection.announce_block(100, "test_block_hash", {0x05, 0x06, 0x07});
  // Note: May return false if no connections yet, which is OK for this test
...
void test_cluster_discovery() {
  ...
  // Test getting connected nodes (may be empty if bootstrap nodes are not
  // running)
...
int main() {
  ...
  try {
    ...
    std::cout << "✅ Multi-network support (mainnet/testnet/devnet/localnet)"
              << std::endl;
    ...
    std::cout << "🔄 Automatic peer discovery and heartbeat management"
              << std::endl;
