...
struct MockTick {
  ...
  MockTick(uint64_t s, uint64_t t)
      : slot(s), tick_index(t), hash(32, 0),
        timestamp(std::chrono::system_clock::now()), duration_us(400) {
...
public:
  ...
  void generate_ticks_for_slot(uint64_t slot) {
    ...
    for (uint64_t tick = 0; tick < ticks_per_slot_; ++tick) {
      ...
      std::this_thread::sleep_for(
          std::chrono::microseconds(target_tick_duration_us_ / 100));
  ...
  void simulate_byzantine_behavior(const std::string &validator_pubkey,
                                   int behavior_type) {
    ...
    case 1: // Double voting
    {
      ...
      MockVote vote2(validator_pubkey, current_slot - 1,
                     validator.stake_amount); // Conflicting vote
    ...
    case 2: // Voting on future slots
    {
      ...
      MockVote future_vote(validator_pubkey, future_slot,
  ...
  void slash_validator(const std::string &validator_pubkey,
                       double penalty_rate) {
    auto it = validator_index_.find(validator_pubkey);
    if (it == validator_index_.end())
      return;
    ...
    uint64_t penalty =
        static_cast<uint64_t>(validator.stake_amount * penalty_rate);
  ...
  uint64_t get_total_votes_processed() const {
    return total_votes_processed_.load();
  }
  uint64_t get_total_ticks_generated() const {
    return total_ticks_generated_.load();
  }
  ...
  double get_average_tick_duration() const {
    if (total_ticks_generated_.load() == 0)
      return 0.0;
    // This would be calculated from actual tick timestamps in a real
    // implementation
...
private:
  ...
  void distribute_epoch_rewards(MockEpoch &epoch) {
    ...
    for (auto &validator : validators_) {
      ...
      double stake_ratio =
          (double)validator.stake_amount / total_active_stake_.load();
...
void test_consensus_supermajority_detection() {
  ...
  // Note: In this simplified test, we would need to advance slots to see the
  // confirmation
...
void test_consensus_leader_rotation() {
  ...
  std::cout << "✅ Leader rotation working across " << leaders_seen.size()
            << " validators" << std::endl;
}
...
void test_consensus_slashing_mechanism() {
  ...
  uint64_t expected_penalty =
      static_cast<uint64_t>(bad_validator_initial_stake * 0.05);
  uint64_t expected_final_stake =
      bad_validator_initial_stake - expected_penalty;

  ASSERT_EQ(bad_validator_final_stake, expected_final_stake);
  ASSERT_TRUE(
      bad_validator_still_active); // Should still be active with 5% penalty
...
void test_consensus_performance_metrics() {
  ...
  std::cout << "  - Average tick duration: " << avg_tick_duration << "μs"
            << std::endl;
  ...
  std::cout << "  - Vote processing rate: " << votes_per_second << " votes/sec"
            << std::endl;
  std::cout << "  - Tick generation rate: " << ticks_per_second << " ticks/sec"
            << std::endl;
...
void test_consensus_parallel_voting() {
  ...
  std::cout << "  - Throughput: " << votes_per_second << " votes/sec"
            << std::endl;
...
void run_consensus_mechanism_tests(TestRunner &runner) {
  runner.run_test("Consensus Basic Operations",
                  test_consensus_basic_operations);
  runner.run_test("Consensus Vote Processing", test_consensus_vote_processing);
  runner.run_test("Consensus Supermajority Detection",
                  test_consensus_supermajority_detection);
