...
void test_performance_analyzer() {
    ...
    auto& metrics = monitoring::GlobalConsensusMetrics::instance();
    metrics.increment_blocks_processed();
    metrics.record_block_validation_time(0.025); // 25ms - well within 100ms target
    
    // Generate performance report
    auto report = analyzer.generate_report(std::chrono::minutes(1));
    
    // Debug: Print performance values
    std::cout << "Debug - Block validation time: " << report.avg_block_validation_time_ms << "ms" << std::endl;
    std::cout << "Debug - Blocks processed: " << report.total_blocks_processed << std::endl;
    std::cout << "Debug - Blocks per second: " << report.blocks_per_second << std::endl;
    ...
    ASSERT_TRUE(json_report.find("consensus_performance_report") != std::string::npos);
...
void test_prometheus_exporter() {
    ...
    auto counter = registry.counter("test_counter", "Test counter for Prometheus export");
...
void run_consensus_timing_tests(TestRunner& runner) {
    ...
    runner.run_test("Consensus Metrics Initialization", test_consensus_metrics_initialization);
...
int main() {
    ...
    std::cout << "\nConsensus timing tests completed: " 
              << (runner.all_passed() ? "all passed" : "some failed") << std::endl;
