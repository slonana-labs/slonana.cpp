...
bool test_retry_mechanism() {
  ...
  auto result = FaultTolerance::retry_with_backoff(
      [&simulator]() { return simulator.unreliable_operation(); }, policy);
  ...
  if (simulator.get_call_count() != 3) {
    std::cout << "❌ Expected 3 calls, got " << simulator.get_call_count()
              << std::endl;
    return false;
  }

  std::cout << "✅ Retry mechanism working - succeeded after "
            << simulator.get_call_count() << " attempts" << std::endl;

  // Test max retry limit
  simulator.reset();
  auto fail_result = FaultTolerance::retry_with_backoff(
      [&simulator]() { return simulator.always_fail_operation(); }, policy);
...
bool test_circuit_breaker() {
  ...
  for (int i = 0; i < 3; ++i) {
    auto result = breaker.execute(
        [&simulator]() { return simulator.always_fail_operation(); });
    if (result.is_ok()) {
  ...
  std::cout << "✅ Circuit breaker opened after " << config.failure_threshold
            << " failures" << std::endl;

  // Test that circuit stays open
  auto blocked_result = breaker.execute(
      [&simulator]() { return simulator.unreliable_operation(); });
...
bool test_degradation_manager() {
  ...
  if (!manager.is_operation_allowed("rpc", "read_account")) {
    std::cout << "❌ Read operations should be allowed in NORMAL mode"
              << std::endl;
    return false;
  }
...
bool test_checkpoint_basic() {
  ...
  if (!save_result.is_ok()) {
    std::cout << "❌ Failed to save checkpoint: " << save_result.error()
              << std::endl;
    return false;
  }
  ...
  if (!verify_result.is_ok()) {
    std::cout << "❌ Failed to verify checkpoint: " << verify_result.error()
              << std::endl;
    return false;
  }
  ...
  if (!data_save_result.is_ok()) {
    std::cout << "❌ Failed to save data: " << data_save_result.error()
              << std::endl;
    return false;
  }
  ...
  if (!data_load_result.is_ok()) {
    std::cout << "❌ Failed to load data: " << data_load_result.error()
              << std::endl;
    return false;
  }
...
bool test_retryable_error_detection() {
  ...
  std::vector<std::string> retryable_errors = {
      "Connection timeout", "Network unavailable", "Service temporarily busy",
      "Rate limit exceeded"};
  ...
  std::vector<std::string> non_retryable_errors = {
      "Invalid credentials", "Permission denied", "Resource not found",
      "Malformed request"};
