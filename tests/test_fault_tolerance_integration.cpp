...
public:
    static bool test_banking_stage_fault_tolerance() {
        std::cout << "\n=== Testing Banking Stage Fault Tolerance Integration ===" << std::endl;
        ...
        if (!save_result.is_ok()) {
            std::cout << "❌ Failed to save banking state: " << save_result.error() << std::endl;
            return false;
        }
        ...
        if (!restore_result.is_ok()) {
            std::cout << "❌ Failed to restore banking state: " << restore_result.error() << std::endl;
            return false;
        }
        ...
        if (!process_result.is_ok()) {
            std::cout << "❌ Fault-tolerant transaction processing failed: " << process_result.error() << std::endl;
            return false;
        }
        std::cout << "✅ Fault-tolerant transaction processing working" << std::endl;
        
        return true;
    }
    
    static bool test_rpc_server_fault_tolerance() {
        std::cout << "\n=== Testing RPC Server Fault Tolerance Integration ===" << std::endl;
        
        ValidatorConfig config;
        config.rpc_bind_address = "127.0.0.1:18899"; // Use different port for testing
        ...
        try {
            ...
            // during actual RPC operations. For this test, we just verify initialization.
            
            return true;
        } catch (const std::exception& e) {
            std::cout << "❌ RPC Server initialization failed: " << e.what() << std::endl;
            return false;
        }
    }
    ...
    static bool test_recovery_manager_integration() {
        ...
        auto banking_checkpoint = std::make_shared<FileCheckpoint>("/tmp/banking_integration");
        auto rpc_checkpoint = std::make_shared<FileCheckpoint>("/tmp/rpc_integration");
        
        // Create some test data in the checkpoints
        std::vector<uint8_t> test_data = {1, 2, 3, 4, 5};
        auto banking_save = banking_checkpoint->save_data("integration_test_banking", test_data);
        auto rpc_save = rpc_checkpoint->save_data("integration_test_rpc", test_data);
        ...
        auto checkpoint_result = recovery_manager.create_system_checkpoint("integration_test");
        if (!checkpoint_result.is_ok()) {
            std::cout << "❌ System checkpoint failed: " << checkpoint_result.error() << std::endl;
            return false;
        }
        ...
        std::cout << "✅ Recovery manager integration working (checkpoint creation verified)" << std::endl;
    ...
    static bool test_circuit_breaker_integration() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (i < 3) {
                ...
                if (result.is_ok() || result.error().find("Circuit breaker") == std::string::npos) {
                    std::cout << "❌ Expected circuit breaker to block call " << i << std::endl;
                    return false;
                }
            }
        ...
        std::cout << "✅ Circuit breaker integration working correctly" << std::endl;
        return true;
    }
};
...
int main() {
    ...
    all_passed &= FaultToleranceIntegrationTest::test_banking_stage_fault_tolerance();
    all_passed &= FaultToleranceIntegrationTest::test_rpc_server_fault_tolerance();
    all_passed &= FaultToleranceIntegrationTest::test_recovery_manager_integration();
    all_passed &= FaultToleranceIntegrationTest::test_degradation_scenarios();
    all_passed &= FaultToleranceIntegrationTest::test_circuit_breaker_integration();
    
    if (all_passed) {
        std::cout << "\n✅ All fault tolerance integration tests passed!" << std::endl;
        std::cout << "🎉 The validator now has comprehensive fault tolerance mechanisms!" << std::endl;
        return 0;
    } else {
        std::cout << "\n❌ Some fault tolerance integration tests failed!" << std::endl;
        return 1;
    }
}