...
void test_genesis_validation() {
    ...
    auto validation_result = slonana::genesis::GenesisManager::validate_genesis_config(config);
    ...
    validation_result = slonana::genesis::GenesisManager::validate_genesis_config(config);
...
void test_mainnet_entrypoints() {
    ...
    auto mainnet_entrypoints = slonana::network::MainnetEntrypoints::get_mainnet_entrypoints();
    auto testnet_entrypoints = slonana::network::MainnetEntrypoints::get_testnet_entrypoints();
    ...
    for (const auto& entrypoint : mainnet_entrypoints) {
        ...
        ASSERT_TRUE(entrypoint.find("slonana.org") != std::string::npos); // Should be our domain
    }
...
void test_network_type_conversion() {
    ...
    // Test string to enum (compare as strings since enum can't be directly compared in ASSERT_EQ)
    ASSERT_EQ("mainnet", slonana::genesis::GenesisManager::network_type_to_string(
...
void test_economic_parameters() {
    ...
    ASSERT_EQ(1000000ULL, mainnet_config.economics.min_validator_stake); // 1M lamports
