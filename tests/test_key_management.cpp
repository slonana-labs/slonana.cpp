...
void test_key_utilities() {
    ...
    // Test key derivation
    {
        ...
        std::vector<uint8_t> salt = {0x12, 0x34, 0x56, 0x78, 0xAB, 0xCD, 0xEF, 0x00};
        ...
        auto key_result3 = key_utils::derive_key_from_passphrase("different_passphrase", salt);
...
void test_encrypted_file_key_store() {
    ...
    std::string storage_path = "/tmp/slonana_keystore_test_" + std::to_string(std::time(nullptr));
    
    {
        ...
        // Store key
        auto store_result = key_store->store_key("test_key_001", test_key.value(), metadata);
        ...
        auto update_result = key_store->update_metadata("test_key_001", updated_metadata);
...
void test_cryptographic_key_manager() {
    ...
    std::string storage_path = "/tmp/slonana_keymgr_test_" + std::to_string(std::time(nullptr));
    
    {
        ...
        policy.rotation_interval = std::chrono::seconds(1); // Very short for testing
...
void test_secure_validator_identity() {
    ...
    std::string storage_path = "/tmp/slonana_identity_test_" + std::to_string(std::time(nullptr));
...
void test_enhanced_validator_identity() {
    ...
    std::string storage_path = "/tmp/slonana_enhanced_test_" + std::to_string(std::time(nullptr));
...
void test_key_lifecycle_scenarios() {
    ...
    std::string storage_path = "/tmp/slonana_lifecycle_test_" + std::to_string(std::time(nullptr));
    
    {
        ...
        auto revoke_result = key_manager->revoke_key(rotated_key_id, "Manual test revocation");
        ...
        auto emergency_result = key_manager->emergency_revoke_all_keys("Emergency test");
