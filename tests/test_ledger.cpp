...
void test_block_serialization() {
    ...
    // Note: Vector comparison is complex, so we'll just check they're the same size
...
void test_ledger_manager_performance() {
    ...
    std::cout << "Performance: Stored 100 blocks in " << duration.count() << "ms" << std::endl;
...
void run_ledger_tests(TestRunner& runner) {
    ...
    runner.run_test("Ledger Manager Initialization", test_ledger_manager_initialization);
    runner.run_test("Ledger Manager Block Storage", test_ledger_manager_block_storage);
    runner.run_test("Ledger Manager Multiple Blocks", test_ledger_manager_multiple_blocks);
    runner.run_test("Ledger Manager Block By Hash", test_ledger_manager_block_by_hash);
    runner.run_test("Ledger Manager Invalid Operations", test_ledger_manager_invalid_operations);
    runner.run_test("Ledger Manager Performance", test_ledger_manager_performance);
}