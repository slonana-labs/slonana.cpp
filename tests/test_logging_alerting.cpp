...
void test_json_encoding_edge_cases() {
  ...
  logger.log_structured(LogLevel::ERROR, "test_json", "Edge case test message",
                        "JSON_EDGE_001", context);
...
void test_async_logging_bounded_queue() {
  ...
  // Generate more messages than the queue can hold to test bounded queue
  // behavior
...
void test_alert_rate_limiting() {
  ...
  // Send multiple alerts rapidly - only first should be processed due to rate
  // limiting
  for (int i = 0; i < 5; ++i) {
...
void test_structured_logging() {
  ...
  std::unordered_map<std::string, std::string> context = {
      {"component", "test"}, {"transaction_id", "12345"}};

  // This should not crash and should log appropriately
  logger.log_structured(LogLevel::INFO, "test_module", "Test message",
                        "TEST_001", context);
  logger.log_structured(LogLevel::ERROR, "test_module", "Error message",
                        "TEST_ERR_001");
...
void test_critical_failure_logging() {
  ...
  std::unordered_map<std::string, std::string> context = {
      {"error_type", "network_failure"}, {"peer_count", "0"}};

  // Test critical failure logging
  logger.log_critical_failure("network", "Network connectivity lost", "NET_001",
                              context);
...
void test_json_formatting() {
  ...
  logger.log_structured(LogLevel::ERROR, "test_module", "JSON test message",
                        "JSON_001");
...
void test_console_alert_channel() {
  ...
  LogEntry test_entry{std::chrono::system_clock::now(),
                      LogLevel::CRITICAL,
                      "test_module",
                      "main_thread",
...
void test_file_alert_error_handling() {
  ...
  auto file_channel =
      AlertChannelFactory::create_file_channel(invalid_path, true);
...
void test_end_to_end_alert_triggering() {
  ...
  logger.add_alert_channel(
      AlertChannelFactory::create_file_channel(temp_file, true));
  logger.add_alert_channel(AlertChannelFactory::create_console_channel(true));
  logger.add_alert_channel(
  ...
  std::unordered_map<std::string, std::string> context = {
      {"component", "network"}, {"peer_count", "0"}, {"last_seen", "30s ago"}};

  logger.log_critical_failure("network", "Complete network isolation detected",
                              "NET_E2E_001", context);
  ...
  ASSERT_TRUE(content.find("Complete network isolation detected") !=
              std::string::npos);
...
void test_slack_alert_channel() {
  auto slack_channel = AlertChannelFactory::create_slack_channel(
      "https://hooks.slack.com/test", true);
...
void test_logger_with_alert_channels() {
  ...
  logger.log_critical_failure("test_integration",
                              "Integration test critical failure", "INT_001");
