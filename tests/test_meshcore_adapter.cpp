...
TEST_F(MeshCoreAdapterTest, JoinWithBootstrapNodes) {
    ...
    auto join_ms = std::chrono::duration_cast<std::chrono::milliseconds>(join_duration).count();
...
TEST_F(MeshCoreAdapterTest, MeshJoinTimePerformance) {
    config_.bootstrap_nodes = {"127.0.0.1:9001", "127.0.0.1:9002", "127.0.0.1:9003"};
...
TEST_F(MeshCoreAdapterTest, PeerChurnRecovery) {
    ...
    // Check recovery time
    auto recovery_ms = std::chrono::duration_cast<std::chrono::milliseconds>(recovery_time).count();
