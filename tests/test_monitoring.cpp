...
void test_counter_creation_and_operation() {
  ...
  ASSERT_EQ(static_cast<int>(MetricType::COUNTER),
            static_cast<int>(counter->get_type()));
...
void test_gauge_creation_and_operation() {
  ...
  ASSERT_EQ(static_cast<int>(MetricType::GAUGE),
            static_cast<int>(gauge->get_type()));
...
void test_histogram_creation_and_operation() {
  ...
  auto histogram =
      registry->histogram("test_histogram", "Test histogram metric", buckets);
  ...
  ASSERT_EQ(static_cast<int>(MetricType::HISTOGRAM),
            static_cast<int>(histogram->get_type()));
...
void test_metrics_with_labels() {
  ...
  std::map<std::string, std::string> labels = {{"service", "validator"},
                                               {"environment", "test"}};

  auto counter = registry->counter("test_labeled_counter",
                                   "Test counter with labels", labels);
  ASSERT_TRUE(counter.get() != nullptr);
...
void test_global_metrics_registry() {
  ...
  auto counter =
      registry1.counter("global_test_counter", "Global test counter");
  counter->increment();
  ASSERT_EQ(1.0, counter->get_value());
...
void run_monitoring_tests() {
  ...
  runner.run_test("counter_creation_and_operation",
                  test_counter_creation_and_operation);
  runner.run_test("gauge_creation_and_operation",
                  test_gauge_creation_and_operation);
  runner.run_test("histogram_creation_and_operation",
                  test_histogram_creation_and_operation);
  runner.run_test("timer_functionality", test_timer_functionality);
  runner.run_test("metrics_with_labels", test_metrics_with_labels);
  runner.run_test("registry_metric_management",
                  test_registry_metric_management);
  runner.run_test("same_metric_name_returns_same_instance",
                  test_same_metric_name_returns_same_instance);
