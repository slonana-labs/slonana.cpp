...
private:
    ...
    for (int i = 0; i < num_threads; ++i) {
      threads.emplace_back([&, i]() {
        ...
        for (int j = 0; j < operations_per_thread; ++j) {
          ...
          request.request_id =
              "req_" + std::to_string(i) + "_" + std::to_string(j);
  ...
  static void run_load_balancer_request_queue_stress_test() {
    ...
    for (int i = 0; i < num_threads; ++i) {
      threads.emplace_back([&, i]() {
        for (int j = 0; j < operations_per_thread; ++j) {
          ConnectionRequest request;
          request.request_id =
              "queue_req_" + std::to_string(i) + "_" + std::to_string(j);
    ...
    std::cout << "  ✓ Lock-free queue active: "
              << (ops_per_sec > 10000 ? "YES ✓" : "NO") << "\n\n";
  }
    ...
    for (int i = 0; i < num_threads; ++i) {
      threads.emplace_back([&, i]() {
        ...
        for (int j = 0; j < operations_per_thread; ++j) {
          ...
          node.address =
              "192.168." + std::to_string(i) + "." + std::to_string(j % 256);
      ...
      threads.emplace_back([&, i]() {
        for (int j = 0; j < operations_per_thread; ++j) {
          ...
          if (j % 2 == 0) {
            ConnectionRequest request;
            request.request_id =
                "mixed_req_" + std::to_string(i) + "_" + std::to_string(j);
            request.service_name = "mixed_service";
            balancer.route_request(request);
          } else {
            NetworkNode node;
            node.node_id =
                "mixed_node_" + std::to_string(i) + "_" + std::to_string(j);
  ...
  static void run_queue_metrics_test() {
    ...
    DistributedLoadBalancer balancer("metrics_balancer", config,
                                     custom_capacity);
...
int main() {
  ...
  std::cout
      << "╔════════════════════════════════════════════════════════════╗\n";
  std::cout
      << "║  Network Layer Concurrency Stress Test Suite              ║\n";
  std::cout
      << "║  Testing Lock-Free and Async Patterns                     ║\n";
  std::cout
      << "╚════════════════════════════════════════════════════════════╝\n";
