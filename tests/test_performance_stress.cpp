...
public:
  ...
  uint64_t get_transactions_processed() const {
    return transactions_processed_.load();
  }
  uint64_t get_rpc_requests_processed() const {
    return rpc_requests_processed_.load();
  }
...
void test_high_throughput_transaction_processing() {
  ...
  double tps =
      static_cast<double>(total_transactions) / (duration.count() / 1000.0);
...
void test_concurrent_rpc_requests() {
  ...
  std::cout << "✅ Request rate: " << std::fixed << std::setprecision(2) << rps
            << " RPS" << std::endl;
...
void test_memory_usage_under_load() {
  ...
  std::cout << "✅ Growth factor: " << std::fixed << std::setprecision(2)
            << growth_factor << "x" << std::endl;
...
void test_network_protocol_stress() {
  ...
  std::cout << "✅ Message rate: " << std::fixed << std::setprecision(2)
            << messages_per_second << " messages/sec" << std::endl;
...
void test_validator_performance_scaling() {
  ...
  for (int load : load_levels) {
    ...
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
        end_time - start_time);
    double throughput =
        static_cast<double>(load) / (duration.count() / 1000000.0);
  ...
  double throughput_degradation =
      throughput_results.front() / throughput_results.back();
...
void test_failure_recovery() {
  ...
  std::cout << "✅ System recovered successfully from simulated failure"
            << std::endl;
}
...
void run_performance_stress_tests(TestRunner &runner) {
  runner.run_test("High Throughput Transaction Processing",
                  test_high_throughput_transaction_processing);
  ...
  runner.run_test("Validator Performance Scaling",
                  test_validator_performance_scaling);
