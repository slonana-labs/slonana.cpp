...
void test_poh_basic_functionality() {
    ...
    PohConfig config;
    config.target_tick_duration = std::chrono::microseconds(1000); // 1ms for faster testing
    ...
    std::cout << "PoH sequence progressed from " << initial_sequence << " to " << later_sequence << std::endl;
...
void test_poh_metrics_integration() {
    ...
    poh.set_tick_callback([](const PohEntry& entry) {
        ...
        metrics.set_poh_sequence_number(static_cast<int64_t>(entry.sequence_number));
    });
    
    poh.set_slot_callback([](Slot slot, const std::vector<PohEntry>& entries) {
        auto& metrics = slonana::monitoring::GlobalConsensusMetrics::instance();
        metrics.set_poh_current_slot(static_cast<int64_t>(slot));
        std::cout << "Slot " << slot << " completed with " << entries.size() << " entries" << std::endl;
    });
    ...
    auto stats = poh.get_stats();
    std::cout << "Final PoH stats: " << stats.total_ticks << " ticks generated" << std::endl;
...
void test_global_poh_instance() {
    ...
    std::cout << "Current PoH: sequence=" << current.sequence_number << ", slot=" << current_slot << std::endl;
