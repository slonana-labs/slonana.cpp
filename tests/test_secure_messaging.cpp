...
void test_secure_message_serialization() {
  ...
  msg.encrypted_payload =
      std::vector<uint8_t>{0x48, 0x65, 0x6C, 0x6C, 0x6F}; // "Hello"
...
void test_message_crypto_basic() {
  SecureMessagingConfig config;
  config.enable_message_signing =
      false; // Disable signing for test without keys
  config.enable_message_encryption =
      false; // Disable encryption for test without keys
  ...
  auto protect_result =
      crypto.protect_message(plaintext, "test_message", "test_peer");
...
void test_secure_messaging_end_to_end() {
  SecureMessagingConfig config;
  config.enable_tls =
      false; // Disable TLS for this test to avoid cert requirements
  config.enable_message_signing =
      false; // Disable signing for test without keys
  config.enable_message_encryption =
      false; // Disable encryption for test without keys
  ...
  auto prepare_result = messaging.prepare_outbound_message(
      plaintext, "gossip_message", "peer_123");

  ASSERT_TRUE(prepare_result.is_ok());

  auto encrypted_data = prepare_result.value();
  ASSERT_TRUE(encrypted_data.size() >
              plaintext.size()); // Should be larger due to headers

  // Test message processing
  auto process_result =
      messaging.process_inbound_message(encrypted_data, "peer_123");
  if (!process_result.is_ok()) {
    std::cout << "Process result error: " << process_result.error()
              << std::endl;
  }
...
void test_replay_protection() {
  ...
  // Prepare message
  auto prepare_result =
      messaging.prepare_outbound_message(plaintext, "test", "peer");
  ASSERT_TRUE(prepare_result.is_ok());

  auto encrypted_data = prepare_result.value();

  // First processing should succeed
  auto process_result1 =
      messaging.process_inbound_message(encrypted_data, "peer");
  ASSERT_TRUE(process_result1.is_ok());

  // Second processing of same message should fail (replay detection)
  auto process_result2 =
      messaging.process_inbound_message(encrypted_data, "peer");
  ...
  auto old_process_result =
      messaging.process_inbound_message(old_encrypted, "peer");
...
void test_security_statistics() {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    auto process_result =
        messaging.process_inbound_message(prepare_result.value(), "peer");
    ASSERT_TRUE(process_result.is_ok());
  }
...
void test_nonce_ttl_based_cleanup() {
  ...
  config.nonce_cache_size =
      1000; // Large enough to not trigger size-based cleanup
...
void test_nonce_generation_security() {
  ...
  // Should have bits set in most positions (not perfect test but catches
  // obvious issues)
...
int main() {
  ...
  try {
    ...
    std::cout << std::endl
              << "=== All Secure Messaging Tests Passed! ===" << std::endl;
    return 0;
