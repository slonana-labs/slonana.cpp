...
void test_quic_secure_data_fallback() {
  ...
  // When secure messaging is disabled, send_secure_data should fall back to
  // regular send
  std::vector<uint8_t> test_data = {0x48, 0x65, 0x6C, 0x6C, 0x6F}; // "Hello"

  // This should not crash even though connection doesn't exist
  bool result = client.send_secure_data("nonexistent_connection", 1, test_data,
                                        "test_message");
...
int main() {
  ...
  try {
    ...
    std::cout << std::endl
              << "=== All Secure Networking Integration Tests Passed! ==="
              << std::endl;
    return 0;
