...
public:
  static void test_rapid_init_shutdown() {
    ...
    // **PERFORMANCE OPTIMIZATION**: Initialize PRNG once for cycle delays with
    ...
    for (int cycle = 0; cycle < num_cycles; ++cycle) {
      try {
        ...
        slonana::consensus::Hash genesis_hash(
            32, static_cast<uint8_t>(cycle % 256));
        ...
        auto worker = [&](int thread_id) {
          // **PERFORMANCE OPTIMIZATION**: Initialize PRNG once per thread with
          // deterministic seeding for CI reproducibility
          std::mt19937 gen(54321 + cycle * 100 +
                           thread_id); // Fixed seed for deterministic CI runs
          ...
          while (!stop_workers.load(std::memory_order_acquire)) {
            try {
              ...
              for (auto &byte : tx_hash) {
                byte =
                    static_cast<uint8_t>((cycle + thread_id + local_ops) % 256);
              }

              // Try operations with individual exception handling
              try {
                (void)slonana::consensus::GlobalProofOfHistory::mix_transaction(
                    tx_hash);
              } catch (const std::exception &e) {
                // Expected during shutdown, don't log unless verbose
                if (verbose_output && local_ops < 5) {
                  std::cout << "ℹ️  Thread " << thread_id
                            << " mix_transaction error (expected): " << e.what()
                            << "\n";
                }
              }

              try {
                (void)slonana::consensus::GlobalProofOfHistory::
                    get_current_entry();
              } catch (const std::exception &e) {
                ...
                if (verbose_output && local_ops < 5) {
                  std::cout
                      << "ℹ️  Thread " << thread_id
                      << " get_current_entry error (expected): " << e.what()
                      << "\n";
                }
              }
              ...
              try {
                ...
                if (verbose_output && local_ops < 5) {
                  std::cout
                      << "ℹ️  Thread " << thread_id
                      << " get_current_slot error (expected): " << e.what()
                      << "\n";
                }
              }
              ...
              if (local_ops % 10 == 0) {
                ...
                std::this_thread::sleep_for(
                    std::chrono::nanoseconds(jitter_dist(gen)));
              }
    ...
    std::cout << "   - Successful cycles: " << successful_cycles.load() << "/"
              << num_cycles << "\n";
    std::cout << "   - Failed cycles: " << failed_cycles.load() << "\n";
    std::cout << "   - Success rate: "
              << (100.0 * successful_cycles.load() / num_cycles) << "%\n";

    if (failed_cycles.load() == 0) {
      std::cout
          << "🎉 Shutdown race test passed - no race conditions detected!\n";
    } else {
      std::cout
          << "❌ Some cycles failed - potential race conditions detected!\n";
    }
  }
};
...
int main() {
  ...
  try {
    ...
    std::cout
        << "💡 This test validates shutdown safety under high contention\n";
