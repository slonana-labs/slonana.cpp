...
void test_snapshot_source_parsing() {
  ...
  ASSERT_TRUE(SnapshotBootstrapManager::parse_snapshot_source("auto") ==
              SnapshotSource::AUTO);
  ASSERT_TRUE(SnapshotBootstrapManager::parse_snapshot_source("mirror") ==
              SnapshotSource::MIRROR);
  ASSERT_TRUE(SnapshotBootstrapManager::parse_snapshot_source("none") ==
              SnapshotSource::NONE);
  ASSERT_TRUE(SnapshotBootstrapManager::parse_snapshot_source("invalid") ==
              SnapshotSource::AUTO);
...
void test_snapshot_source_to_string() {
  ...
  ASSERT_TRUE(SnapshotBootstrapManager::snapshot_source_to_string(
                  SnapshotSource::AUTO) == "auto");
  ASSERT_TRUE(SnapshotBootstrapManager::snapshot_source_to_string(
...
void test_bootstrap_manager_initialization() {
  ...
  try {
    ...
    ASSERT_TRUE(manager.get_snapshot_directory() ==
                test_dir.string() + "/snapshots");
...
void test_needs_bootstrap() {
  ...
  fs::path test_dir =
      fs::temp_directory_path() / "slonana_bootstrap_needs_test";
  fs::create_directories(test_dir);
...
void test_progress_callback() {
  ...
  fs::path test_dir =
      fs::temp_directory_path() / "slonana_bootstrap_callback_test";
  fs::create_directories(test_dir);
  ...
  try {
    ...
    manager.set_progress_callback([&progress_messages](const std::string &phase,
                                                       uint64_t current,
                                                       uint64_t total) {
      progress_messages.push_back(phase + " (" + std::to_string(current) + "/" +
                                  std::to_string(total) + ")");
    });
