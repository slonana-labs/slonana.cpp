...
void test_spl_memo_program() {
  ...
  ASSERT_EQ(ExecutionResult::SUCCESS, outcome.result);
  ASSERT_TRUE(outcome.logs.find("Hello, Solana from C++!") !=
              std::string::npos);
...
void test_extended_system_program_nonce() {
  ...
  ASSERT_EQ(1, account.data[72]); // Initialized flag (32 authority + 32 nonce +
                                  // 8 lamports = 72)
...
void test_comprehensive_spl_ecosystem() {
  ...
  std::string memo_text =
      "Comprehensive SPL ecosystem test completed successfully";
  ...
  if (nonce_account_it == accounts.end()) {
    // Account not in the accounts map, need to create a direct context
    // execution
    ...
    auto advance_outcome =
        engine.execute_transaction({nonce_advance}, accounts);
    ASSERT_EQ(ExecutionResult::SUCCESS, advance_outcome.result);
  }

  std::cout << "Comprehensive SPL ecosystem test passed" << std::endl;
  std::cout
      << "Successfully executed: nonce init, ATA creation, memo, nonce advance"
      << std::endl;
}
...
void test_spl_governance_program() {
  ...
  realm_instruction.accounts.push_back(
      PublicKey(32, 0x02)); // Governance token mint
  realm_instruction.accounts.push_back(
      PublicKey(32, 0x03)); // Council token mint
  ...
  // Add required accounts
  proposal_instruction.accounts.push_back(
      PublicKey(32, 0x11)); // Proposal account
  proposal_instruction.accounts.push_back(PublicKey(32, 0x01)); // Realm account
  proposal_instruction.accounts.push_back(
      PublicKey(32, 0x12)); // Governance account
  proposal_instruction.accounts.push_back(PublicKey(32, 0x13)); // Proposer
  proposal_instruction.accounts.push_back(
      PublicKey(32, 0x14)); // Token owner record

  auto proposal_result =
      governance_program.execute(proposal_instruction, context);
  ...
  // Add required accounts
  vote_instruction.accounts.push_back(
      PublicKey(32, 0x21)); // Vote record account
  ...
  vote_instruction.accounts.push_back(
      PublicKey(32, 0x23)); // Token owner record
...
void test_spl_stake_pool_program() {
  ...
  // Add required accounts
  init_instruction.accounts.push_back(
      PublicKey(32, 0x01)); // Stake pool account
  ...
  init_instruction.accounts.push_back(
      PublicKey(32, 0x05)); // Withdraw authority
  ...
  std::memcpy(deposit_instruction.data.data() + 1, &deposit_amount,
              sizeof(uint64_t));

  // Add required accounts
  deposit_instruction.accounts.push_back(
      PublicKey(32, 0x01)); // Stake pool account
  ...
  deposit_instruction.accounts.push_back(
      PublicKey(32, 0x13)); // Pool token account
  ...
  auto deposit_result =
      stake_pool_program.execute(deposit_instruction, context);
...
void test_spl_multisig_program() {
  ...
  // Add required accounts
  create_instruction.accounts.push_back(
      PublicKey(32, 0x01)); // Multisig account
...
void test_complete_spl_ecosystem() {
  ...
  auto program_ids = registry.get_all_program_ids();
  ASSERT_EQ(6, program_ids.size()); // ATA, Memo, Extended System, Governance,
                                    // Stake Pool, Multisig
  ...
  std::cout << "Complete SPL ecosystem integration working correctly"
            << std::endl;
  std::cout << "Successfully tested: governance, stake pool, multisig programs"
            << std::endl;
}
...
int main() {
  ...
  try {
    ...
    runner.run_test("SPL ATA Program Idempotent",
                    test_spl_ata_program_idempotent);
    runner.run_test("SPL Memo Program", test_spl_memo_program);
    runner.run_test("Extended System Program Nonce",
                    test_extended_system_program_nonce);
    runner.run_test("SPL Program Registry", test_spl_program_registry);
    runner.run_test("Enhanced Engine with SPL Programs",
                    test_enhanced_engine_with_spl_programs);
    runner.run_test("SPL Programs Performance", test_spl_programs_performance);
    runner.run_test("Comprehensive SPL Ecosystem",
                    test_comprehensive_spl_ecosystem);
  ...
  std::cout << "- ✅ SPL Multisig program for multi-signature wallets"
            << std::endl;
