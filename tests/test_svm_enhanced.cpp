...
void test_spl_token_program() {
  ...
  init_mint_instruction.data = {
      0, 6, 0}; // InitializeMint instruction with 6 decimals + padding
...
void test_parallel_transaction_execution() {
  ...
  for (int batch = 0; batch < 3; ++batch) {
    ...
    for (int tx = 0; tx < 5; ++tx) {
      ...
      instruction.accounts[0].resize(32,
                                     batch * 10 + tx); // Unique write account
      instruction.accounts[1].resize(32, batch * 10 + tx +
                                             100); // Unique read account
  ...
  auto outcome =
      engine.execute_parallel_transactions(transaction_batches, accounts);
  ...
  std::cout << "Parallel execution completed in " << duration << " microseconds"
            << std::endl;
  std::cout << "Processed " << metrics.parallel_transactions_executed
            << " transaction batches" << std::endl;
}
...
void test_performance_improvements() {
  ...
  auto outcome_basic =
      basic_engine.execute_transaction(instructions, accounts_basic);
  ...
  std::vector<std::vector<slonana::svm::Instruction>> single_batch = {
      instructions};
  ...
  auto duration_enhanced =
      std::chrono::duration_cast<std::chrono::microseconds>(end_enhanced -
                                                            start_enhanced)
          .count();
  ...
  std::cout << "Basic engine execution time: " << duration_basic
            << " microseconds" << std::endl;
  std::cout << "Enhanced engine execution time: " << duration_enhanced
            << " microseconds" << std::endl;

  if (duration_enhanced < duration_basic) {
    double improvement =
        (double)(duration_basic - duration_enhanced) / duration_basic * 100.0;
...
void test_enhanced_engine_compatibility() {
  ...
  std::cout << "Enhanced engine maintains full compatibility with base class"
            << std::endl;
}
...
int main() {
  ...
  try {
    runner.run_test("Enhanced Engine Initialization",
                    test_enhanced_engine_initialization);
    ...
    runner.run_test("Enhanced Engine Compatibility",
                    test_enhanced_engine_compatibility);
