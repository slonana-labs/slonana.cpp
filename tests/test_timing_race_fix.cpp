...
public:
  static void test_concurrent_timing_access() {
    ...
    // Note: enable_lock_contention_tracking can be enabled for detailed
      ...
      for (int i = 0; i < operations_per_thread; ++i) {
        try {
          ...
          try {
            ...
            if (verbose_output) {
              std::cout << "⚠️  Thread " << thread_id
                        << " get_current_entry error: " << e.what() << "\n";
            }
          }

          // Minimal delay to allow other threads to compete with some
          // randomization
          if (i % 50 == 0) {
            // Add randomized jitter (0.5-1.5 microseconds) to emulate
            // real-world timing Using pre-initialized PRNG for performance
    ...
    std::cout << "✅ Timing race test completed in " << duration.count()
              << "ms\n";
    std::cout << "📊 Results:\n";
    ...
    if (failed_operations.load() == 0) {
      std::cout
          << "🎉 Timing race test passed - no race conditions detected!\n";
    } else {
...
int main() {
  ...
  try {
    ...
    std::cout << "💡 This test validates the fix for last_tick_time_ race "
                 "condition\n";
