...
bool test_lockout_basic() {
  ...
  assert(lockout.is_locked_out_at_slot(110) ==
         true); // 110 is within lockout (100 < 110 <= 132)
  assert(lockout.is_locked_out_at_slot(132) ==
         true); // 132 is the boundary (100 < 132 <= 132)
  assert(lockout.is_locked_out_at_slot(133) ==
         false); // 133 is beyond lockout (133 > 132)
  assert(lockout.is_locked_out_at_slot(100) ==
         false); // Cannot lock out the slot itself
  assert(lockout.is_locked_out_at_slot(99) ==
         false); // Slots before are not locked out

  // Test serialization
  auto serialized = lockout.serialize();
  assert(serialized.size() ==
         12); // 8 bytes for slot + 4 bytes for confirmation_count
...
bool test_lockout_manager() {
  ...
  assert(manager.is_slot_locked_out(105) ==
         false); // Beyond lockout of slot 100 (100 + 4 = 104)
  assert(manager.is_slot_locked_out(115) ==
         true); // Within lockout of slot 110 (110 + 8 = 118)
...
bool test_tower_basic() {
  ...
  assert(tower.can_vote_on_slot(56) ==
         false); // Within lockout period (55 + 1 = 56)
...
bool test_vote_state() {
  ...
  // Process another vote (this will remove expired votes with
...
bool test_lockout_edge_cases() {
  ...
  assert(lockout_utils::slots_conflict(100, lockout1, 105) ==
         true); // 105 is within 100 + 8
  assert(lockout_utils::slots_conflict(100, lockout1, 109) ==
         false); // 109 is beyond 100 + 8

  // Test lockout validation
  std::vector<Lockout> valid_lockouts = {Lockout(100, 2), Lockout(110, 1),
                                         Lockout(120, 3)};
  assert(lockout_utils::validate_lockouts(valid_lockouts) == true);

  std::vector<Lockout> invalid_lockouts = {Lockout(110, 1), // Out of order
                                           Lockout(100, 2)};
