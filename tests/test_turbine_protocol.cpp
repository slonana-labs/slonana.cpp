...
bool test_turbine_broadcast() {
    ...
    broadcast.set_send_callback([&](const Shred& shred, const std::vector<TurbineNode>& targets) {
        send_called = true;
        assert(!targets.empty());
    });
    
    broadcast.set_receive_callback([&](const Shred& shred, const std::string& from) {
        receive_called = true;
        assert(!from.empty());
    });
