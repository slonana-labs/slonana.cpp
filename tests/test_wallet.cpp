...
void test_device_type_to_string() {
  ASSERT_EQ(std::string("Ledger Nano S"),
            device_type_to_string(DeviceType::LEDGER_NANO_S));
  ASSERT_EQ(std::string("Ledger Nano X"),
            device_type_to_string(DeviceType::LEDGER_NANO_X));
  ASSERT_EQ(std::string("Trezor Model T"),
            device_type_to_string(DeviceType::TREZOR_MODEL_T));
  ASSERT_EQ(std::string("Unknown"), device_type_to_string(DeviceType::UNKNOWN));
}

void test_connection_status_to_string() {
  ASSERT_EQ(std::string("Disconnected"),
            connection_status_to_string(ConnectionStatus::DISCONNECTED));
  ASSERT_EQ(std::string("Connected"),
            connection_status_to_string(ConnectionStatus::CONNECTED));
  ASSERT_EQ(std::string("Ready"),
            connection_status_to_string(ConnectionStatus::READY));
  ASSERT_EQ(std::string("Error"),
            connection_status_to_string(ConnectionStatus::ERROR));
}

void test_signing_result_to_string() {
  ASSERT_EQ(std::string("Success"),
            signing_result_to_string(SigningResult::SUCCESS));
  ASSERT_EQ(std::string("User Cancelled"),
            signing_result_to_string(SigningResult::USER_CANCELLED));
  ASSERT_EQ(std::string("Device Error"),
            signing_result_to_string(SigningResult::DEVICE_ERROR));
  ASSERT_EQ(std::string("Timeout"),
            signing_result_to_string(SigningResult::TIMEOUT));
}
...
void test_hardware_wallet_factory() {
  ...
  ASSERT_THROWS(create_hardware_wallet(DeviceType::UNKNOWN),
...
void test_wallet_manager_initialization() {
  ...
  ASSERT_EQ(
      ...
      manager.get_available_devices().size()); // No devices discovered yet
...
void test_device_discovery() {
  ...
  // Test manual discovery (should return empty list since no real devices and
  // mock disabled)
...
void run_wallet_tests() {
  ...
  runner.run_test("connection_status_to_string",
  ...
  runner.run_test("wallet_manager_initialization",
                  test_wallet_manager_initialization);
  runner.run_test("wallet_manager_configuration",
                  test_wallet_manager_configuration);
