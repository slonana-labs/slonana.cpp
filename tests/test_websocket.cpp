...
void test_websocket_notifications() {
    ...
    ws_server->notify_signature_status("test_signature", "{\"confirmationStatus\":\"confirmed\"}");
    
    // Test slot change notification
    ws_server->notify_slot_change(100, "99", 95);
    
    // Test block change notification
    ws_server->notify_block_change(100, "test_hash", "{\"blockhash\":\"test_hash\"}");
    
    // Test program account change notification
    ws_server->notify_program_account_change("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA",
                                             "account_pubkey123",
...
void test_websocket_performance() {
    ...
    for (int i = 0; i < 100; ++i) {
        ws_server->notify_account_change("test_pubkey_" + std::to_string(i), 
    ...
    std::cout << "WebSocket performance test: 100 notifications in " << duration.count() << " microseconds" << std::endl;
...
int main() {
    ...
    try {
        ...
        std::cout << "📊 Account, signature, slot, and block subscriptions available" << std::endl;
        std::cout << "⚡ Production-ready for dApp real-time integration" << std::endl;
